...
vvi dp;
vector<vector<map<int, int>>> antidp;
vector<vector<set<int>>> edgesrem;
int sumdist(int u, int p, int d, int depth)
{
    if (depth == 0)
    {
    ...
    int O = d * 8 + depth;
    int ret = dp[u][d * 8 + depth];
    if (setcontains(antidp[u][d * 8 + depth], p)) ret -= antidp[u][d * 8 + depth][p];
...
int32_t main()
{
    ...
    edges.resize(n);
    edgesrem.resize(n, vector<set<int>>(64));
    rep(i, n - 1)
    {
        dread2(int, a, b);
    ...
    dp.resize(n, vi(64, 0));
    antidp.resize(n, vector<map<int, int>>(64));
