...
vector<set<int>> remedges;
vp2 dp;
vector<map<int, p2>> antidp;
p2 tot(int u, int p, vvi& edges)
{
    p2 ret = dp[u];
    if (setcontains(antidp[u], p)) ret = ret - antidp[u][p];


    for (auto it = remedges[u].begin(); it != remedges[u].end();)
    {
        if (*it == p)
        {
            it++;
...
int ans = 0;
void calc(int u, int p, int& d, vvi& edges)
{
    d++;
    ans += d;
    repe(e, edges[u])
    {
        if (e != p) calc(e, u, d, edges);
    }
    d++;
}
