...
int32_t main()
{
	...
	rep(i, k)
	{
		...
		while (q.size())
		{
			...
			repe(e, edges[u]) q.emplace(d - e.second, e.first);
		}
	}
	...
	rep(i, k)
	{
		pq.emplace(0, 1 << i, cities[i]);
	}

	int ans = inf;
	vvi visited(n, vi(1 << k));
	vvi dis(n, vi(1 << k, -inf));
	while (pq.size())
	{
		int d, vis, u;
		tie(d, vis, u) = pq.top();
		...
		rep(i, k)
		{
			if ((vis & (1 << i)) == 0)
			{
				if (d - dist[i][u] > dis[u][vis | (1 << i)])
				{
					dis[u][vis | (1 << i)] = d - dist[i][u];
					pq.emplace(d - dist[i][u], vis | (1 << i), u);
				}
			}
		}
		...
		repe(e, edges[u])
		{
			if (d - e.second > dis[e.first][vis])
			{
				dis[e.first][vis] = d - e.second;
				pq.emplace(d - e.second, vis, e.first);
			}
		}
	}
