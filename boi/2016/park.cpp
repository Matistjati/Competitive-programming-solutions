...
int32_t main()
{
	...
	{
		dread3(int, x, y, r);
		trees.emplace_back(x, y, r);
	}

	typedef tuple<double, int, int> dp3;
	vector<dp3> intersections;
	int bottomwall = n;
	int leftwall = n + 1;
	int topwall = n + 2;
	int rightwall = n + 3;
	rep(i, n)
	{
		int x, y, r;
		tie(x, y, r) = trees[i];
		intersections.emplace_back(y - r, i, bottomwall);
		intersections.emplace_back(x - r, i, leftwall);
		intersections.emplace_back(abs(h - y) - r, i, topwall);
		intersections.emplace_back(abs(w - x) - r, i, rightwall);
		repp(j, i + 1, n)
		{
			if (i == j) continue;
			int x2, y2, r2;
			tie(x2, y2, r2) = trees[j];
			intersections.emplace_back(sqrt(dist(p2(x, y), p2(x2, y2))) - r - r2, i, j);
		}
	}
	...
	repe(q, queries)
	{
		...
		while (p < intersections.size() && get<0>(intersections[p]) + 0.001 < r * 2)
		{
			int d, a, b;
			tie(d, a, b) = intersections[p];
			uf.merge(a, b);
			p++;
		}

		int qans = 1 | 2 | 4 | 8;
		if (uf.find(leftwall) == uf.find(topwall))
		{
			if (c == topleft) qans = topleft;
			else qans &= ~topleft;
		}

		if (uf.find(topwall) == uf.find(rightwall))
		{
			if (c == topright) qans = topright;
			else qans &= ~topright;
		}

		if (uf.find(rightwall) == uf.find(bottomwall))
		{
			if (c == bottomright) qans = bottomright;
			else qans &= ~bottomright;
		}

		if (uf.find(bottomwall) == uf.find(leftwall))
		{
		...
		if (uf.find(topwall) == uf.find(bottomwall))
		{
