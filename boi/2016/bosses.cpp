...
int eval(vi& parent)
{
	...
	queue<int> isleaf;
	rep(i, n) if (deg[i] == 0) isleaf.push(i);
	vi val(n);
	while (isleaf.size())
	{
		int u = isleaf.front();
...
int32_t main()
{
	...
	rep(i, n)
	{
		...
		vvp2 pq(n + 1);
		repe(c, children[i]) pq[1].emplace_back(c, i);

		int rounds = 0;
		int ind = 0;
		while (rounds < n - 1)
		{
			if (ind >= pq.size()) break;
			repe(v, pq[ind])
			{
				int u, p;
				tie(u, p) = v;
				...
				depth[u] = ind;
				rounds++;
				repe(c, children[u]) if (ind + 1 < pq.size()) pq[ind + 1].emplace_back(c, u);
			}
			ind++;
		}

		if (rounds < n - 1) continue;
		ans = min(ans, eval(parent));
	}
