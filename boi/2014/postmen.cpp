...
int dfs(int u, vvp2& edges, vvi& sols, vi& deg, vi& intour, vi& edgedel)
{
    if (intour[u])
    {
        sols.push_back({});
        sols.back().push_back(u);
        return u;
    }
    intour[u]++;
    ...
    while (edges[u].size())
    {
        ...
        int k = dfs(v, edges, sols, deg, intour, edgedel);
        if (k != -1 && u != k)
        {
            intour[u]--;
            sols.back().push_back(u);
...
int32_t main()
{
    ...
    {
        if (!deg[i]) continue;
        dfs(i, edges, sols, deg, intour, edgedel);
    }
