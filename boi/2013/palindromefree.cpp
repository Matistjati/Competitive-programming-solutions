...
int comp(int index, bool unlimited, bool nonzero, int prev, int prevprev, string& digits, vvvvi& dp)
{
    if (index == digits.size()) return 1;

    int& v = dp[index][prev + 1][prevprev + 1][(unlimited << 1) + nonzero];
    if (v != -1) return v;
    int ret = 0;


    int hi = unlimited ? 10 : digits[index] - '0' + 1;
    rep(d, hi)
    {
        if (d == prev || d == prevprev) continue;
        bool newnonzero = nonzero || d != 0;
        ret += comp(index + 1, unlimited || (d != hi - 1), newnonzero, newnonzero ? d : -1, newnonzero ? prev : -1, digits, dp);
    }
...
{
    if (hi == -1) return 0;
    string digits = to_string(hi);


    vvvvi dp(digits.size(), vvvi(11, vvi(11, vi(4, -1))));
    return comp(0, false, false, -1, -1, digits, dp);
}
