...
bool iseven(int u, int p, int s, int d, vector<map<int, int>>& edges)
{
    if (u == s && u != p) return d % 2 == 0;

    repe(e, edges[u]) if (e.first != p) return iseven(e.first, u, s, d + 1, edges);
}

void cancelcycle(int u, int p, int s, int d, int& cum, p2& ret, vector<map<int, int>>& edges, vi& flow)
{
    if (u == s && u != p) return;
    if (d % 2 == 0) cum += flow[u];
    else cum -= flow[u];
    ...
    repe(e, edges[u])
    {
        ...
        ret = { e.second,u };
        cancelcycle(e.first, u, s, d + 1, cum, ret, edges, flow);
        break;
    }
}

void solvetree(vi& flow, vector<map<int, int>>& edges, vi& ans)
{
    int n = flow.size();
    queue<int> leafs;
    rep(i, n) if (edges[i].size() == 1) leafs.emplace(i);
    while (leafs.size())
    {
        int l = leafs.front();
        leafs.pop();
        ...
        ans[begin(edges[l])->second] = flow[l] * 2;
        int t = begin(edges[l])->first;
        flow[t] -= flow[l];
...
int32_t main()
{
    ...
    vi ans(m);
    solvetree(flow, edges, ans);

    // Check if even cycle remains
    rep(i, n)
    {
        if (edges[i].size())
        {
            if (iseven(i, i, i, 0, edges))
            {
                cout << "0";
                quit;
            }
            else
            {
                ...
                int cum = 0;
                p2 ret;
                cancelcycle(i, i, i, 0, cum, ret, edges, flow);
                flow[i] -= cum / 2;
                flow[ret.second] -= cum / 2;
                ans[ret.first] = cum;
                edges[i].erase(ret.second);
                edges[ret.second].erase(i);
                solvetree(flow, edges, ans);
            }
        }
    }
