...
int32_t main()
{
	...
	rep(i, n)
	{
		rep(j, m)
		{
			if (grid[i][j] == '\\') grid[i][j] = DOWN;
			else grid[i][j] = UP;
		}
	}
	priority_queue<p4> pq;
	pq.emplace(-(grid[0][0] != DOWN), DOWN, 0, 0);

	vvvi vis(n, vvi(m, vi(2)));
	vp3 dirsdown = { {1,0,UP},{1,1,DOWN},{0,1,UP},{0,-1,UP},{-1,0,UP},{-1,-1,DOWN} };
	vp3 dirsup = { {1,0,DOWN},{0,-1,DOWN},{1,-1,UP},{-1,1,UP},{0,1,DOWN},{-1,0,DOWN} };
	while (pq.size())
	{
		int d, align, r, c;
		tie(d, align, r, c) = pq.top();
		pq.pop();

		if (vis[r][c][align]) continue;
		vis[r][c][align] = 1;

		if (r == n - 1 && c == m - 1 && align == DOWN)
		{
			cout << -d;
			quit;
		}

		vp3& transitions = align == DOWN ? dirsdown : dirsup;

		repe(dir, transitions)
		{
			p2 np = p2(r, c) + p2(get<0>(dir), get<1>(dir));
			if (np.first < 0 || np.second < 0 || np.first >= n || np.second >= m) continue;
			pq.emplace(d - (get<2>(dir) != grid[np.first][np.second]), get<2>(dir), np.first, np.second);
		}
	}
	cout << "NO SOLUTION";
