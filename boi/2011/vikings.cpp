...
int32_t main()
{
	...
	rep(i, n)
	{
		rep(j, m)
		{
			if (grid[i][j] == 'V') vikingstart = p2(i, j);
			if (grid[i][j] == 'Y') mestart = p2(i, j);
		}
	}

	vvi killzone(n, vi(m));
	vvvi killzonevis(n, vvi(m, vi(4)));
	vvp2 pqme(n + m + 1);
	pqme[0].emplace_back(mestart);
	vvp2 pqviking(n + m + 1);
	pqviking[0].emplace_back(vikingstart);
	vvi visviking(n, vi(m));
	vvi visme(n, vi(m));
	rep(i, pqme.size())
	{


		while (pqviking[i].size())
		{
			p2 u = pqviking[i].back();
			pqviking[i].pop_back();

			if (visviking[u.first][u.second]) continue;
			visviking[u.first][u.second] = 1;

			repe(dir, dirs)
			{
				p2 np = dir + u;
				if (np.first < 0 || np.second < 0 || np.first >= n || np.second >= m) continue;
				if (grid[np.first][np.second] == 'I') continue;
				if (i + 1 < pqviking.size()) pqviking[i + 1].emplace_back(np);
				rep(j, dirs.size())
				{
					p2 dir2 = dirs[j];
					p2 k = np;
					while (true)
					{
						killzone[k.first][k.second] = 1;
						k += dir2;
						if (k.first < 0 || k.second < 0 || k.first >= n || k.second >= m) break;
						if (grid[k.first][k.second] == 'I') break;
						if (killzonevis[k.first][k.second][j]) break;
						killzonevis[k.first][k.second][j] = 1;
					}
				}
			}
		...
		while (pqme[i].size())
		{
			...
			if (visme[u.first][u.second]) continue;
			visme[u.first][u.second] = 1;
			...
			repe(dir, dirs)
			{
				p2 np = dir + u;
				if (np.first < 0 || np.second < 0 || np.first >= n || np.second >= m) continue;
				if (grid[np.first][np.second] == 'I' || killzone[np.first][np.second]) continue;
				if (i + 1 < pqme.size()) pqme[i + 1].emplace_back(np);
			}
		}
	}
	cout << "NO";
