...
int32_t main()
{
    ...
    rep(i, n) read(strings[i]);

    vvi diffs(m, vi(4));
    vector<ull> rowW(n);
    uniform_int_distribution<ull> dist(0);
    rep(i, n) rowW[i] = dist(rng);
    //rowW = { 2,5,3,7 };
    ull s = 0;
    rep(i, n) s += rowW[i];
    ...
    rep(c, 4)
    {
        rep(col, m)
        {
            ...
            rep(row, n)
            {
                if (strings[row][col] == chars[c]) colsum += rowW[row];
            }
            diffs[col][c] = colsum;
        }
    }
    ...
    rep(i, n)
    {
        ...
        rep(j, m)
        {
            rep(c, 4)
            {
                if (chars[c] == strings[i][j]) continue;
                rowsum += diffs[j][c];
            }
        }

        if (rowsum==(s-rowW[i]) * k)
        {
            cout << i + 1;
            quit;
        }
