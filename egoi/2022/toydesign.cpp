...
int nrq = 0;
int iscon(int a, int i, int j) { return Connected(a, i + 1, j + 1); }
...
void ToyDesign(int n, int max_ops)
{
    ...
    rep(i, n - 1)
    {
        int v = chain.size() ? chain.back().first : 0;
        int k = iscon(v, curr, curr - 1);

        if (k != chain.back().first)
        {
            comp.push_back(++compcnt);
            chain.emplace_back(k, comp.back());
        ...
        else
        {
            ...
            {
                int m = (l + r) / 2;
                if (iscon(chain[m].first, curr, chain[m].second) != chain[m].first)
                {
                    l = m;
                }
                else
    ...
    repe(comp, comps)
    {
        repp(i, 1, comp.second.size())
        {
            edges.emplace_back(comp.second[0] + 1, comp.second[i] + 1);
        }
    }
