...
int32_t main()
{
    ...
    while (good.size() < n)
    {
        ...
        {
            int k = randint(1, 4e5);
            if (setcontains(seen, k) || setcontains(seengood, k)) continue;
        ...
        {
            int s = 0;
            rep(i, has.size()) if (mask & (1 << i)) s += has[i];
            if (!setcontains(seen, s) && s != 0 && find(all(good), s) == good.end())
            {
                seengood.insert(s);
                good.push_back(s);
            }
    ...
    while (good.size() > n) good.pop_back();
    ...
    rep(mask, 1 << has.size())
    {
        ...
        if (s == k)
        {
            cout << "! 1 " << popcount(mask) << "\n";
            cout << k << "\n";
            rep(i, has.size()) if (mask & (1 << i)) cout << has[i] << " ";
            cout << endl;
        }
    }
