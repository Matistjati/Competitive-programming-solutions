...
int dfs(int u, vb& vis, vb& unallowed, vi& type, vvp2& edges)
{
	if (unallowed[u]) return 0;
	if (vis[u]) return 0;
	vis[u] = 1;
	int ret = type[u] == 1;
	repe(e, edges[u]) ret += dfs(e.first, vis, unallowed, type, edges);
	return ret;
}


bool possible(vi& type, vvp2& edges, vi& dist, int mid, bool printans)
{
	int n = type.size();
	...
	queue<int> q;
	rep(i, n) if (type[i] == -1) q.emplace(i);
	...
	while (q.size())
	{
		...
		if (type[u] == -1)
		{
			repe(e, edges[u]) q.emplace(e.first);
		}
		else
		{
			if (type[u] == 0)
	...
	vb vis2(n);
	int numhikers = 0;
	rep(i, n) if (type[i] == 1) numhikers++;
	rep(i, n)
	{
		if (type[i] == 1)
		{
			if (dfs(i, vis2, blocked, type, edges) != numhikers)
			{
				return 0;
			}
			break;
...
int32_t main()
{
	...
	rep(i, n) if (type[i] == 1) pq.emplace(0, i);
	vi dist(n, inf);
	while (pq.size())
	{
		int d, u;
	...
	while (hi - lo > 1)
	{
		...
		if (possible(type, edges, dist, mid, 0))
		{
			hi = mid;
		}
		else
