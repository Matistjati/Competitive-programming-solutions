...
int ans(int left, bool placeA, vvi& dp)
{
    if (left == 0) return 1;

    int& v = dp[left][placeA];
    ...
    if (!placeA)
    {
        for (int i = 2; left-i >= 0; i+=2)
        {
            ret += ans(left - i, !placeA, dp) % mod;
            ret %= mod;
        }
    }
    else
    {
        for (int i = 1; left - i >= 0; i += 2)
        {
            ret += ans(left - i, !placeA, dp) % mod;
            ret %= mod;
        }
...
int32_t main()
{
    ...
    dread(int, n);
    vvi dp(n + 1, vi(2, -1));
    cout << (ans(n, true, dp) + ans(n, false, dp))%mod;
