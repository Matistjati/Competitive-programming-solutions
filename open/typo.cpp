...
struct stringhash
{
    ...
    stringhash(int mod) : base(randint(200LL, int(1e8))), mod(mod) {}
...
int32_t main()
{
    ...
    repe(w, dict)
    {
        int v = (s1.hashstring(w)) + (s2.hashstring(w) << 32);
        seenwords.insert(v);
    }
    ...
    repe(w, dict)
    {
        ...
        rep(i, a.size())
        {
            if (setcontains(seenwords, (a[i] + (b[i] << 32))))
            {
                any = 1;
                cout << w << "\n";
                break;
