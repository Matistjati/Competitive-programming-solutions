...
set<vi> ret;
vvi choices;
void generatechoices(vi curr, vi& maxel)
{

    if (curr.size() == maxel.size())
    {
        ret.insert(curr);
        return;
    }
    ...
    rep(i, maxel[curr.size()]+1)
    {
        vi c(curr);
        c.push_back(i);
        generatechoices(c, maxel);
    }
}
...
hashType stringHash(vi& s, int index)
{
    int pow = 153;
    hashType ret = 0;
    hashType base = 1;
...
int best(int index, vi powers, int sum)
{
    if (sum == 0 || index==choices.size())
    {
        return 0;
    }
    int h = stringHash(powers, index);
    if (setcontains(memo, h)) return memo[h];


    int ret = 0;
    ret = max(ret, best(index + 1, powers, sum));
    {
        bool works = true;
        rep(i, powers.size())
        {
            powers[i] -= choices[index][i];
            sum -= choices[index][i];
            works &= powers[i] >= 0;
        }
        ...
        if (works)
        {
            ret = max(ret, 1+best(index + 1, powers, sum));
        }
    }
...
int32_t main()
{
    ...
    dread(int, n);
    vi powers;
    repe(p, power_list(factor(n))) powers.push_back(p.second);

    int ans = powers.size();
    repe(p, powers) p--;
    powers.erase(remove(all(powers), 0), powers.end());


    generatechoices({}, powers);
    // Remove illegal
    vi emptyset(powers.size(), 0);
    ret.erase(emptyset);
    // Remove all we already greedily picked
    rep(i, powers.size())
    {
        vi emptyset(powers.size(), 0);
        emptyset[i] = 1;
    ...
    repe(v, ret) choices.push_back(v);
    cout << best(0, powers, accumulate(all(powers), 0)) + ans;
