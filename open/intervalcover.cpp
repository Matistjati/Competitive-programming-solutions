...
int32_t main()
{
    ...
    while (cin >> lo >> hi)
    {
        int n;
        cin >> n;
        vector<tuple<long double, long double, int>> intervals;

        rep(i, n)
        {
            long double a, b;
            cin >> a >> b;
            intervals.emplace_back(a, b, i);
        }
        sort(all(intervals));
        ...
        {
            int bestind = -1;
            long double bestv = lo;

            while (i < n && get<0>(intervals[i]) <= lo)
            {
                long double l, r; int ind;
                tie(l, r, ind) = intervals[i];
            ...
            ans.push_back(get<2>(intervals[bestind]));
            lo = bestv;
        } while (lo < hi);
