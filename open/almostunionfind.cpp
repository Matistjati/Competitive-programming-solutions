...
struct UF
{
    ...
    vi par;
    int n;
    UF(int n) : n(n), nodes(n), par(n), sum(n)
    {
        rep(i, n) sum[i] = i + 1;
        rep(i, n) par[i] = i;
        rep(i, n) nodes[i].insert(i);
    }
    ...
    void merge(int a, int b)
    {
        ...
        repe(node, nodes[b]) nodes[a].insert(node);
        repe(node, nodes[b]) par[node] = a;
        nodes[b] = set<int>();
        sum[a] += sum[b];
        sum[b] = 0;
    }
