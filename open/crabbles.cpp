...
struct Node
{
    ...
    void build(int i, string& s)
    {
        ...
        children[s[i]]->build(i + 1, s);
    }

    int dfs(vi& cnt, vvi& vals, int v)
    {
        int ret = isterminal ? v : 0;
        repe(e, children)
        {
            int c = e.first - 'a';
            if (cnt[c] < vals[c].size())
            {
                cnt[c]++;
                ret = max(ret, e.second->dfs(cnt, vals, v + vals[c][cnt[c] - 1]));
                cnt[c]--;
            }
        }
        return ret;
...
int32_t main()
{
    ...
    rep(i, m)
    {
        ...
        rep(j, 26) sort(rall(vals[j]));
        cout << root->dfs(cnt, vals, 0) << "\n";
    }
