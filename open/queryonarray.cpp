...
struct Mod {
    ll x;
    Mod(ll xx) : x(xx) { normalize(); }
    Mod() : x(0) {}
    void normalize() { x %= mod; x += mod; x %= mod; }
...
struct Tree
{
    ...
    Tree(int n, vector<Mod> pfi) : n(n), tree(4 * n), lazy(4 * n), prefixI(pfi) {}

    void push(int x, int l, int r) {
        int mid = (l + r) / 2;
        tree[x * 2] = tree[x * 2] + lazy[x] * (prefixI[mid] - prefixI[max(0LL, l - 1)]);
...
int32_t main()
{
    ...
    vector<vector<Mod>> sum(4, vector<Mod>(n));
    repp(i, 1, n)
    {
    ...
    while (q--)
    {
        ...
        if (t == 1)
        {
            rep(i, 4) trees[i].add(l, r, poly(l, i));
        }
        else if (t == 2)
        {
            rep(i, 4) trees[i].add(l, r, -poly(l, i));
