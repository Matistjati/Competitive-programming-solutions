...
int32_t main()
{
    ...
    int rank = 25;
    int stars = 0;
    map<int, int> rankToStar;
    repp(i, 1, 11) rankToStar[i] = 5;
    repp(i, 11, 16) rankToStar[i] = 4;
    repp(i, 16, 21) rankToStar[i] = 3;
    repp(i, 21, 26) rankToStar[i] = 2;
    repe(game, games)
    {
        bool win = game == 'W';
        if (rank == 0) break;
        ...
        if (win)
        {
            ...
            if (rankToStar[rank]<stars)
            {
                stars -= rankToStar[rank];
                rank--;
            }
        }
        //cout << stars << " " << rank << "\n";
    }
    cout << ((rank!=0)?to_string(rank):"Legend");
    quit;
}