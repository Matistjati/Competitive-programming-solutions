...
bool bpm(int u, vb& vis, vi& match, vvb& edges)
{
    ...
    rep(i, edges.size())
    {
        if (edges[u][i])
        {
            if (match[i - match.size()] == -1 || bpm(match[i - match.size()], vis, match, edges))
            {
                match[i - match.size()] = u;
                return true;
            }
        }
    }
...
int32_t main()
{
    ...
    vvi square(n, vi(n));
    rep(i, n) rep(j, n) read(square[i][j]);


    set<int> numstoplace;
    rep(i, n) numstoplace.insert(i + 1);

    rep(i, n) rep(j, n) numstoplace.erase(square[i][j]);
    ...
    rep(_, n - k)
    {
        ...
        int num = *numstoplace.begin();
        numstoplace.erase(numstoplace.begin());

        vb vis(n * 2);
        vi match(n, -1); // col to row
        rep(i, n)
        {
            if (bpm(i, vis, match, edges))
                rep(i, n) vis[i] = 0;
        }

        rep(i, n)
        {
            square[match[i]][i] = num;
        }
