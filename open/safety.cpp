...
double solve(int i, vi& nums)
{
    ...
    rep(_, 30)
    {
        ...
        rep(j, nums.size())
        {
            ...
            double theirscore = nums[j];
            totcost += (myscore + 0.0000001 - theirscore) / s;
        }
        if (totcost + mid >= 1)
        {
            hi = mid;
...
int32_t main()
{
    ...
    rep(FUCKYOU, t)
    {
        ...
        rep(i, n)
        {
            cout << fixed << setprecision(15) << solve(i, nums) * 100;
            if (i != n - 1) cout << " ";
        }
        if (FUCKYOU != t - 1) cout << "\n";
    }
