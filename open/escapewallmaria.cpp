...
int32_t main()
{
    ...
    int fastest = inf;
    vp3 dirs = { {0,1,'L'},{0,-1,'R'}, {1,0,'U'}, {-1,0,'D'} };
    queue<pair<p2, int>> q;
    p2 start = { -1,-1 };
    rep(i, n) rep(j, m) if (grid[i][j] == 'S') start = { i,j };
    q.emplace(start, 0);
    vvb vis(n, vb(m));
    while (q.size())
    {
        p2 pos; int cost;
        tie(pos, cost) = q.front();
        q.pop();
        ...
        repe(dir, dirs)
        {
            p2 np = pos + p2(get<0>(dir), get<1>(dir));

            if (grid[np.first][np.second]=='0'|| grid[np.first][np.second]==get<2>(dir))
            {
                q.emplace(np, cost + 1);
            }
        }
    ...
    cout << ((t<fastest) ? "NOT POSSIBLE" : to_string(fastest));
