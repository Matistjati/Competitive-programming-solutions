...
vpll* factorprecomp(ull v)
{
    static map<ull, vpll> seen;
    if (setcontains(seen, v)) return &seen[v];
    seen[v] = power_list(factor(v));
    return &seen[v];
}
...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        vpll* factors = factorprecomp(exponent);

        if (binpow(v, exponent + 1, sz) != v)
        {
            cout << "-1";
            cerr << "\nno solution exp phi(k)";
            quit;
        }
        ...
        repe(l, *factors)
        {
            rep(i, l.second)
            {
                if (binpow(v, (exponent / l.first) + 1, sz) == v)
                {
                    exponent /= l.first;
                }
                else
