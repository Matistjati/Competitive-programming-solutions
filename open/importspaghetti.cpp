...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        rep(j, k)
        {
            ...
            repe(dep, dependencies)
            {
                edges[nameToId[dep]].emplace_back(myId);
            }
        }
    }
    ...
    rep(i, n)
    {
        ...
        vi cycle;
        int lastCounter = 0;
        while (last!=-1&&lastCounter<1)
        {
            if (last==i) lastCounter++;
            cycle.push_back(last);
            last = vis[last];
        }

        if (cycle.size() && cycle.size() < shortestlen)
        {
            shortest = cycle;
    ...
    if (shortest.empty()) cout << "SHIP IT";
    else
    {
        repe(c, shortest) cout << idToName[c] << " ";
    }
