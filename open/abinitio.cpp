...
const int h_base = 4003;
inline int p2hash(int a, int b) {
    return a + b * h_base;
}

const int max_v = 16020000;
typedef bitset<max_v> edgeset;
...
int32_t main()
{
    ...
    edgeset* edgesr = new edgeset();
    edgeset* edgesc = new edgeset();
    edgeset* edgesrc = new edgeset();

    rep(i, e)
    {
        dread2(int, a, b);
        edges->set(p2hash(a, b));
        edgesr->set(p2hash(b, a));
    }

    rep(i, v) rep(j, v) if (i != j)
    {
        edgesc->set(p2hash(i, j));
    }
    edgesrc = edgesc;

    rep(i, v) rep(j, v) if (i != j) edgesc->set(p2hash(i, j));
    rep(i, v) rep(j, v) if (i != j) edgesrc->set(p2hash(j, i));
    edgesc = new edgeset(*edgesc & ~*edges);
    edgesrc = new edgeset(*edgesrc & ~*edgesr);
    ...
    while (q--)
    {
        ...
        if (t == 1)
        {
            rep(i, g)
            {
                edgesc->set(p2hash(i, g));
                edgesc->set(p2hash(g, i));
                edgesrc->set(p2hash(g, i));
                edgesrc->set(p2hash(i, g));
            }
            g++;
        }
        else if (t == 2)
        {
            ...
            edges->set(p2hash(a, b));
            edgesr->set(p2hash(b, a));
            edgesc->set(p2hash(a, b), 0);
            edgesrc->set(p2hash(b, a), 0);
        }
        else if (t == 3)
        {
            dread(int, x);
            ...
            rep(i, g)
            {
                if (i == x) continue;
                p2 ed = { i, x };
                edges->set(p2hash(ed.first, ed.second), 0);
                edgesc->set(p2hash(ed.first, ed.second));
                edgesr->set(p2hash(ed.first, ed.second), 0);
                edgesrc->set(p2hash(ed.first, ed.second));
        ...
        {
            dread2(int, a, b);
            edges->set(p2hash(a, b), 0);
            edgesr->set(p2hash(b, a), 0);
            edgesc->set(p2hash(a, b));
            edgesrc->set(p2hash(b, a));
        }
        else if (t == 5)
        {
            swap(edges, edgesr);
