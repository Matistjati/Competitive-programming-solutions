...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        {
            dread2(int, c, v);
            cardstack.push_back(p2(c, v));
            cards.erase(p2(-cnt[v], v));
            cnt[v] += c;
            cards.emplace(p2(-cnt[v], v));
        }
        if (t == "PUT_BOTTOM")
        {
            dread2(int, c, v);
            cardstack.push_front(p2(c, v));
            cards.erase(p2(-cnt[v], v));
            cnt[v] += c;
            cards.emplace(p2(-cnt[v], v));
        }
        if (t == "REMOVE_TOP")
        {
            ...
            while (c > 0)
            {
                if (c >= cardstack.back().first)
                {
                    int count, u;
                    tie(count, u) = cardstack.back();
                    cardstack.pop_back();
                    cards.erase(p2(-cnt[u], u));
                    cnt[u] -= count;
                    cards.emplace(p2(-cnt[u], u));
                    c -= count;
                }
                else
                {
        ...
        if (t == "REMOVE_BOTTOM")
        {
            ...
            while (c > 0)
            {
                if (c >= cardstack.front().first)
                {
                    int count, u;
                    tie(count, u) = cardstack.front();
                    cardstack.pop_front();
                    cards.erase(p2(-cnt[u], u));
                    cnt[u] -= count;
                    cards.emplace(p2(-cnt[u], u));
