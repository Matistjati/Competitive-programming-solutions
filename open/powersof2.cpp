...
vvvi dp;
int ans(int index, bool unlimited, int prefixmatch, vi& num, vi& pattern)
{
    if (index == num.size()) return prefixmatch == pattern.size();

    int& v = dp[index][prefixmatch][unlimited];
    if (v != -1) return v;

    int ret = 0;
    int high = unlimited ? 9 : num[index];
    rep(d, high+1)
    {
        int pref = prefixmatch;
        if (pref < pattern.size())
        {
            if (d == pattern[pref]) pref++;
            else
            {
                // Find largest prefix that matches
                vi k;
                rep(i, prefixmatch) k.push_back(pattern[i]);
                k.push_back(d);
                pref = 0;
                repp(i, 0, k.size())
                {
        ...
        ret += ans(index + 1, unlimited || (d < num[index]), pref, num, pattern);
    }
    return (v=ret);
}
...
int32_t main()
{
    ...
    vi num;
    repe(s, to_string(n)) num.push_back(s - '0');
    vi pattern;

    repe(s, to_string(binpow(2, e))) pattern.push_back(s - '0');
    dp = vvvi(num.size(), vvi(pattern.size() + 1, vi(2, -1)));
    cout << ans(0, 0, 0, num, pattern);
