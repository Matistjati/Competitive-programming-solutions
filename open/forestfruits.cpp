...
int32_t main()
{
    ...
    rep(i, c)
    {
        dread(int, loc);
        loc--;
        if (dp[loc] != inf) locs.emplace_back(dp[loc]);
    }

    sort(all(locs));

    if (k > locs.size() && m > locs.size())
    {
        cout << -1;
    }
    else
    {
        int ans = 0;
        rep(i, min(m, k)) ans = max(ans, locs[i]);
        cout << ans * 2;
    }
