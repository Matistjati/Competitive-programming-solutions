...
inline p2 apply(char c, p2 a, p2 b)
{
	if (c == '+') return a + b;
	return p2(a.first * b.first, a.second * b.second);
}

p2 comp(int l, int r, vector<char>& op, vi& nums, vvp2& dp)
{
	...
	repp(i, l, r)
	{
		p2 res = apply(op[i], comp(l, i, op, nums, dp), comp(i + 1, r, op, nums, dp));
		if (ret.first == inf) ret = res;
...
int32_t main()
{
	...
	while (cin >> line && line != "END")
	{
		...
		vector<string> numss = split(line, '*');
		vi nums;
		repe(num, numss)
		{
			nums.push_back(stoll(num));
		}
		vvp2 dp(op.size() + 1, vp2(op.size() + 1, p2(inf, inf)));
		assert(op.size() + 1 == nums.size());
		p2 ans = comp(0, op.size(), op, nums, dp);
