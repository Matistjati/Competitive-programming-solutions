...
struct network_simplex {
    ...
    void run() {
        ...
        block_size = max(ll(ceil(sqrt(E + V))), min(5LL, V + 1LL));
        next_arc = 0;
...
int main()
{
    ...
    sort(all(edges), [](tuple<ll, ll, ll> a, tuple<ll, ll, ll> b)
        {
    ...
    ll big = 1e9;
    auto max_cycle = [big](vector<p2> edges)
        {
            network_simplex flow(16);
            auto in = [](int x) {return x * 2; };
            auto out = [](int x) {return x * 2 + 1; };
            rep(i, 8) flow.addEdge(in(i), out(i), 1, -big);
            rep(i, 8) flow.addEdge(in(i), out(i), 10000, 0);
            for (auto [a, b] : edges)
            {
                flow.addEdge(out(a), in(b), 1, -1);
            }
            flow.mincost_circulation();
            return -flow.get_circulation_cost();
        };

    ll big8 = big * 8;

    auto isgood = [&](int lb, int ub)
        {
            vector<p2> edg;
            repp(i, lb, ub + 1) edg.emplace_back(get<0>(edges[i]), get<1>(edges[i]));
            ll c = max_cycle(edg);
            if (c < big * 8 || (c - big8) < k) return false;
            return true;
        };
    ...
    ll ans = *max_element(all(w))-*min_element(all(w));

    int ub = 0;
    rep(lb, m)
    {
        while (ub < m && !isgood(lb,ub)) ub++;
        if (ub<m) ans = min(ans, w[ub] - w[lb]);
    }
