...
int32_t main()
{
    ...
    dread(int, n1, n2, n3, n4);

    vi quant = { n1,n2,n3,n4 };
    vi value = { 1,5,10,25 };
    int t = 0;
    rep(i, 4) t += quant[i] * value[i];
    if (p > t)
    {
        cout << "Impossible";
        quit;
    ...
    int taken = 0;
    int cutoff = 1e4;
    while (p >= cutoff && ind < 4)
    {
        int tryamount = max(0LL, quant[ind] - 25);
        if (p - tryamount * value[ind] >= cutoff)
        {
            p -= tryamount * value[ind];
            taken += tryamount;
            quant[ind] -= tryamount;
        }
        else
        {
            int x = (p - cutoff) / value[ind];
            p -= value[ind] * x;
            quant[ind] -= x;
            taken += x;
        }
    ...
    cerr << "greedily taken:" << taken << "\n";
    vi dp(p + 1, -inf);
    dp[0] = 0;

    auto decompose = [](int& x, int v, vp2& res)
    {
        rep(j, 32)
        {
            if (x - (1LL << j) >= 0)
            {
                x -= 1LL << j;
                res.emplace_back(v * (1LL << j), 1LL << j); // cost, profit
            }
            else break;
        }
    };
    ...
    rep(i, 4)
    {
        ...
        while (quant[i])
        {
            decompose(quant[i], value[i], items);
        }
        ...
        repe(item, items)
        {
            per(j, dp.size())
            {
                //debif(j == 116 && item.first == 16 * 5);
                if (j - item.first < 0) continue;
