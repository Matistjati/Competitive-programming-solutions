...
typedef vector<vector<p2>> matrix;
...
matrix multiply(matrix& a, matrix& b)
{
	...
	matrix ret = matrix(a.size(), vector<p2>(a[0].size()));
	...
	rep(i, ret.size())
	{
		rep(j, ret[0].size())
		{
			...
			rep(k, b[0].size())
			{
				p2 p = p2((a[i][k].first * b[k][j].first) % mod, (a[i][k].second * b[k][j].second) % mod);
				v = p2((v.first * p.second + v.second * p.first) % mod, (v.second * p.second) % mod);
			}
...
int32_t main()
{
	...
	rep(i, n)
	{
		...
		if (t == 0)
		{
			rep(j, n) p.push_back(p2(0, 1));
			p.back() = { 1,1 };
		}
		else
		{
			rep(j, n) p.push_back({ nums[j] % mod, t % mod });
		}

		adj.push_back(p);
	}

	vector<vector<p2>> pos;
	pos.push_back({});
	rep(i, n)
	{
		pos[0].push_back({ 0,1 });
	...
	while (k)
	{
		if (k % 2 == 1)
		{
			pos = multiply(pos, adj2);
		}
		k >>= 1;
		adj2 = multiply(adj2, adj2);
	}

	rep(i, 10)
	{
		if ((20 * pos.back().back().first) % mod == (19 * pos.back().back().second) % mod)
		{
			cout << l + i << "\n";
			quit;
		}
		pos = multiply(pos, adj);
	}
	cout << -1;
