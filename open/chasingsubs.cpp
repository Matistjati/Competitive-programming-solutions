...
int32_t main()
{
    ...
    vull needlehash(alphabetsize);

    const ull base = 153;
    vvi needlepositions(alphabetsize);

    ull needlem = 1;
    per(i, needle.size())
    {
        ull v = needle[i] - 'a';
        needlepositions[v].push_back(i);
        repe(v, needlehash) v *= base;
        needlehash[v] += 1;
        needlem *= 153;
    }

    vull stackhash(alphabetsize);


    ull stackm = 1;
    if (stack.size() < needle.size())
    {
        cout << "0";
        quit;
    }
    queue<int> topop;
    for (int i = stack.size() - 1; i >= sz(stack) - sz(needle); i--)
    {
        int v = stack[i] - 'a';
        topop.push(v);
        repe(v, stackhash) v *= base;
        stackhash[v] += 1;
        if (i != stack.size() - needle.size()) stackm *= 153;
    }
    vi ans;
    per(i, stack.size() - needle.size() + 1)
    {
        bool works = 1;
        rep(p, needlepositions.size())
        {
            if (needlepositions[p].empty()) continue;
            ull a = stackhash[stack[i + needlepositions[p].back()] - 'a'];
            ull b = needlehash[needle[needlepositions[p].back()] - 'a'];
            works &= a == b;
            if (!works) break;
        }

        if (works) ans.push_back(i);
        stackhash[topop.front()] -= stackm;
        topop.pop();
    ...
    if (ans.size() == 1)
    {
        repp(i, ans.back(), ans.back() + needle.size())
        {
            cout << stack[i];
        }
    }
