...
signed main()
{
	...
	rep(i, n) rep(j,m) flow.addEdge(numnodes - 2, i+j*n, (grid[i][j]=='.'?b:0));
	rep(i, n) rep(j,m) flow.addEdge(i + j * n, numnodes - 1, (grid[i][j] == '#' ? b : 0));
	...
	rep(i, n)
	{
		rep(j, m)
		{
			repe(dir, dirs)
			{
				...
				flow.addEdge(i + j * n, np.first + np.second * n, a);
			}
		}
	}
