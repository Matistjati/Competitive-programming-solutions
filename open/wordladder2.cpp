...
int32_t main()
{
    ...
    dread(int, n);
    readvector(string, dict, n);
    unordered_map<string, int> wordset;
    rep(i, n) wordset[dict[i]] = i;

    vvi edges(n);
    vector<vector<pair<int, string>>> edges2(n);
    ...
    rep(i, n)
    {
        rep(j, dict[i].size())
        {
            ...
            {
                dict[i][j] = c;
                if (setcontains(wordset, dict[i]))
                {
                    edges[i].push_back(wordset[dict[i]]);
                }
            }
            dict[i][j] = bef;
        }
                ...
                rep(k, dict[i].size())
                {
                    ...
                    repp(c2, 'A', 'Z' + 1)
                    {
                        ...
                        if (setcontains(wordset, dict[i]))
                        {
                            string c(dict[i]);
                            c[j] = bef;
                            edges2[i].emplace_back(wordset[dict[i]], c);
                        }
                    }
                    dict[i][k] = bef2;
                }
    ...
    typedef tuple<int, string, int> state;
    ...
    while (q.size())
    {
        ...
        if (vis[u][edge.size() > 0]) continue;
        vis[u][edge.size() > 0] = 1;

        if (u == 1)
        {
            if (edge.size() && ans.second == inf) ans.second = d + 1, bestedge = edge;
            if (edge.empty() && ans.first == inf) ans.first = d;
        ...
        if (edge.empty())
        {
            repe(e, edges2[u])
            {
                q.emplace(d + 1, e.second, e.first);
            }
        }
    }
