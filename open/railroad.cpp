        ...
        rep(i, n)
        {
            if (edges[i].size() == 2)
            {
                ...
                while (q.size())
                {
                    ...
                    repe(e, edges[u])
                    {
                        if (!blacklist[e.second]) totweight += get<2>(edgelist[e.second]);
                        blacklist[e.second] = 1;
                        q.emplace(e.first);
                    }
                }

                assert(endpoints.size() == 2);
                newedges.emplace_back(endpoints[0], endpoints[1], totweight);
            }
        }

        rep(i, m)
        {
            if (!blacklist[i]) newedges.emplace_back(edgelist[i]);
        }
