...
int32_t main()
{
    ...
    rep(i, q)
    {
        ...
        if (s.first == e.first) repp(y, s.second, e.second+1) active[y][s.first]++;
        else repp(x, s.first, e.first+1) active[s.second][x]++;
    }

    int white = 0;
    rep(i, m) rep(j, n) white += active[i][j] == 0;
    UF uf(n* m);
    ...
    rep(i, m) rep(j, n)
    {
        ...
        repe(dir, dirs)
        {
            p2 newPos = p2(i, j) + dir;
            if (newPos.first < 0 || newPos.first >= m || newPos.second < 0 || newPos.second >= n) continue;
            if (active[newPos.first][newPos.second] > 0) continue;
            white-=uf.merge(i*n+j, newPos.first*n+newPos.second);
        }
    }
    ...
    perr(i, 1, q)
    {
        ...
        if (s.first == e.first)
        {
            repp(y, s.second, e.second + 1)
            {
                if (active[y][s.first]==0)
                {
                    white++;
                    repe(dir, dirs)
                    {
                        p2 newPos = p2(y, s.first) + dir;
                        if (newPos.first < 0 || newPos.first >= m || newPos.second < 0 || newPos.second >= n) continue;
                        if (active[newPos.first][newPos.second] > 0) continue;
                        white -= uf.merge(y * n + s.first, newPos.first * n + newPos.second);
                    }
                }
            }
        }
        ...
        {
            repp(x, s.first, e.first + 1)
            {
                if (active[s.second][x] == 0)
                {
                    white++;
                    repe(dir, dirs)
                    {
                        p2 newPos = p2(s.second, x) + dir;
                        if (newPos.first < 0 || newPos.first >= m || newPos.second < 0 || newPos.second >= n) continue;
                        if (active[newPos.first][newPos.second] > 0) continue;
                        white -= uf.merge(s.second * n + x, newPos.first * n + newPos.second);
                    }
                }
            }
        }
