...
int32_t main()
{
    ...
    dread(int, a, b, CC);
    vp2 stuff;
    stuff.emplace_back(a, 'A');
    stuff.emplace_back(b, 'B');
    stuff.emplace_back(CC, 'C');
    sort(rall(stuff));

    if (stuff[0].first > r * c / 2)
    {
        cout << "impossible";
        quit;
    }
    ...
    rep(i, r)
    {
        ...
        {
            if (stuff[0].first == 0) break;
            ans[i][j] = stuff[0].second;
            stuff[0].first--;
        }
    }
    ...
    rep(i, r)
    {
        ...
        {
            if (ans[i][j] != '.') continue;
            if (stuff[1].first == 0) break;
            ans[i][j] = stuff[1].second;
            stuff[1].first--;
        }
    }
    ...
    rep(i, r)
    {
        ...
        {
            if (ans[i][j] != '.') continue;
            if (stuff[1].first == 0) continue;
            bool can = 1;
            repe(dir, dirs)
            {
                p2 np = dir + p2(i, j);
                if (np.first < 0 || np.second < 0 || np.first >= r || np.second >= c) continue;
                can &= ans[np.first][np.second] != stuff[1].second;
            }
            if (!can) continue;
            ans[i][j] = stuff[1].second;
            stuff[1].first--;
    ...
    rep(i, r)
    {
        ...
        {
            if (ans[i][j] != '.') continue;
            if (stuff[2].first == 0) continue;
            ans[i][j] = stuff[2].second;
            stuff[2].first--;
        }
    }
