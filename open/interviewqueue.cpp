...
int32_t main()
{
    ...
    dread(int, n);
    readvector(int, numsin, n);

    set<p2> nums;
    rep(i, n) nums.emplace(i, numsin[i]);
    ...
    {
        bool happy = true;
        if (i > 0) if (numsin[i - 1] > numsin[i]) happy = false;
        if (i < n - 1) if (numsin[i + 1] > numsin[i]) happy = false;
        if (!happy) remindex.emplace_back(i);
    }
    ...
    while (remindex.size())
    {
        ...
        rep(i, remindex.size())
        {
            ans.back().push_back(numsin[remindex[i]]);
            nums.erase(p2(remindex[i], numsin[remindex[i]]));
        }

        rep(i, remindex.size())
        {
            auto it = nums.lower_bound(p2(remindex[i], inf));
            if (it == nums.end()) continue;
            if (it == nums.begin()) continue;

            if (it->second < prev(it)->second)
            {
                newremindex.insert(it->first);
            }
            else if (it->second > prev(it)->second)
            {
                newremindex.insert(prev(it)->first);
            }
        }

        remindex = vi(all(newremindex));
    }
