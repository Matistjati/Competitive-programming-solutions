...
struct Node
{
    ...
    int cheapest(string& s, int i)
    {
        if (i == s.size()) return s.size();
        int ret = (numterminal == 1 ? i + 1 : inf);
        return min(ret, children[s[i]]->cheapest(s, i + 1));
    }
};
...
int32_t main()
{
    ...
    dread(int, n);
    readvector(string, names, n);

    Node* root = new Node();
    int ansslow = 0;
    repe(word, names) ansslow += word.size();
    set<string> uniquewords(all(names));
    repe(word, uniquewords)
    {
        root->insert(word, 0);
    }
    ...
    repe(word, names)
    {
        ansfast += root->cheapest(word, 0);
    }
    cout << ansslow - ansfast;
