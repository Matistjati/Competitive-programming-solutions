...
struct UF
{
    ...
    UF(int n, vi& size) : par(n), size(n, 1), value(size), ans(n), alive(n), edges(n)
    {
        rep(i, n) par[i] = i;
        rep(i, n) alive[i].push_back(i);
    }
    ...
    void add(int u)
    {
        ...
        if (!edges[u].size()) return;
        int totval = value[u];
        int largest = find(edges[u][0]);
        vi components = { u };
        repe(edge, edges[u])
        {

            int e = find(edge);
            if (visneighbours[e] == ver) continue;
            visneighbours[e] = ver;
            ...
            if (value[e] < value[u])
            {
                repe(v, alive[e]) ans[v] += value[e];
                alive[e] = vi();
            }
            size[e] = alive[e].size();
            if (size[e] >= size[largest]) largest = e;
        }


        repe(comp, components)
        {
            if (comp == largest) continue;
            size[largest] += alive[comp].size();
            repe(u, alive[comp]) alive[largest].push_back(u);
            par[comp] = largest;
        }
        value[largest] = totval;
...
int32_t main()
{
    ...
    vp2 nodes;
    rep(i, n) nodes.emplace_back(sizes[i], i);
    sort(all(nodes));
    ...
    rep(i, n)
    {
        ...
        repe(e, edges[u])
        {
            if (added[e]) uf.edges[u].emplace_back(e);
        }
        added[u] = 1;
        uf.add(nodes[i].second);
    }
    ...
    {
        int u = uf.find(i);
        if (visneighbours[u] == ver) continue;
        visneighbours[u] = ver;

        repe(v, uf.alive[u]) uf.ans[v] += uf.value[u];
    }

    rep(i, n) cout << uf.ans[i] << " ";
