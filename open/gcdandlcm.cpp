...
ull gcd(multiset<ull> a, vector<ull>& b)
{
	multiset<ull> res;
	repe(f, b)
	{
...
int32_t main()
{
	...
	vector<ull> factors;
	repe(f, factor(x)) factors.push_back(f);
	repe(f, factor(y)) factors.push_back(f);
	multiset<ull> factorSet(all(factors));
	vector<ull> divisors = getDivisors(factors);
	set<pair<ull,ull>> ans;
	repe(div, divisors)
	{
		multiset<ull> factorK(factorSet);

		vector<ull> divFactors = factor(div);
		repe(f, divFactors) eraseOne(factorK, f);

		ull k = product(factorK);
		if (gcd(factorK, divFactors) == x)
		{
			ans.emplace(product(factorK), div);
		}
