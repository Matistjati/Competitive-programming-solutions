...
int32_t main()
{
    ...
    vi ones;
    rep(i, towers.size()) if (towers[i] == '1') ones.push_back(i);

    int res = 0;
    rep(i, towers.size())
    {
        auto nextTower = lower_bound(all(ones), i);
        int ans = inf;

        ans = min(ans, *nextTower-i);
        if (nextTower != begin(ones)) ans = min(ans, i-*prev(nextTower));
        res = max(res, ans);
    }
    cout << res;
