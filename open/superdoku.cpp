...
bool bpm(int u, vb& vis, vi& match, vvb& edges)
{
    ...
    rep(i, edges.size())
    {
        if (edges[u][i])
        {
            if (match[i] == -1 || bpm(match[i], vis, match, edges))
            {
                match[i] = u;
                return true;
            }
...
int32_t main()
{
    ...
    vvi square(n, vi(n));
    rep(i, k) rep(j, n) read(square[i][j]);
    rep(i, n) rep(j, n) square[i][j]--;

    rep(i, k)
    {
        if (set<int>(all(square[i])).size() != n)
        {
            cout << "no";
            quit;
        }
