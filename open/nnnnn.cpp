...
string decode(string v)
{
    ...
    int lg10floor = v.size() - 1;

    int k;
    repp(d, 1, 1e6)
    {
        int lg10floord = d - 1 + log10(d);
        if (lg10floord >= lg10floor)
        {
            string addend = "";
            addend += '1';
            rep(i, d - 1) addend += '0';
            string res = multiply(to_string(d), addend);
            if (res > v)
            {
                k = d - 1;
            }
    ...
    string res = v;
    string subbend = addend;
    subbend = multiply(subbend, to_string(k));
    res = findDiff(res, subbend);
    string tmp;
    bool nonzero = false;
    rep(i, res.size())
    {
        if (res[i] != '0') nonzero = true;
        if (nonzero) tmp += res[i];
    }
    res = tmp;
    res = longDivision(res, k);
    res = findSum(res, addend);
...
string encode(string v)
{
    ...
    return multiply(v, to_string(lg10 + 1));
}
