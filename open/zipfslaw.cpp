...
int32_t main()
{
    ...
    while (true)
    {
        ...
        string w;
        while (getline(cin,w)&&(w!="EndOfText"))
        {

            rep(i, w.size())
            {
                string k;
                while (i<w.size()&&(w[i]>='a'&&w[i]<='z')||w[i]>='A'&&w[i]<='Z')
                {
                    k += tolower(w[i]);
                    i++;
                }
        ...
        repe(occ, occs)
        {
            if (occ.second==n)
            {
                ans.insert(words[occ.first]);
            }
        }
        if (ans.size() == 0) cout << "There is no such word.\n";
        else repe(w, ans) cout << w << "\n";
    }
