...
deque<int>* dfs(int u, int p, vvi& edges)
{

    deque<int>* smol, * large;
    smol = large = nullptr;
    repe(e, edges[u])
    {
        if (e == p) continue;
        deque<int>* other = dfs(e, u, edges);
        other->push_front(0);
        if (smol == nullptr) smol = other;
        else large = other;
    }

    if (!smol && !large)
    {
        deque<int>* ret = new deque<int>({ 1 });
        return ret;
    }
    if (!smol)
    {
        smol = new deque<int>({ 0 });
    }
    if (!large)
    {
        large = new deque<int>({ 0 });

    }

    if (smol->size() > large->size()) swap(smol, large);

    int shallowest = min(smol->size(), large->size());
    while (smol->size() > shallowest + 1) smol->pop_back();
    while (large->size() > shallowest + 1) large->pop_back();
...
int32_t main()
{
    ...
    deque<int> ans = *dfs(0, 0, edges);
    cout << n - accumulate(all(ans), 0);
