...
int32_t main()
{
    ...
    vp2 countries(n);
    rep(i, n) cin >> countries[i].first >> countries[i].second;
    ...
    while (pq.size())
    {
        ...
        if (u == n - 1)
        {
            cout << fixed << setprecision(15) << exp(p);
            quit;
        }
        ...
        repe(e, edges[u])
        {
            ...
            {
                double prob = (t - reald) / double(t - s);
                pq.emplace(p + log(prob), e, d + 1);
            }
        }
    }
    cout << 0;
