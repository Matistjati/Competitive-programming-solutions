...
std::vector<T> MaxConvolutionWithConvexShape(std::vector<T> anyShape, const std::vector<T>& convexShape) {
    ...
    const std::vector<int> best = smawck(comparator, n, n);
    std::vector<T> ans(n);
    for (int i = 0; i < n; i++) {
...
int32_t main()
{
    ...
    {
        dread2(int, s, v);
        profits[s].push_back(v);
    }

    vi ans(c + 1, -inf);
    ans[0] = 0;

    repp(i, 1, profits.size())
    {
        ...
        repp(j, 1, p.size()) p[j] += p[j - 1];
        p.insert(p.begin(), 0);

        rep(res, i)
        {
            vi conv;
            for (int k = res; k < ans.size(); k += i) conv.push_back(ans[k]);
            conv = MaxConvolutionWithConvexShape(conv, p);

            for (int k = res, j = 0; k < ans.size(); k += i, j++) ans[k] = conv[j];
        }
    }
