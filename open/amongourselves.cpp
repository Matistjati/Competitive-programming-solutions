...
struct UF
{
	...
	void undo()
	{
		...
		changes.pop();
		//assert(change.a == find(change.a));
		//assert(change.b == find(change.b));
...
void dc(int l, int r, int fr)
{
	if (l == r)
	{
		ans[l] = uf.find(0) == uf.find(target);
	}
	else
	{
		int mid = (l + r) / 2;
		repp(i, mid + missed, fr) uf.merge(edgelist[i].first, edgelist[i].second);
		dc(l, mid, min(sz(edgelist), mid + missed));
		repp(i, mid + missed, fr) uf.undo();
		repp(i, l, mid + 1) uf.merge(edgelist[i].first, edgelist[i].second);
		dc(mid + 1, r, fr);
		repp(i, l, mid + 1) uf.undo();
	}
}

void dfs(int u, vi& col, vb& vis, vvp2& edges)
{
	if (vis[u]) return;
	vis[u] = 1;
	assert(col[u]);
	...
	rep(i, m - missed + 1)
	{
		if (ans[i])
		{
			...
			rep(j, i)
			{
				int a, b;
				tie(a, b) = edgelist[j];
				edges[a].emplace_back(b, accs[j]);
				edges[b].emplace_back(a, accs[j]);
			}
			repp(j, i + missed, m)
			{
				int a, b;
			...
			vb vis(n);
			dfs(0, col, vis, edges);
			cout << " " << (col[target] == 1 ? "crewmate" : "imposter");
			quit;
		}
	}
	cout << "-1";
