...
int32_t main()
{
    ...
    repe(t, times)
    {
        while (starting.size() && -get<0>(starting.top()) <= t)
        {
            int a, b, i;
            tie(a, b, i) = starting.top();
            starting.pop();

            int v = 0;
            auto it = covers.lower_bound(b);
            if (it != covers.end()) v = it->second + 1;
            ans[i] = v;
            covers[b] = max(covers[b], v);
            it = covers.find(b);
            while (it != begin(covers) && prev(it)->second < v) covers.erase(prev(it));
        }
    }
    rep(i, n)
    {
