...
int32_t main()
{
    ...
    while ((cin >> n) && n)
    {
        ...
        rep(i, n)
        {
            repp(j, i + 1, n)
            {
                double slope = (points[i].second - points[j].second) / nonzero(points[i].first - points[j].first);
                lines[slope]++;
            }
            repe(slope, lines) ans = max(ans, slope.second);
            lines = map<double, int>();
        }
