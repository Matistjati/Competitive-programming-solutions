...
void mark_alive(int u, vector<map<int, int>>& edges, vi& indeg, vb& isalive)
{
    if (isalive[u]) return;
    isalive[u] = 1;
    ...
    repe(e, edges[u])
    {
        indeg[e.first] += e.second;
        mark_alive(e.first, edges, indeg, isalive);
    }
}
...
int32_t main()
{
    ...
    dread2(int, n, m);
    vp2 initialedges(m);
    rep(i, m) read(initialedges[i].first), read(initialedges[i].second);
    rep(i, m) initialedges[i] += p2(-1, -1);
    ...
    rep(i, m)
    {
        ...
        {
            int a, b;
            tie(a, b) = initialedges[i];
            edges[a][b]++;
        }
    }

    mark_alive(0, edges, indeg, isalive);
    vi answers;
    reverse(all(queries));
    rep(i, q)
    {
        ...
        int a, b;
        tie(a, b) = initialedges[v];
        if (t == 1)
        {
            if (isalive[a])
            {
                indeg[b]++;
                mark_alive(b, edges, indeg, isalive);
            }
            else
            {
                edges[a][b]++;
        ...
        {
            answers.push_back(indeg[v]);
        }
    }
    reverse(all(answers));
    repe(a, answers) cout << a << "\n";
