...
int32_t main()
{
	...
	rep(i, 7)
	{
		dread(int, c);
		c--;
		if (!setcontains(citiesind, c)) citiesind[c] = citiesind.size();
	}


	vvi dist(citiesind.size(), vi(citiesind.size(), inf));
	...
	repe(city, citiesind)
	{
		...
		while (pq.size())
		{
			...
			repe(e, edges[u])
			{
				pq.emplace(dis - e.second, e.first);
			}
		}

		repe(ind, citiesind) dist[city.second][ind.second] = d[ind.first];
	}

	vi order(citiesind.size() - 1);
	...
	do
	{
		int d = dist[0][1 + order[0]];
		rep(i, order.size() - 1) d += dist[1 + order[i]][1 + order[i + 1]];
		ans = min(ans, d);
