    ...
    while (cin >> n && n != -1)
    {
        ...
        rep(i, n)
        {
            repe(e, edgelist)
            {
                ...
                int ne = d[e.first] + energy[e.second];
                if (ne >= 0) continue;
                d[e.second] = min(d[e.second], d[e.first] + energy[e.second]);
            }
        }
        ...
        rep(j, 100)
        {
            ...
            {
                if (d[e.first] >= inf) continue;
                int nc = d[e.first] + energy[e.second];
                if (nc >= 0) continue;
                if (nc < d[e.second])
                {
                    x = e.second;
        ...
        cout << (winnable ? "winnable" : "hopeless") << "\n";
    }
