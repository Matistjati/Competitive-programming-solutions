// Idea: when do we want to take less than optimal? High followed by lots of lows, i.e. 10 1 1 1 1.. 
// Because numbers are small, we should never want to take less than about 10 below the max
...
int simulate(int candy, vi& kids)
{
    int ret = 0;
    repe(k, kids)
    {
...
int32_t main()
{
    ...
    readvector(int, kids, n);

    int ans = inf;
    int lo = max(0LL, m - 4);
    for (int i = m; i >= lo; i--)
    {
        ans = min(ans, simulate(i, kids));
    }
