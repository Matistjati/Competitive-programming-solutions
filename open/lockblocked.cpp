...
void dfs(int u, int p, vvp2& edges)
{
    vis[u] = 1;
    tin[u] = low[u] = timer++;
    ...
    repe(e, edges[u])
    {
        ...
        {
            low[u] = min(low[u], tin[e.first]);
        }
        else
        {
            dfs(e.first, u, edges);
            low[u] = min(low[u], low[e.first]);
            if (low[e.first] > tin[u]) isbridge[e.second] = 1;
        }
    }
}

void mark(int u, vi& vis, vvp2& edges)
{
    if (vis[u]) return;
    vis[u] = 1;
    repe(e, edges[u]) if (!isbridge[e.second]) mark(e.first, vis, edges);
}

int count(int u, vi& vis, vvp2& edges)
{
    if (vis[u]) return 0;
    vis[u] = 1;
    int ret = 1;
    repe(e, edges[u]) ret += count(e.first, vis, edges);
    return ret;
}
