...
int32_t main()
{
    ...
    vvi queues(t + 10);
    queues[0].emplace_back(0);

    for (int d = 0; d < queues.size(); d += queues[d].empty())
    {
        ...
        {
            p2 e = edges[i];
            int newpos = u + e.first;
            if (newpos < 0 || newpos >= dist.size()) continue;
            int newcost = d + e.second;
            if (newcost >= queues.size()) continue;
            if (newcost >= dist[newpos]) continue;

            par[newpos] = i;
            dist[newpos] = newcost;
            queues[newcost].emplace_back(newpos);
        }
    }
