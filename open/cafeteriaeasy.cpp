...
int32_t main()
{
    ...
    rep(i, 5)
    {
        ...
        dread(string, b);
        if (a == "_") plates[i][0] = -1;
        else plates[i][0] = stoi(a);

        if (b == "_") plates[i][1] = -1;
        else plates[i][1] = stoi(b);
    }
    ...
    rep(i, 5)
    {
        ...
        dread(string, b);
        if (a == "_") plates[i][2] = -1;
        else plates[i][2] = stoi(a);

        if (b == "_") plates[i][3] = -1;
        else plates[i][3] = stoi(b);
    }
    ...
    while (true)
    {
        ...
        rep(i, 5)
        {
            repp(j, i + 1, 5)
            {
                ...
                rep(k, 4)
                {
                    if (plates[i][k] == -1 && plates[j][k] != -1) plates[i][k] = plates[j][k] * plates[i][ratio] / plates[j][ratio], any = 1;
                    if (plates[i][k] != -1 && plates[j][k] == -1) plates[j][k] = plates[i][k] * plates[j][ratio] / plates[i][ratio], any = 1;
                }
            }
        }
    ...
    rep(i, 5)
    {
        assert(plates[i][0] != -1 && plates[i][1] != -1);
        cout << plates[i][0] << " " << plates[i][1] << " ";
    }
    cout << "\n";
    rep(i, 5)
    {
        assert(plates[i][2] != -1 && plates[i][3] != -1);
        cout << plates[i][2] << " " << plates[i][3] << " ";
    }
