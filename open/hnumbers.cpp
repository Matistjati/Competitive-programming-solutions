...
int32_t main()
{
    ...
    vi smallest_factor(n_hi);
    rep(i, n_hi) smallest_factor[i] = i;

    for (int p = 5; p*p <= n_hi; p+=4) {
        // If prime[p] is not changed, then it is a prime

        if (smallest_factor[p] == p)
        {

            for (int i = p * p; i <= n_hi; i += p)
                if (smallest_factor[i] == i) smallest_factor[i] = p;
        }
    }
    vi num_primes(n_hi);
    int primes = 0;

    for (int p = 1; p < n_hi; p+=4)
    {
        if (smallest_factor[p] != p)
        {
            int k = p;
