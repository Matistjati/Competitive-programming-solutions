...
int target;
int solve(int seen, int n, int hi, vi& nums, set<int>& numset, map<int, int> indexof, vi& dp)
{
    if (seen == target)
    {
        return 0;
    }
    if (popcount(seen) + 1 == n)
    {
        return 1;
    }
    ...
    rep(i, n)
    {
        ...
        repp(j, i + 1, n)
        {
            ...
            bool works = 1;
            for (int k = 0; nums[i] + k * diff < hi; k++)
            {
                if (setcontains(numset, nums[i] + k * diff))
                {
                    mask |= (1 << (indexof[nums[i] + k * diff]));
                }
                else
                {
                    works = 0;
            ...
            if (!works) continue;
            int newseen = seen | mask;
            if (newseen == seen) continue;
            ret = min(ret, 1 + solve(newseen, n, hi, nums, numset, indexof, dp));
        }
    }
    rep(i, n)
    {
        if ((seen & (1 << i)) == 0) ret = min(ret, 1 + solve(seen | (1 << i), n, hi, nums, numset, indexof, dp));
    }
    return v = ret;
}
...
int32_t main()
{
    ...
    while (cin >> m >> n)
    {
        ...
        sort(all(nums));
        target = (1 << n) - 1;
        set<int> numset(all(nums));
        vi dp(1 << n, -1);
        map<int, int> indexof;
        rep(i, n) indexof[nums[i]] = i;
        cout << solve(0, n, m, nums, numset, indexof, dp) << "\n";
    }
