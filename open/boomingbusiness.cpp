...
int h, w;
int dp[301][301][301];
int cnt(int depth, int placed, int deepest)
{
    if (depth == 0) return 0;
    if (placed == w)
    {
    ...
    int& v = dp[depth][placed][deepest];
    if (v != -1) return v;

    int ret = 0;
    ret = (ret + cnt(depth - 1, placed, max(depth, deepest))) % mod;
    ret = (ret + cnt(depth + 1, placed + 1, max(depth + 1, deepest))) % mod;
