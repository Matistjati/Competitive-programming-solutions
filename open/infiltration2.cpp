...
vi tryone(vector<bitset<75>>& edges)
{
    int n = edges.size();
...
vi trytwo(vector<bitset<75>>& edges)
{
    int n = edges.size();
...
vi trythree(vector<bitset<75>>& edges)
{
    int n = edges.size();

    rep(i, n) repp(j, i + 1, n) repp(k, j + 1, n)
        if ((edges[i] | edges[j] | edges[k]).count() == n)
        {
            return { i,j,k };
        }
...
vi tryfour(vector<bitset<75>>& edges)
{
    int n = edges.size();

    rep(i, n) repp(j, i + 1, n) repp(k, j + 1, n) repp(l, k + 1, n)
        if ((edges[i] | edges[j] | edges[k] | edges[l]).count() == n)
        {
            return { i,j,k,l };
        }
...
vi tryfive(vector<bitset<75>>& edges)
{
    int n = edges.size();

    rep(i, n) repp(j, i + 1, n) repp(k, j + 1, n) repp(l, k + 1, n) repp(m, l + 1, n)
        if ((edges[i] | edges[j] | edges[k] | edges[l] | edges[m]).count() == n)
        {
            return { i,j,k,l,m };
        }
...
int32_t main()
{
    ...
    while (cin >> n)
    {
        vector<bitset<75>> edges(n);
        rep(i, n)
        {
            dread(string, adj);
            rep(j, n) if (adj[j] == '1') edges[i][j] = 1;
            edges[i][i] = 1;
        }
