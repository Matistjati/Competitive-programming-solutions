...
double best(int u, int vis, vector<point>& pts, vector<pair<int, vector<pair<int, double>>>>& coins, vector<vector<double>>& dp)
{
	if (popcount(vis) == coins.size())
	{
		return 0;
	}
	...
	{
		if (vis & (1 << i)) continue;
		double nd = dist(currpos, pts[coins[i].first]);
		repe(coin, coins[i].second)
		{
			ret = min(ret, coin.second + nd + best(coin.first, vis | (1 << i), pts, coins, dp));
		}
	}
	return v = ret;
}
...
int32_t main()
{
	...
	dread4(int, n, mx, my, mz);

	vector<point> pts = { {mx,my,mz} };

	vector<pair<int, vector<pair<int, double>>>> coins;
	...
	rep(i, n)
	{
		...
		{
			dread3(int, x, y, z);
			scoins.push_back({ x,y,z });
			ind.push_back(pts.size());
			pts.push_back({ x,y,z });
		}
		scoins.push_back({ sx,sy,sz });

		priority_queue<tuple<double, int, int>> pq;
		pq.emplace(0, scoins.size() - 1, 0);
		vector<double> best(k + 1);
		vvi vis(k + 1, vi(1 << (k + 1)));
		while (pq.size())
		{
			double d;
			int u, b;
			...
			{
				if (b & (1 << i)) continue;
				pq.emplace(d - dist(scoins[u], scoins[i]), i, b);
			}
		}

		vector<pair<int, double>> coinv;
		rep(i, k)
		{
			coinv.emplace_back(ind[i], best[i]);
		}
		pair<int, vector<pair<int, double>>> t;
		t.first = myind;
		t.second = coinv;
		coins.emplace_back(t);
	}

	vector<vector<double>> dp(pts.size(), vector<double>(1 << n, -inf));
	cout << setprecision(10) << best(0, 0, pts, coins, dp);
