...
int32_t main()
{
    ...
    rep(i, n)
    {
        while (j<q && get<0>(items[i])>get<0>(queries[j]))
        {
            int _, k, ind;
            tie(_, k, ind) = queries[j];
            auto it = begin(available);
            while (it != end(available) && k > 0)
            {
                answers[ind].push_back(it->second);
                k--;
                it = next(it);
            }
        ...
        available.insert(p2(-get<1>(items[i]), get<2>(items[i])));
    }
