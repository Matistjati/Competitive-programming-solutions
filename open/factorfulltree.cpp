...
void dfs(int u, int p, vvi& edges, int cum, int c, vi& cpath)
{
    if (color[u] == -1 && color[p] != -1) cum = c * color[p];
    else cum *= c;
    cpath.push_back(u);

    if (cum > best && color[u] == -1)
    {
        best = cum;
        path = cpath;
    }

    repe(e, edges[u]) if (e != p) dfs(e, u, edges, cum, c, cpath);

    cpath.pop_back();
}

const int MAX_PR = 1000;
...
int32_t main()
{
    ...
    while (true)
    {
        ...
        path = vi();
        vi cpath;
        dfs(0, 0, edges, 1, primes[i], cpath);
        ...
        repp(j, 1, path.size())
        {
            if (color[path[j]] == -1)color[path[j]] = color[path[j - 1]] * primes[i];
        }
        i++;
    }

    rep(i, n) cout << color[i] << " ";
    cerr << "\n\n" << *max_element(all(color));
    quit;
}