...
struct Node
{
    ...
    Node(Node* parent) : par(parent), tab(nullptr) {}

    Node* insert(string& s, int i, bool shouldTab=true)
    {
        if (i == s.size())
        {
            if (this->tab==nullptr) this->tab = this;
            return this;
        }
        if (!setcontains(children, s[i]))
        {
            children[s[i]] = new Node(this);
            //children[s[i]]->s = this->s+s[i];
        }

        Node* r = children[s[i]]->insert(s, i + 1, shouldTab);

        if (tab == nullptr && shouldTab) tab = r;
...
int32_t main()
{
    ...
    {
        dread(string, s);
        nodes.push_back(root->insert(s, 0, 0));
    }
    ...
    while (q.size())
    {
        ...
        if (setcontains(vis, u)) continue;
        vis.insert(u);
        u->d = d;

        repe(c, u->children)
        {
            q.emplace(c.second, d + 1);
        }
        if (u->par != nullptr) q.emplace(u->par, d + 1);
        if (u->tab != nullptr) q.emplace(u->tab, d + 1);
    }
