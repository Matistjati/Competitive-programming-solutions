...
double eval(vp2& lines, double time)
{
    double mi = inf;
    double ma = -inf;
    repe(line, lines)
    {
        double v = line.first * (time - line.second);
        mi = min(mi, v);
        ma = max(ma, v);
    }
    return abs(mi - ma);
...
int32_t main()
{
    ...
    rep(i, 120)
    {
        double a = lo + (hi - lo) / 3;
        double b = hi - (hi - lo) / 3;
        if (eval(lines, a) < eval(lines, b))
        {
            hi = b;
        }
        else lo = a;
    }
    cout << fixed << setprecision(15) << eval(lines, lo);
