...
int32_t main()
{
    ...
    dread(int, k);
    readvector(int, nums, k);

    vi prefix(k);
    prefix[0] = nums[0];
    repp(i, 1, k) prefix[i] = prefix[i - 1] ^ nums[i];

    int v = 0;
    rep(j, k) v ^= nums[j];
    vi nums2(nums);
    nums2.insert(nums2.begin(), v);


    vi prefix2(k + 1);
    prefix2[0] = nums2[0];
    repp(i, 1, k + 1) prefix2[i] = prefix2[i - 1] ^ nums2[i];


    int xorall = 0;
    rep(i, k + 1) xorall ^= nums2[i];
    ...
    while (q--)
    {
        ...
        auto comp = [&](int i)
        {
            ...
            else
            {
                int v = 0;
                i -= k;
                if ((i / (k + 1)) % 2 == 1) v ^= xorall;
                return v ^ prefix2[i % (k + 1)] ^ prefix.back();
            }
        };
