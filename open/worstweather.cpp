...
struct Node
{
	...
	Node(int l, int r) : l(l), r(r), v(-inf), s(0), lc(nullptr), rc(nullptr) {}
	...
	int query(int ql, int qr)
	{
		...
		int ret = -inf;
		if (lc) ret = max(ret, lc->query(ql, qr));
		if (rc) ret = max(ret, rc->query(ql, qr));
		return ret;
	}
	...
	int querysum(int ql, int qr)
	{
		...
		int ret = 0;
		if (lc) ret += lc->querysum(ql, qr);
		if (rc) ret += rc->querysum(ql, qr);
		return ret;
	}
};
...
int32_t main()
{
	...
	while (cin >> n)
	{
		...
		rep(i, m)
		{
			...
			int av = root->query(a, a);
			int bv = root->query(b, b);
			bool knownbetween = (a + 1 == b) || root->query(a + 1, b - 1) != -inf;

			if (av == -inf && bv == -inf)
			{
				cout << "maybe";
			}
			else if (av == -inf && bv != -inf)
			{
				if (root->query(a + 1, b - 1) < bv)
				{
					cout << "maybe";
				}
				else
			...
			{
				if (av >= bv && root->query(a + 1, b - 1) < bv)
				{
					if (!(a + 1 == b) && root->querysum(a + 1, b - 1) < (b - 1 - (a + 1) + 1))
					{
						cout << "maybe";
					}
					else
