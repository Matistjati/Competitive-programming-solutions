...
int32_t main()
{
    ...
    dread3(int, n, e, d);
    readvector(int, exploding, e);
    readvector(int, defuse, d);
    sort(all(exploding));
    sort(all(defuse));

    vp2 numdefuse(n);
    rep(i, n) numdefuse[i].second = i;

    int i = 0;
    int j = 0;
    int t = 0;
    int player = 0;
    while ((i < e || j < d) && numdefuse.size() > 1)
    {
        int a = inf;
        if (i < e) a = exploding[i];
        int b = inf;
        if (j < d) b = defuse[j];

        if (a < b)
        {
            player = (player + a - t) % numdefuse.size();
            if (numdefuse[player].first == 0)
            {
                numdefuse.erase(numdefuse.begin() + player);
                player--;
            }
            else
            {
                numdefuse[player].first--;
            }
            t = exploding[i];
            i++;
        }
        ...
        {
            player = (player + b - t) % numdefuse.size();
            numdefuse[player].first++;
            numdefuse[player].first = min(5LL, numdefuse[player].first);
            t = defuse[j];
            j++;
        }
    }
