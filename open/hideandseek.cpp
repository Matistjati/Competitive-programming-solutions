...
unsigned int clz(unsigned int x) { return _lzcnt_u32(x); }
unsigned int ctz(unsigned int x) { return _tzcnt_u32(x); }
#define bswap64(x) _byteswap_uint64(x)
#define assert(x) debassert(x)
#else
#define popcount(x) __builtin_popcount(x)
unsigned int clz(unsigned int x) { return __builtin_clz(x); }
unsigned int ctz(unsigned int x) { return __builtin_ctzll(x); }
...
vi timein;
vi timeout;
void dfs(int u, int p, int& time, vvp2& edges)
{
    timein[u] = time++;

    repe(e, edges[u]) if (e.first != p) dfs(e.first, u, time, edges);
...
vi ans(int u, int p, vi dp, int target, vvp2& edges)
{
    vi ret(dp);
    ...
    repe(e, edges[u])
    {
        ...
        int cost = is_ancestor(e.first, target) ? e.second : e.second * 2;

        vi newret = ans(e.first, u, ret, target, edges);

        vi newdp(dp.size());
        rep(i, dp.size())
        {
            newdp[i] = max(ret[i], newdp[i]);

            if (cost + i < dp.size())
            {
                newdp[i + cost] = max(newdp[i + cost], newret[i] + 1);
            }
...
int32_t main()
{
    ...
    {
        vi dp(n + 1);
        dp = ans(0, -1, dp, i, edges);
        m_caves = max(m_caves, *max_element(all(dp)));
    }
