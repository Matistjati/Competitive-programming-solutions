...
void topsort(int u, vi& vis, vi& sort, vvi& edges)
{
    if (vis[u]) return;
    vis[u] = 1;

    repe(e, edges[u]) topsort(e, vis, sort, edges);

    sort.push_back(u);
}

int compsize(int u, vi& vis, vvi& redges)
{
    if (vis[u]) return 0;
    vis[u] = 1;
    int ret = 1;

    repe(e, redges[u]) ret += compsize(e, vis, redges);
...
int32_t main()
{
    ...
    repe(i, sort)
    {
        int compsz = compsize(i, vis, redges);
        if (compsz > 1)
        {
            cout << "IMPOSSIBLE";
            quit;
