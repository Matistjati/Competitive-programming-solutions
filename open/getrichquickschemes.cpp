...
struct MCMF {
    ...
    MCMF(int N) :
        N(N), ed(N), red(N), cap(N, vi(N)), flow(cap), cost(cap),
    ...
    pair<ll, ll> maxflow(int s, int t) {
        ...
        repp(i, 0, N) repp(j, 0, N) totcost += cost[i][j] * flow[i][j];
...
int32_t main()
{
    ...
    dread(int, n);
    vp2 categories(n);
    rep(i, n) read(categories[i].first), read(categories[i].second);
    dread(int, s);
    int sz = n + s + 5;
    MCMF flow(sz);
    int src = sz - 1;
    ...
    rep(i, n)
    {
        flow.addEdge(src, i, categories[i].second, -categories[i].first);
    }
    ...
    rep(i, s)
    {
        ...
        rep(j, a)
        {
            dread(int, e);
            e--;
            flow.addEdge(e, i + n, inf, 0);
        }
        flow.addEdge(i + n, sink, l, 0);
    }



    p2 res = flow.maxflow(src, sink);
    cout << (-res.second) / 100;
