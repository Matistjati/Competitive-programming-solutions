...
int32_t main()
{
    ...
    sort(all(sortednums));
    map<int, int> pos;
    rep(i, n) pos[sortednums[i]] = i;
    rep(i, n)
    {
        nums[i] = pos[nums[i]];
    }
    vi finalpos(n);
    rep(i, n) finalpos[nums[i]] = i;
    ...
    // Can also be solved with a segment tree
    ordered_set<p2> numset;
    rep(i, n) numset.insert(mp(finalpos[sortedv[i]], sortedv[i]));

    rep(i, n)
    {
        int cost = inf;
        int pos = numset.order_of_key(p2(finalpos[sortedv[i]], sortedv[i]));
        int clo = pos;
        int chi = numset.size() - pos - 1;
        cost = min(clo, chi);

        numset.erase(p2(finalpos[sortedv[i]], sortedv[i]));
