...
p2 goalpos;
set<p2> positions;
map<p2, string> moves = { {{0,1},"up"},{{0,-1},"down"},{{-1,0},"left"},{{1,0},"right"} };
string ans;
void move(p2 dir)
{
    playerpos += dir;
    ...
    int i = 0;
    bool trashed = 0;
    while (setcontains(positions, p + dir * i) || goalpos == p + dir * i)
    {
        if (goalpos == p + dir * i)
        {
            trashed = 1;
            positions.erase(p + dir * i);
            break;
        }
        i++;
    }
...
int32_t main()
{
    ...
    {
        dread2(int, a, b);
        positions.emplace(a, b);
    }

    auto getsign = [](int x) {return x >= 0 ? -1 : 1; };

    auto dist = [](p2 a, p2 b) {return abs(a.first - b.first) + (a.second - b.second); };

    while (positions.size())
    {
        p2 t = *begin(positions);
        p2 fgdir = p2(1, 0) * getsign(t.first - goalpos.first);
        p2 sgdir = p2(0, 1) * getsign(t.second - goalpos.second);


        if (playerpos.first == t.first)
        {
            move(fgdir * -1);
        }
        p2 spdir = p2(0, 1) * getsign(playerpos.second - t.second);
        while (playerpos.second != t.second) // Aligned on y
        {
            move(spdir);
        }

        p2 fpdir = p2(1, 0) * getsign(playerpos.first - t.first);
        while (dist(playerpos, t) > 1)
        {
            move(fpdir);
        }
        fpdir = p2(1, 0) * getsign(playerpos.first - t.first);
        if (fpdir.first != fgdir.first)
        {
            move(sgdir);
            move(fpdir);
            move(fpdir);
        ...
        while (playerpos.first != (goalpos - fgdir).first)
        {
            move(fgdir);
        }
        //// aligned on first
        ...
        move(sgdir * -1);
        move(fgdir);
        spdir = p2(0, 1) * getsign(playerpos.second - t.second);
        fpdir = p2(1, 0) * getsign(playerpos.first - t.first);

        if (fpdir.second != fgdir.second)
        {
            move(fpdir);
            move(spdir);
            move(spdir);
        ...
        while (playerpos.second != (goalpos - sgdir).second)
        {
            move(sgdir);
        }
    }
