...
int best(int i, int t, vi& nums)
{
    ...
    int ret = inf;
    ret = min(ret, max(t + nums[i], best(i + 1, t + nums[i], nums)));
    who[i][t] = 0;
    int k = max(t - nums[i], best(i + 1, t - nums[i], nums));
    if (k < ret)
    {
        ret = k;
        who[i][t] = 1;
...
int32_t main()
{
    ...
    while (t--)
    {
        ...
        while (state.first != n)
        {
            path.push_back(who[state.first][state.second]);
            if (who[state.first][state.second]) state.second -= nums[state.first];
            else state.second += nums[state.first];
            state.first++;
        }
        rep(i, n) cout << (path[i] ? 'D' : 'U');
        cout << "\n";
    }
