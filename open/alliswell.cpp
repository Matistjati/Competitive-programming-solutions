...
int32_t main()
{
    ...
    dread3(int, r, c, ppl);
    readvector(int, ranges, ppl);
    rep(i, ppl) ranges[i] *= ranges[i];
    vvi grid(r, vi(c));
    vp2 coords(ppl);
    rep(i, r)
    {
    ...
    priority_queue<tuple<int, p2, int>> pq;
    pq.emplace(-grid[0][0], p2(0, 0), 0);
    int ans = 1e9;
    vvvi vis(26, vvi(r, vi(c)));
    while (pq.size())
    {
        int d, t;
        p2 p;
        tie(d, p, t) = pq.top(); pq.pop();

        if (vis[t][p.first][p.second]) continue;
        vis[t][p.first][p.second] = 1;

        while (t < ppl && dist(p, coords[t]) <= ranges[t])
        {
            t++;
        }
        ...
        repe(dir, dirs)
        {
            ...
            pq.emplace(d - grid[np.first][np.second], np, t);
        }
    }
    if (ans >= 1e9)
    {
