...
int32_t main()
{
    ...
    vector<string> dict;
    unordered_map<ull,int> nameToId;
    ...
    rep(i, n)
    {
        rep(j, dict[i].size())
        {
            repp(k, 'a', 'z' + 1)
            {
                string c(dict[i]);
                c[j] = k;
                ull hash = stringHash(c);
                if (setcontains(nameToId, hash))
                {
                    if (nameToId[hash]!=i) edges[i].push_back(nameToId[hash]);
                }
            }
        }
    }
    ...
    while (getline(cin, line))
    {
        ...
        queries.emplace_back(sp[0],sp[1]);
    ...
    repe(quest, queries)
    {
        queue<p2> q;
        vi prev(n,-1);
        int start = nameToId[stringHash(quest.first)];
        q.emplace(start, -2);
        int target = nameToId[stringHash(quest.second)];
        ...
        while (last!=-2)
        {
            path.push_back(dict[last]);
            last = prev[last];
        }
        reverse(all(path));
        path.push_back(dict[target]);
