...
#endif
const ll mod = 5557;//1e9 + 7;
vp2 dirs = { {0,1},{0,-1},{1,0},{-1,0} };

vvi mult(vvi& a, vvi& b)
{
	assert(a[0].size() == b.size());
	vvi ret(a.size(), vi(b[0].size()));
...
int32_t main()
{
	...
	rep(i, m)
	{
		...
		dread(string, food);
		int mask = 0;
		repe(c, food) mask |= foodbin[c];
		a--; b--;
		edgelist.emplace_back(a, b, mask);
	}
	dread(int, t);
	...
	rep(mask, 1 << k)
	{
		...
		while (k)
		{
			if (k % 2) dist = mult(dist, mat);
			mat = mult(mat, mat);
			k >>= 1;
		}
		ans += (popcount(mask) % 2 ? -1 : 1) * dist[0][n * 2];
	}
