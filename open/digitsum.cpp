...
vector<string> nums;
p2 sdigits(int index, bool unlimited, string num, vvp2& memo, vi& number)
{
    if (index == -1)
    {
        nums.push_back(num);
    ...
    p2& v = memo[index][unlimited];
    if (v.first != -1) return v;

    p2 ret = { 0, 0 };
    int limit = unlimited ? 10 : number[index] + 1;
    rep(i, limit)
    {
        p2 r = sdigits(index - 1, unlimited || (i < number[index]), num+to_string(i), memo, number);
        ret.first += i * r.second+r.first;
        ret.second += r.second;
    }
    v = ret;
    return v;
...
int num_less(int k)
{
    ...
    repe(c, strK) numbers.push_back(c - '0');
    vvp2 memo(strK.size(), vp2(2, { -1,-1 }));
    reverse(all(numbers));
    return sdigits(strK.size() - 1, 0, "", memo, numbers).first;
}
...
int32_t main()
{
    ...
    {
        dread2(int, lo, hi);
        cout << num_less(hi) - num_less(lo-1) << "\n";
    }
