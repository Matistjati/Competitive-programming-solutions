...
void domove(action a)
{
    p2 from = a.from.first;
    p2 to = a.to.first;
    ...
    if (a.newType != '-')
    {
        board[to.second][to.first] = a.newType;
    }
}
...
void undomove()
{
    ...
    if (m.orig != '-')
    {
        board[m.from.first.second][m.from.first.first] = m.orig;
    }
}
...
{
    if ((pos.second == 7 && board[pos.second][pos.first] == 'p') || (pos.second == 0 && board[pos.second][pos.first] == 'P'))
    {
        va ret = movedir(knight, pos, 1, white);
        repe(m, ret) m.orig = white ? 'P' : 'p';
        repe(m, ret) m.newType = white ? 'N' : 'n';
        va r = movedir(queen, pos, inf, white);
        repe(m, r) m.orig = white ? 'P' : 'p';
        repe(m, r) m.newType = white ? 'Q' : 'q';
...
bool in_check(bool white)
{
    ...
    {
        if (board[move.to.first.second][move.to.first.first] == king)
        {
            return true;
        }
    }
...
int32_t main()
{
    ...
    {
        debif(actiontonotation(move) == "d8a4");
        domove(move);
        //if (move.to.first.first == 6 && move.to.first.second == 3) deb;
