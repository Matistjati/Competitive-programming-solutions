...
signed main()
{
	...
	int sr, sc;
	cin >> sr >> sc;
	flow.addEdge(numnodes - 2, r * c + sr + sc * r, inf);
	...
	rep(i, r)
	{
		rep(j, c)
		{
			...
			{
				flow.addEdge(i + j * r, numnodes - 1, inf);
			}
			flow.addEdge(r * c + i + j * r, i + j * r, grid[i][j]);
			repe(dir, dirs)
			{
				p2 np = p2(i + dir.first, j + dir.second);
				if (np.first < 0 || np.second < 0 || np.first >= r || np.second >= c) continue;
				flow.addEdge(i+j*r, r * c + np.first + np.second * r, inf);
			}
		}
	}
