...
int dp[10001][11];
int dpversion[10001][11];
int version = 0;
int ans(int i, int j, int n, string& pattern, vvi& longestvalidsuffix)
{
    if (i == n)
    {
        return 1;
    ...
    if (dpversion[i][j] == version) return dp[i][j];
    int ret = 0;

    auto branch = [&](char nextc)
    {
        int newj = longestvalidsuffix[j][nextc - '0'];
        if (newj == pattern.size()) return 0LL;

        return ans(i + 1, newj, n, pattern, longestvalidsuffix);
    };

    ret = branch('0');
    ret = (ret + branch('1')) % mod;

    dpversion[i][j] = version;
    return dp[i][j] = ret;
}
...
int32_t main()
{
    ...
    memset(dpversion, -10, sizeof(version));
    dread(int, t);
    while (t--)
    {
        version++;
        dread(int, n);
        dread(string, forbidden);
        vvi longestvalidsuffix(forbidden.size(), vi(2, inf));

        vi chars = { '0','1' };
        auto comp = [&](int ind, int c)
        {
            char cv = chars[c];
            string s = forbidden.substr(0, ind) + string(1, cv);

            int best = 0;
            rep(i, s.size())
            {
                string suf = s.substr(i);
                string pre = forbidden.substr(0, suf.size());
                if (pre == suf)
                {
                    best = s.size() - i;
                    break;
            ...
            longestvalidsuffix[ind][c] = best;
        };
        rep(i, forbidden.size()) comp(i, 0), comp(i, 1);
        cout << ans(0, 0, n, forbidden, longestvalidsuffix) << "\n";
    }
