...
int32_t main()
{
    ...
    dread(int, n);
    readvector(int, in_heights, n);

    vi heights;
    int prev = in_heights[0];
    heights.push_back(prev);
    // Remove all chains that are the same
    repp(i, 1, n)
    {
        if (in_heights[i] != prev) heights.push_back(in_heights[i]);
        prev = in_heights[i];
    }
    ...
    vi peaks;
    vi valleys;
    if (heights.size() > 1 && heights[0] > heights[1]) peaks.push_back(0);
    if (heights.size() > 1 && heights[0] < heights[1]) valleys.push_back(0);
    repp(i, 1, n - 1)
    {
    ...
    if (heights.size() > 1 && heights.back() > heights[heights.size() - 2]) peaks.push_back(n - 1);
    if (heights.size() > 1 && heights.back() < heights[heights.size() - 2]) valleys.push_back(n - 1);
    ...
    repe(peak, peaks)
    {
        ...
        auto v = lower_bound(all(valleys), peak);
        int prev = inf;
        int next = inf;

        if (v != end(valleys)) next = heights[*v];
        if (v != begin(valleys)) prev = heights[*(v - 1)];

        ans = max(ans, min(h - next, h - prev));
    }
