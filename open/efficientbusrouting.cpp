...
int countleaves(int u, int p, vvi& edges, vi& cnt, vi& largest)
{

    int ret = edges[u].size() == 1;
    repe(e, edges[u]) if (e != p)
    {
        int k = countleaves(e, u, edges, cnt, largest);
        ret += k;
        largest[u] = max(largest[u], k);
    }

    return cnt[u] = ret;
}

void findleafs(int u, int p, vvi& edges, vi& leaflist)
{
    if (edges[u].size() == 1) leaflist.push_back(u);
    repe(e, edges[u]) if (e != p) findleafs(e, u, edges, leaflist);
}
...
int32_t main()
{
    ...
    vi subtreecnt(n);
    vi largestchild(n);
    countleaves(0, 0, edges, subtreecnt, largestchild);
    int leafcnt = 0;
    rep(i, n) if (edges[i].size() == 1) leafcnt++;

    int best = -1;
    int bestv = inf;
    rep(i, n)
    {
        int m = max(largestchild[i], leafcnt - subtreecnt[i]);
        if (m < bestv)
        {
            bestv = m;
            best = i;
    ...
    {
        vi leafs;
        findleafs(e, best, edges, leafs);
        pq.emplace(leafs.size(), leaflist.size());
        leaflist.emplace_back(leafs);
    }
    ...
    while (pq.size() > 1)
    {
        ...
        ans.emplace_back(leaflist[a.second].back(), leaflist[b.second].back());
        leaflist[a.second].pop_back();
        leaflist[b.second].pop_back();
        if (leaflist[a.second].size())
        {
            pq.emplace(leaflist[a.second].size(), a.second);
        }
        if (leaflist[b.second].size())
        {
            pq.emplace(leaflist[b.second].size(), b.second);
        }
    }
    ...
    {
        p2 a = pq.top();
        ans.emplace_back(leaflist[a.second].back(), best);
    }
