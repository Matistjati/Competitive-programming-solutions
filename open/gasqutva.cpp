...
vp2 dp;
vector<map<int, p2>> antidp;
vector<set<int>> edgesrem;
p2 calc(int u, int p, vi& chari, vvi& edges)
{

    p2 ret = p2(0,1)+ dp[u];
    if (setcontains(antidp[u], p)) ret = ret- antidp[u][p];
    for (auto it = edgesrem[u].begin(); it != edgesrem[u].end();)
    {
        if (*it == p)
        {
            it = next(it);
        ...
        p2 k = calc(*it, u, chari,edges);
        ret += k;
        antidp[u][*it] = k;
        dp[u] += k;
        it = edgesrem[u].erase(it);
    }
    ret.first += ret.second * chari[u];
    p2 c = p2(ret.first + 0, ret.second + 0);
    return c;
...
int32_t main()
{
    ...
    rep(i, n)
    {
        p2 r = calc(i,-1,chari,edges);
        int sc = r.first - chari[i] * n + 100 * n;
        if (sc > ans)
        {
            ans = sc;
            person = i;
