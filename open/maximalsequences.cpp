...
int32_t main()
{
    ...
    // For each query, binary search for how long a sequence we can place
    // We can place a sequence if number of occurences of given numbers is equal to length [ql, m]
    // Compress all values so we can binary search in an occurence array to count number of occurences of each number

    dread(int, n);
    readvector(int, nums, n);
    map<int, int> compress;
    int i = 0;
    repe(num, set<int>(all(nums)))
    {
        compress[num] = i++;
    }
    rep(i, n) nums[i] = compress[nums[i]];

    vvi occurences(compress.size());
    rep(i, n) occurences[nums[i]].push_back(i);
    ...
    while (q--)
    {
        dread2(int, ql, s);
        ql--;
        readvector(int, qnumsraw, s);
        vi qnums;
        rep(i, s) if (setcontains(compress, qnumsraw[i])) qnums.push_back(qnumsraw[i]);
        s = qnums.size();

        rep(i, s) qnums[i] = compress[qnums[i]];
        ...
        while (r - l > 1)
        {
            ...
            rep(i, s)
            {
                auto it = upper_bound(all(occurences[qnums[i]]), m);
                auto it2 = lower_bound(all(occurences[qnums[i]]), ql);
                cnt += distance(it2, it);
            }
