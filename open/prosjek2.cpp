...
double ans(int u, double mid, double value, vi& nums, vector<double>& dp)
{
    if (u == nums.size()) return value;
    double& v = dp[u];
    if (v != -1) return v + value;
    double ret = value;

    ret = max(ret, ans(u + 1, mid, value + nums[u] - mid, nums, dp));
    v = ret - value;
    return ret;
}
...
int32_t main()
{
    ...
    rep(i, 100)
    {
        ...
        double tot = 0;
        rep(i, k) tot += nums[i] - mid;

        double best = -inf;
        vector<double> dp(n, -1);
        repp(i, k, n)
        {
            best = max(best, tot + ans(i, mid, 0, nums, dp));
            tot -= nums[i - k];
            tot += nums[i];
            best = max(best, tot);
        }
