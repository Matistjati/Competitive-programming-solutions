...
bool possible(vp2& people, int m)
{
    int n = people.size();
    ...
    rep(i, n)
    {
        int maxdist = dist(people[i], people[besti]);

        if (maxdist <= m)
        {
            c.emplace_back(people[i]);
        }
        else
        {
            d.emplace_back(people[i]);
        }
    }
