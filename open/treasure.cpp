...
int32_t main()
{
    ...
    cost['M'] = 3;
    priority_queue<p4> q;
    q.emplace(-1, k, start.first, start.second);
    vvvi vis(n, vvi(m, vi(k + 1)));
    while (q.size())
    {
        int a, b, e, c;
        tie(c, e, a, b) = q.top();
        ...
        repe(dir, dirs)
        {
            ...
            int energy = e;
            int co = cost[grid[np.first][np.second]];
            if (energy < co)
            {
                continue;
            }
            ...
            q.emplace(c, energy, np.first, np.second);
        }
    }
    cout << -1;
