...
$w(LONG WINAPI $x(_EXCEPTION_POINTERS*); , )namespace $f {
...
vector<array<int, 3>> queries_sorted;
// assumes queries to be [l,r]
template<typename T, typename Monoid>
vector<T> offlineRangeQueries(const vector<T>& arr, T identity)
{
	int n = arr.size();
	int q = queries_sorted.size();
	...
	for (int i = 0; i < n; i++)
	{
		for (int j = n - i; j < n + 1; j += (j & -j)) fenwick[j] = Monoid{}(fenwick[j], arr[i]);

		while (qp < q&& queries_sorted[qp][1] == i)
		{
			T res = identity;
			for (int j = n - queries_sorted[qp][0]; j > 0; j -= (j & -j)) res = Monoid{}(res, fenwick[j]);
...
typedef pair<short, short> el;
const short inf = short((1 << 15)-1);
struct maxquery
{
	el operator()(el a, el b)
	{
		return el(max(a.first, b.first), max(min(a.first, b.first), max(a.second, b.second)));
	}
};
...
struct minquery
{
	el operator()(el a, el b)
	{
		return el(min(a.first, b.first), min(max(a.first, b.first), min(a.second, b.second)));
	}
};
...
signed main()
{
	...
	vector<p2> queries(q);
	rep(i, q) cin >> queries[i].first, cin >> queries[i].second, queries[i].second -= 2;

	queries_sorted.resize(q);
	...
	vector<el> numsp(n);
	rep(i, n) numsp[i] = el(nums[i], inf);
	vector<el> minqueries = offlineRangeQueries<el, minquery>(numsp, el(inf, inf));

	rep(i, n) numsp[i] = el(nums[i], -inf);
	vector<el> maxqueries = offlineRangeQueries<el, maxquery>(numsp, el(-inf, -inf));
	...
	rep(qind, q)
	{
		int l = queries[qind].first, r = queries[qind].second;

		ll p = ll(nums[l - 1]) * nums[r + 1];
		...
		if (r < l + 7)
		{
			repp(i, l, r + 1)
			{
				repp(j, i + 1, r + 1) ans = max(ans, p * nums[i] * nums[j]);
			}
		}
		else
		{
			if (sum(zerop, l, r) > 0) ans = max(ans, 0LL);
			p2 a = minqueries[qind];
			p2 b = maxqueries[qind];
			array<int, 4> nums;
			int numsz = 0;
			ans = max(ans, p * a.first * a.second);
			ans = max(ans, p * a.first * b.first);
			ans = max(ans, p * a.first * b.second);
			ans = max(ans, p * a.second * b.second);
			ans = max(ans, p * a.second * b.first);
			ans = max(ans, p * b.second * b.first);
		}
