...
signed main()
{
    ...
    cin >> n;
    vector<p2> ppl(n);
    rep(i, n) cin >> ppl[i].first >> ppl[i].second;

    int best = 0;
    rep(i, n) if (ppl[i].first > ppl[best].first) best = i;
    rep(i, n) if (i != best) ppl[i].second--;
    sort(all(ppl));

	auto solve = [&](int mul)
		{
			int nodecnt = 2 * n + 2;
			network_simplex flow(nodecnt);
			rep(i, n)
			{
				if (i != n - 1) flow.addEdge(nodecnt - 2, i, 1, 0);
				flow.addEdge(i + n, nodecnt - 1, ppl[i].second, 0);
				repp(j, i + 1, n)
				{
					flow.addEdge(i, j + n, 1, (ppl[i].first ^ ppl[j].first) * mul);
				}
			}
			p2 ans = flow.maxflow(nodecnt - 2, nodecnt - 1);
			return ans.second * mul;
		};
