...
int32_t main()
{
	...
	auto update = [&](int i, int v)
	{
		...
		trees[j].update(i, 1LL << v);
	};
	rep(i, n)
	{
		color[i]--;
		update(tin[i], color[i]);
	}

	auto get = [](Tree& tree, int l, int r)
	{
		if (l >= 0) l = tree.query(l);
		else l = 0;
		return tree.query(r) ^ l;
	};
	...
	while (q--)
	{
		...
		if (t == 0)
		{
			...
			x--;
			int ans = 0;
			repe(tree, trees) ans += popcount(get(tree, tin[x] - 1, tout[x]));
			cout << ans << "\n";
		}
		else
		{
