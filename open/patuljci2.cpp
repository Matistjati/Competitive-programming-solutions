...
int32_t main()
{
    ...
    dread2(int, n, c);
    readvector(int, colors, n);

    vvi colorind(int(1e4) + 10);
    rep(i, n)
    {
        colorind[colors[i]].push_back(i);
    }
    ...
    while (q--)
    {
        ...
        rep(i, 40)
        {
            int col = colors[randint(a, b)];
            auto lo = lower_bound(all(colorind[col]), a);
            auto hi = upper_bound(all(colorind[col]), b);
            if (hi != begin(colorind[col])) hi = prev(hi);
            if (hi - lo + 1 > (b - a + 1) / 2)
            {
                works = col;
                break;
