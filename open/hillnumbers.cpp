...
int hill_numbers(int index, int prev, bool unlimited, bool increasing, vi& limit, vvvvi& dp)
{
    if (index == limit.size())
    {
        return 1;
    ...
    int& v = dp[index][prev][unlimited][increasing];

    if (v != -1) return v;

    int ret = 0;
    int hi = unlimited ? 10 : limit[index] + 1;
    ...
    rep(d, hi)
    {
        ...
        ret += hill_numbers(index + 1, d, unlimited | (d < hi - 1), newDir, limit, dp);
    }
...
int32_t main()
{
    ...
    vi limit(in.size());
    rep(i, in.size()) limit[i] = in[i] - '0';

    vvvvi dp(in.size(), vvvi(10, vvi(2, vi(2, -1))));
    cout << hill_numbers(0, 0, 0, 1, limit, dp) - 1;
