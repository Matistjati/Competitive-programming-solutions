...
int ansfft(vi nums)
{
    ...
    int n = nums.size();
    fast_set<int> numset;
    rep(i, n) numset.insert(nums[i]);
    numset.erase(0);
    if (find(all(nums), 0) != end(nums))
    {
        map<int, int> nummap;
        rep(i, n) nummap[nums[i]]++;

        int numposnegpairs = 0;
        repe(num, nummap) if (num.first != 0) numposnegpairs += num.second * nummap[-num.first];

        int numzeros = nummap[0];
        //deb;
        ...
        // a+b=0
        ans += numposnegpairs * numzeros;
        if (numzeros > 2) ans += (numzeros) * (numzeros - 1) * (numzeros - 2);
        // a+0=a
        repe(num, nummap) if (num.first != 0) ans += numzeros * (num.second) * 2 * (num.second - 1);


        vi filter;
        rep(i, n) if (nums[i] != 0) filter.push_back(nums[i]);
        nums = filter;
        n = nums.size();
    }
    ...
    repe(i, resset)
    {
        //cout << i-range*2 << ": " << res[i] << "\n";
        if (setcontains(numset, i - range * 2 * m))
        {
            ans += round(res[i]) * vec[i - range];
        }
    }
...
{
    int ans = 0;
    rep(i, nums.size()) rep(j, nums.size()) rep(k, nums.size()) if (i != j && i != k && j != k && nums[i] + nums[j] == nums[k])ans++;
    return ans;
}
