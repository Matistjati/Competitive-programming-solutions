...
vi order;
void dfs1(int u, vi& vis, vvi& edges)
{
    if (vis[u]) return;
    vis[u] = 1;

    repe(e, edges[u]) dfs1(e, vis, edges);

    order.push_back(u);
}

void dfs2(int u, int c, vi& indeg, vi& vis, vvi& edges)
{
    vis[u] = c;
    repe(e, edges[u])
    {
...
int32_t main()
{
    ...
    {
        int u = order[i];
        if (vis[u] != -1 || !isally[u]) continue;
        indeg.push_back(0);
        dfs2(u, indeg.size() - 1, indeg, vis, redges);
    }
