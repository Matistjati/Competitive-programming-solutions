...
typedef __gnu_pbds::gp_hash_table<ull, int, chash> fast_map;
#endif
...
struct Node
{
    ...
    Node(int depth) : index(indexCnt++), depth(depth) { nodes.push_back(this); }
    void insert(string k, int i=0)
    {
        if (i == k.size()) { isLeaf = true; return; }
        if (!setcontains(children, k[i])) children[k[i]] = new Node(depth+1);

        children[k[i]]->insert(k, i + 1);
    }
};
...
void filldict(string w, int d, vector<pair<string,int>>& ret)
{
    ret.emplace_back(w,d);
    if (d == 3) return;
...
int32_t main()
{
    ...
    readvector(string, queries, n);

    string k;
    vector<string> realQueries;
    while (cin>>k)
    {
        //if (k == "asd") break;
        root->insert(k, 0);
        realQueries.push_back(k);
    }

    repe(q, queries)
    {

        vector<pair<string, int>> swapped;
        filldict(q, 0, swapped);

        repe(start, swapped)
        {
            // operations, node
            priority_queue<p2> pq;
            pq.emplace(-start.second, 0);
            vi vis(Node::indexCnt + 2);
            ...
            while (pq.size())
            {
                ...
                if (nodes[u]->isLeaf)
                {
                    nodes[u]->reachable = true;
                }

                Node* v = nodes[u];
                char currentChar = start.first[v->depth];
                if (setcontains(v->children, currentChar))
                {
                    pq.emplace(cost, v->children[currentChar]->index);
                }
                ...
                if ((- cost)<3)
                {
                    repp(i, '0', '9' + 1)
                    {
                        if (setcontains(v->children, i))
                        {
                            pq.emplace(cost-1, v->children[i]->index);
                        }
                    }
                }
            }
