...
int dfs(int u, int c, vi& col, vvi& edges)
{
	if (col[u] != -1) return 0;
	col[u] = c;
	int ret = 1;
	repe(e, edges[u]) ret += dfs(e, c, col, edges);
	return ret;
}
...
int32_t main()
{
	...
	while (t--)
	{
		...
		rep(i, n * 2)
		{
			if (col[i] != -1 || edges[i].size() == 0) continue;
			c++;
			compsize.push_back(dfs(i, c, col, edges));
		}
		repe(e, edgelist)
		{
			compsize[col[e.first]]--;
		}
		bool works = 1;
		repe(c, compsize) if (c < 0) works = 0;
