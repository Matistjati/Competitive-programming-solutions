...
int32_t main()
{
	...
	rep(i, cities.size())
	{
		int x, d, r;
		tie(x, d, r) = cities[i];
		auto it = lower_bound(all(positions), p2(x + d, -inf));
		if (it != positions.end())
		{
			edges[i].emplace_back(it->second + n, r + it->first - x);
		}
		auto it2 = upper_bound(all(positions), p2(x - d, inf));
		if (it2 != positions.begin())
		{
			it2 = prev(it2);
			edges[i].emplace_back(it2->second + n * 2, r + x - it2->first);
		}
	}

	rep(i, n - 1)
	{
		edges[positions[i].second + n].emplace_back(positions[i + 1].second + n, positions[i + 1].first - positions[i].first);
	}
	rep(i, n)
	{
		edges[positions[i].second + n].emplace_back(positions[i].second, 0);
	}

	repp(i, 1, n)
	{
		edges[positions[i].second + n * 2].emplace_back(positions[i - 1].second + n * 2, positions[i].first - positions[i - 1].first);
	}
	rep(i, n)
	{
		edges[positions[i].second + n * 2].emplace_back(positions[i].second, 0);
	}
