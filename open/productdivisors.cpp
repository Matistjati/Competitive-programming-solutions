...
int32_t main()
{
    ...
    smallestFactorSieve(MAX_PR);

    dread(int, n);
    readvector(ull, nums, n);
    vector<ull> factors;
    repe(num, nums) repe(f, GetFactors(num)) factors.emplace_back(f);

    map<ull, int> factorCount;
    repe(f, factors) factorCount[f]++;
    ull ans = 1;
    repe(factor, factorCount)
    {
        ans = ((ans)*(factor.second+1)) % mod;
    }
    cout << ans;
