...
template<typename T> using ordered_multiset = tree<int, null_type, less_equal<T>, rb_tree_tag, tree_order_statistics_node_update>;
// lower_bound is now upper_bound and vice versa
...
int32_t main()
{
    ...
    fast_map<ull, fast_set<ull>> edges;
    vector<string> authors;

    string line;
    while (getline(cin, line) && (line != "LIGMA"))
    {
        vector<string> tokens = split(line, " ");
        ull h1 = stringHash(tokens[0]);
        authors.emplace_back(tokens[0]);
        repp(i, 1, tokens.size())
        {
            ull h2 = stringHash(tokens[i]);
            edges[h1].insert(h2);
            edges[h2].insert(h1);
        }
    }

    fast_map<ull, int> dist;

    queue<pair<ull, int>> q;
    fast_set<ull> vis;
    q.emplace(stringHash("PAUL_ERDOS"), 0);
    ...
    repe(author, authors)
    {
        ull h = stringHash(author);
        if (setcontains(dist, h)) cout << author << " " << dist[h] << "\n";
        else cout << author << " no-connection\n";
    }
