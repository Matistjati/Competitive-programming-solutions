...
int ans(int hi, int s, vi& dp)
{
    ...
    repp(i, 1, hi)
    {
        ...
        int ls;
        // Query l bottom
        ls = max(ls, ans(r, s, dp) + 1); // No pea
        ls = max(ls, ans(l, s, dp) + 1 + s); // Yes pea
...
int32_t main()
{
    ...
    vi mattresses;
    rep(i, m) mattresses.push_back(i);

    while (mattresses.size() > 1)
    {
        int mid = choices[mattresses.size()];
        vi lo;
        vi hi;
        rep(i, mid) lo.push_back(mattresses[i]);
        repp(i, mid, mattresses.size()) hi.push_back(mattresses[i]);

        string query = "? ";
        rep(i, lo.size())
        {
            query += to_string(lo[i]);
            if (i != lo.size() - 1) query += " ";
        }
        cout << query << "\n" << flush;
        dread(int, ans);
        if (ans) mattresses = lo;
        else mattresses = hi;
    }
