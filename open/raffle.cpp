...
LogFactorial fact(int(2e6) + 10);

double f(int n, int p, int x)
{
    return x * exp(fact.choose(n, p - 1) - fact.choose(n + x, p));
}
...
int32_t main()
{
    ...
    while (hi - lo >= 5) {
        int mid = (lo + hi) / 2;
        if (f(n,p,mid) < f(n,p,mid + 1)) lo = mid; // (A)
        else hi = mid + 1;
    }
    repp(i, lo + 1, hi + 1) if (f(n,p,lo) < f(n,p,i)) lo = i; // (B)


    cout << setprecision(15) << f(n,p,lo);
