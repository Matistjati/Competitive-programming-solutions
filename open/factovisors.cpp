...
inline vp2 primeFactors(int n)
{
    vp2 ret;

    if (n % 2 == 0) ret.emplace_back(2, 0);
...
int32_t main()
{
    ...
    while (scanf("%d %d", &a, &b) == 2)
    {
        ...
        repe(factor, primeFactors(b))
        {
            if (n_exp(a, factor.first) < factor.second)
            {
                works = false;
                break;
            }
        ...
        cout << b << (works ? " divides " : " does not divide ") << a << "!\n";
        n++;
