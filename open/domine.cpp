...
// optimized to nlogn using alien dp
vp2 choices = {
    {0b100, 0b111},
    {0b100, 0b100},
    {0b110, 0b110},
...
int n, k;
bool good = 1;
dpv best(int row, int prevrow, double penalty)
{
    if (row == n) return dpv();

    dpv& v = dp[prevrow][row];
    if (good && v.v != -1) return v;
    ...
    {
        p2& c = choices[j];
        if (prevrow & c.first) continue;
        int v = 0;
        rep(i, 3) if (c.second & (1LL << i)) v += rows[i][row];

        if (row) rep(i, 3) if (c.first & (1LL << i)) v += rows[i][row - 1];

        dpv res = best(row + 1, c.second, penalty);
        res.v += v - numplaced[j] * penalty;
        res.placed += numplaced[j];
        if (res.v > ret.v)
        {
            ret = res;
        }
...
int32_t main()
{
    ...
    rep(i, 50)
    {
        ...
        dpv unit;
        unit.v = -1;
        rep(i, 8) rep(j, n) dp[i][j] = unit;
        dpv ret = best(0, 0b111, mid);

        if (ret.placed == k)
        {
            cout << int(round(ret.v + ret.placed * mid));
            quit;
        }
        if (ret.placed > k)
        {
    ...
    dpv ret = best(0, 0b111, lo);
    if (ret.placed == k)
    {
        cout << int(round(ret.v + ret.placed * lo));
        quit;
    }
