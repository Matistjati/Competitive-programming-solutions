...
int best(int mask, vi& videos, vi& dp, int k, int target)
{
    if (mask == target)
    {
        return 0;
    ...
    rep(i, k)
    {
        if ((mask & (1 << i)) == 0) ret = min(ret, cost[i] + best(mask | (1 << i), videos, dp, k, target));
    }
...
int32_t main()
{
    ...
    dread2(int, n, k);
    dread(string, videosstr);
    vi videos;
    map<char, int> chartoint;
    rep(i, n)
    {
        if (!setcontains(chartoint, videosstr[i])) chartoint[videosstr[i]] = chartoint.size();
        videos.push_back(chartoint[videosstr[i]]);
    }

    vi dp(1 << k, -1);
    cout << best(0, videos, dp, k, (1 << k) - 1);
