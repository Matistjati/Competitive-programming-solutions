...
int32_t main()
{
    ...
    dread(int, n);
    priority_queue<p2> events;
    rep(i, n)
    {
        dread3(int, s, t, m);
        s -= 1325000000000LL;
        events.emplace(-s, -m);
        events.emplace(-(s - t), m);
    }
    ...
    rep(i, q)
    {
        ...
        s -= 1325000000000LL;
        e -= 1325000000000LL;
        queries.emplace_back(s, i, -1);
        queries.emplace_back(e, i, 1);
    }
    sort(all(queries));
    ...
    {
        int t, ind, m;
        tie(t, ind, m) = queries[i];
        while (events.size() && -events.top().first <= t)
        {
            if (currt != -inf) sum += (-events.top().first - currt) * persec;
            persec += events.top().second;
            currt = -events.top().first;
            events.pop();
        }

        queryans[ind] += (sum + (t - currt) * persec) * m;
    }

    rep(i, q)
    {
        string s = to_string(queryans[i]);
        string f = "";
        string l = "";
        if (s.size() > 3)
        {
            f = s.substr(0, s.length() - 3);
            l = s.substr(s.length() - 3);
        }
        else
        {
            f = "0";
