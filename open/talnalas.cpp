...
int32_t main()
{
    ...
    dread(string, start);
    dread(string, goal);
    readvector(string, nodesv, m);
    set<string> nodes(all(nodesv));
    nodes.insert(goal);
    ...
    {
        string curr, p;
        tie(curr, p) = q.front(); q.pop();

        if (setcontains(vis, curr)) continue;
        vis[curr] = p;
        ...
        rep(i, n)
        {
            repe(diff, diffs)
            {
                int k = curr[i];
                if (k == '9' && diff == 1) k = '0';
                else if (k == '0' && diff == -1) k = '9';
                else k += diff;
                string c(curr);
                c[i] = k;
                if (setcontains(nodes, c))
                {
                    q.emplace(c, curr);
                }
            }
