...
int32_t main()
{
	...
	rep(i, n)
	{
		rep(j, n)
		{
			dread(int, v);
			v--;
			locations[v].emplace_back(i, j, v == 0 ? 0 : inf);
		}
	}
	...
	repp(v, 1, k)
	{
		vp3& curr = locations[v];

		vp3& prev = locations[v - 1];
		sort(all(prev));
		auto solveforward = [&]()
		{
			LineContainer lct;
			...
			rep(i, curr.size())
			{
				while (j < prev.size() && get<0>(prev[j]) < get<0>(curr[i]))
				{
					int r, c, dp;
					tie(r, c, dp) = prev[j];
					lct.add(-(-2 * c), -(dp + c * c));
				...
				int r, c, dp;
				tie(r, c, dp) = curr[i];
				if (lct.size()) get<2>(curr[i]) = min(dp, -lct.query(c) + c * c);
			}
		};
		auto solvebackward = [&]()
		{
		...
		auto solveforward2 = [&]()
		{
			...
			rep(i, curr.size())
			{
				while (j < prev.size() && get<1>(prev[j]) < get<1>(curr[i]))
				{
					int r, c, dp;
					tie(r, c, dp) = prev[j];
					lct.add(-(-2 * r), -(dp + r * r));
				...
				int r, c, dp;
				tie(r, c, dp) = curr[i];
				if (lct.size()) get<2>(curr[i]) = min(dp, -lct.query(r) + r * r);
			}
		};
		auto solvebackward2 = [&]()
		{
	...
	repe(el, locations.back())
	{
		ans = min(ans, get<2>(el));
	}
