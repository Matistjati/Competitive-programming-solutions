...
int rmq[19][maxn];
int timer = 0;
void dfs(int u, int p, int d, vvp2& edges)
{
    tin[u] = timer++;
    up[0][u] = p;
    rmq[0][u] = d;
    ...
    repp(d, 1, 19)
    {
        ...
        rmq[d][u] = min(rmq[d - 1][u], rmq[d - 1][up[d - 1][u]]);
    }
...
int lca(int a, int b) // path min from a -> lca(a,b)
{
...
int32_t main()
{
    ...
    while (q--)
    {
        ...
        bottleneck = min(lca(a, b), lca(b, a));
        if (c <= bottleneck)
        {
            yes = 1;
        }
