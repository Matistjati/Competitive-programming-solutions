...
int main()
{
    ...
    vd nums(n);
    repp(i, 1, n) nums[(i*i)%n]++;

	vd res = conv(nums, nums);
	repe(v, res)v = round(v);
	ll ans = 0;
	vi h(n);
	repp(i, 1, n) h[2 * (i * i) % n]++;
	repp(i, n, 2 * n-1) res[i - n] += res[i];
	rep(i, n-1) res.pop_back();

	rep(i, n) res[i] = (res[i] + h[i]) / 2;
	rep(i, n) ans += round(res[i]) * nums[i];
	cout << ans << "\n";
