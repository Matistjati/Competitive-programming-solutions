...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        rep(j, m) if (row[j] == 'P') positions.emplace_back(i, j, positions.size());
    ...
    p2 start = { -1,-1 };
    p2 outdir;
    rep(i, n) if (grid[i][0] != '#') start = { i,0 }, outdir = { i,-1 };
    rep(i, n) if (grid[i][m - 1] != '#') start = { i,m - 1 }, outdir = { i,m };
    rep(i, m) if (grid[0][i] != '#') start = { 0,i }, outdir = { -1,i };
    rep(i, m) if (grid[n - 1][i] != '#') start = { n - 1,i }, outdir = { n,i };


    queue<tuple<p2, p2, int>> q;

    q.emplace(start, outdir, 0);

    vvp2 parent(n, vp2(m, mp(-2, -2)));
    vvi dist(n, vi(m, -1));
    ...
    {
        p2 pos; p2 par; int t;
        tie(pos, par, t) = q.front();
        q.pop();

        if (parent[pos.first][pos.second].first != -2) continue;
        parent[pos.first][pos.second] = par;
        dist[pos.first][pos.second] = t;

        repe(dir, dirs)
        {
            p2 np = pos + dir;
            if (np.first < 0 || np.first >= n || np.second < 0 || np.second >= m || grid[np.first][np.second] == '#') continue;
            q.emplace(np, pos, t + 1);
        }
    }
    ...
    auto manhattan = [&](const p2& p) {return dist[p.first][p.second]; };

    int ans = 0;
    vector<string> out(positions.size());
    map<p2, char> dirtomove = { {{0,1},'R'},{{0,-1},'L'},{{1,0},'D'},{{-1,0},'U'} };
    while (positions.size())
    {
        ans++;
        map<p2, int> newpositions;
        sort(all(positions), [&](const p3& lhs, const p3& rhs)
            {
                return manhattan(mp(get<0>(lhs), get<1>(lhs))) < manhattan(mp(get<0>(rhs), get<1>(rhs)));
            });

        rep(i, positions.size())
        {
            p2 p = { get<0>(positions[i]), get<1>(positions[i]) };
            p2 np = parent[p.first][p.second];
            if (np.first == -2) continue;
            int ind = get<2>(positions[i]);
            if (setcontains(newpositions, np))
            {
                debassert(!setcontains(newpositions, p));
                newpositions[p] = ind;
                out[ind] += '.';
            }
            else
        ...
        repe(p, newpositions)
        {
            if (p.first.first < 0 || p.first.first >= n || p.first.second < 0 || p.first.second >= m) continue;

            positions.emplace_back(p.first.first, p.first.second, p.second);
        }
    }
