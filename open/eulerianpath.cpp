...
void dfs(int u, vi& ans, vector<multiset<int>>& edges)
{
...
int32_t main()
{
    ...
    while ((cin >> n >> m) && n)
    {
        ...
        rep(i, n)
        {
            ...
            if (indeg[i] + 1 == outdeg[i] && outwards == -1)
            {
                outwards = i;
            }
            else if (outdeg[i] + 1 == indeg[i] && inwards == -1)
            {
                inwards = i;
            }
            else
        ...
        vi ans;
        dfs((outwards==-1)?0:outwards, ans, edges);
        reverse(all(ans));

        works = true;
        rep(i, n) works &= edges[i].empty();
        if (!works)
        {
            cout << "Impossible\n";
            continue;
