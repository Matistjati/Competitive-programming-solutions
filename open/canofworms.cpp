...
void topsort(int u, vi& vis, vi& order, vvi& edges)
{
    if (vis[u]) return;
    vis[u] = 1;
    ...
    repe(e, edges[u])
    {
        topsort(e, vis, order, edges);
    }
    order.push_back(u);
}

void scc(int u, int c, vi& component, vvi& edges)
{
    if (component[u] != -1) return;
    component[u] = c;
    repe(e, edges[u]) scc(e, c, component, edges);
}
...
int32_t main()
{
    ...
    per(i, n)
    {
        ...
        while (waiting.size() && waiting.top().first <= x + r)
        {
            int b = waiting.top().second;
            waiting.pop();
            edges[i].emplace_back(b);
            redges[b].emplace_back(i);
        }
        waiting.emplace(x, i);
    }

    while (waiting.size()) waiting.pop();
    ...
    vi order;
    vi vis(n);
    rep(i, n) topsort(i, vis, order, edges);
    ...
    rep(i, n)
    {
        ...
        scc(u, c++, component, redges);
    }

    vp2 compval(c, p2(inf, -inf));
    rep(i, n)
    {
        int x, k, ind;
        tie(x, k, ind) = worms[i];
        compval[component[i]].first = min(compval[component[i]].first, x - k);
        compval[component[i]].second = max(compval[component[i]].second, x + k);
    }
    ...
    rep(i, n)
    {
        ...
        repe(e, redges[u])
        {
            p2 p = compval[component[u]];
            p2& ne = compval[component[e]];
            ne.first = min(ne.first, p.first);
            ne.second = max(ne.second, p.second);
        }
    }
    ...
    repe(w, worms)
    {
        wormindices.push_back(get<0>(w));
    }

    vi ans(n);
    rep(i, worms.size())
    {
        p2 p = compval[component[i]];
        auto lo = lower_bound(all(wormindices), p.first);
        auto hi = upper_bound(all(wormindices), p.second);
        if (hi != wormindices.begin()) hi = prev(hi);
        ans[get<2>(worms[i])] = hi - lo + 1;
