...
int32_t main()
{
    ...
    vector<tuple<int, int, string>> eqs;
    eqs.emplace_back(count(all(a), '?'), 0, a);
    eqs.emplace_back(count(all(b), '?'), 1, b);
    eqs.emplace_back(count(all(c), '?'), 2, c);
    sort(all(eqs));

    int numeqs = get<0>(eqs[0]) + get<0>(eqs[1]);



    int ans = 0;
    rep(i, binpow(10, numeqs))
    {
        string s = to_string(i);
        while (s.size() < numeqs) s.insert(s.begin(), '0');
        int k = 0;
        a = get<2>(eqs[0]);
        b = get<2>(eqs[1]);
        c = get<2>(eqs[2]);
