...
int32_t main()
{
    ...
    dread2(int, n, l);
    readvector(string, id, n);

    vvi next(n - 1, vi(l, inf));
    rep(i, n - 1)
    {
        stack<int> waiting;
        rep(j, l)
        {
            ...
            else if (id[i][j] < id[i + 1][j])
            {
                while (waiting.size()) next[i][waiting.top()] = j, waiting.pop();
                next[i][j] = j;
            }
            else
            {
                while (waiting.size()) next[i][waiting.top()] = inf, waiting.pop();
                next[i][j] = inf;
            }
        }
    }
    ...
    cout << inter.first + 1 << " " << inter.second + 1;
