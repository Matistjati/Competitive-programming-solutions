...
int from(string number, string system)
{
    int val = 0;
    ...
    rep(i, number.size())
    {
        val += system.find(number[i]) * mul;
        mul *= system.size();
    }
    return val;
}

string to(int val, string system)
{
    string ret = "";
    while (val != 0)
    {
        ret += system[val % system.size()];
        val /= system.size();
    }
    reverse(all(ret));
    return ret;
...
int32_t main()
{
    ...
    rep(case_, t)
    {
        dread3(string, num, froms, tos);

        int val = from(num, froms);
        cout << "Case #" << case_ + 1 << ": " << to(val, tos) << "\n";
    }
