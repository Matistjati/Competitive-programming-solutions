...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        rep(j, m)
        {
            ...
            e--;
            edges[i].push_back(e);
            edgelist.emplace_back(i, e);
        }
        if (!setcontains(charmap, author)) charmap[author] = charmap.size(), groups.push_back({});
        groups[charmap[author]].push_back(i);
        group.push_back(i);
    }
    ...
    vvi dp(n, vi(1 << numcolors, 0));
    int target = (1 << numcolors) - 1;
    while (elapsedmillis() < 11500)
    {
        cnt++;
        vi color(n);
        rep(i, g)
        {
            int col = randint(0, numcolors - 1);
            repe(u, groups[i])
            {
                color[u] = 1 << col;
            }
        ...
        rep(i, n) rep(mask, 1 << numcolors) dp[i][mask] = -1;
        rep(i, n) dp[i][color[i]] = 1;
        rep(mask, 1 << numcolors)
        {
            rep(i, n)
            {
        ...
        rep(mask, 1 << numcolors)
        {
            ...
            rep(i, n)
            {
                if (dp[i][mask] != -1)
                {
                    ...
                    while (1)
                    {
                        int u = dp[path.back()][mask];
                        mask ^= color[path.back()];
                        if (u == -1) break;
                        path.push_back(u);
                    }
                    reverse(all(path));
