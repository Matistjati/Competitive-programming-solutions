...
int32_t main()
{
    ...
    dread(int, n);
    readvector(string, sentence, n);

    dread(int, m);
    map<string, p2> dict;
    map<string, string> wdict;
    rep(i, m)
    {
        dread(string, a);
        dread(string, _);
        dread(string, corr);
        if (corr == "correct") dict[a].first++;
        else dict[a].second++;
        wdict[a] = _;
    }
    ...
    rep(i, n)
    {
        if (dict[sentence[i]].first > 0 && dict[sentence[i]].second > 0) unique = 0;
        if (dict[sentence[i]].first > 1) unique = 0;
        if (dict[sentence[i]].second > 1) unique = 0;
    }
    ...
    if (unique)
    {
        ...
        rep(i, n)
        {
            if (dict[sentence[i]].second > 0) corr = 0;
            cout << wdict[sentence[i]] << " ";
        }
        if (corr) cout << "\ncorrect";
        else cout << "\nincorrect";
    }
    else
    {
        ...
        {
            int inc = 0;
            ans.first *= dict[sentence[i]].first;
            inc += dict[sentence[i]].first;
            if (dict[sentence[i]].second) inc += dict[sentence[i]].second;
            if (inc) ans.second *= inc;
        }
        cout << ans.first << " correct\n" << ans.second - ans.first << " incorrect";
    }
