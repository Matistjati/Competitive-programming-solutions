...
struct Tree
{
    vi tree;
    int n;
    Tree(int n) : n(n), tree(5 * n, 0) {}

    void update(int x, int l, int r, int ind, int v)
    {
        if (l==r) tree[x] = v;
        else
        {
            int mid = (l + r) / 2;
            if (ind <= mid) update(x * 2, l, mid, ind, v);
            else update(x * 2 + 1, mid + 1, r, ind, v);
            tree[x] = tree[x * 2] + tree[x * 2 + 1];
        }
    }

    void update(int ind, int value) { update(1, 0, n - 1, ind, value); }

    int query(int x, int l, int r, int ql, int qr)
    {
        if (r<ql || l>qr) return 0;
        if (r <= qr && l >= ql) return tree[x];
        int mid = (l + r) / 2;
...
int32_t main()
{
    ...
    dread(int, n);
    readvector(int, a, n / 2);
    readvector(int, b, n / 2);
    ...
    int scoreA = accumulate(all(a), 0LL);
    int scoreB = accumulate(all(b), 0LL);

    Tree playerA(n);
    Tree playerB(n);
    rep(i, a.size()) playerA.update(a[i]-1, 1);
    rep(i, b.size()) playerB.update(b[i]-1, 1);
    ...
    rep(k, n)
    {
        if (k % 2 == 0)
        {
            while (playerA.query(i, i) == 0) i--;
            playerA.update(i, 0);
            scoreA += playerA.query(0, i);
            scoreB += playerB.query(0, i);
        }
        else
        {
            while (playerB.query(j, j) == 0) j--;
            playerB.update(j, 0);
            scoreA += playerA.query(0, j);
            scoreB += playerB.query(0, j);
        }
    }
