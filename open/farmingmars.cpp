...
int32_t main()
{
    ...
    readvector(double, ph, n);
    map<double, vi> occurences;

    rep(i, n) occurences[ph[i]].push_back(i);
    ...
    while (m--)
    {
        ...
        rep(j, 40)
        {
            double v = ph[randint(a,b)];
            auto lo = lower_bound(all(occurences[v]), a);
            auto hi = upper_bound(all(occurences[v]), b);
