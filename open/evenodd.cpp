...
int rangecostsmart(int hi)
{
    ...
    repe(c, choices)
    {
        ...
        else
        {
            int pprev = prev;
            prev = ret;
            ret += modv(modv(modv(ret - pprev) * 2) + modv(v + v / 2));
            if (v % 2 == 1) ret = modv(ret - cost(prev_lo + 1));
            prev_lo = v;
            v *= 2;
        }
    }
...
int32_t main()
{
    ...
    // Solution is basically: if we know S(x), what will S(x*2) be? turns out we can easily compute that
    dread2(int, a, b);
    cout << modv(rangecostsmart(b) - rangecostsmart(a - 1));
