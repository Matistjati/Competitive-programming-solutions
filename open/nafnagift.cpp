...
int dp[1001][1001];
int dpchoice[1001][1001];
int shortest(int a, int b, string& as, string& bs)
{
    ...
    {
        int nb = b;
        if (nb < bs.size() && as[a] == bs[b]) nb++;
        int r = shortest(a + 1, nb, as, bs);
        if (r + 1 < ret.first)
        {
            ret = p2(r + 1, 1);
        }
    ...
    {
        int na = a;
        if (na < as.size() && bs[b] == as[a]) na++;
        int r = shortest(na, b + 1, as, bs);
        if (r + 1 < ret.first)
        {
            ret = p2(r + 1, 2);
        }
...
int32_t main()
{
    ...
    p2 state = p2(0, 0);
    string ans = "";
    while (state.first != a.size() || state.second != b.size())
    {
        assert(dpchoice[state.first][state.second] != -1);
        if (dpchoice[state.first][state.second] == 1)
        {
            ans += a[state.first];
            if (a[state.first] == b[state.second]) state.second++;
            state.first++;
        }
        else
        {
            ans += b[state.second];
            if (b[state.second] == a[state.first]) state.first++;
            state.second++;
        }
    }
    cout << ans;
