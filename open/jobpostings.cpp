...
int32_t main()
{
	...
	while (cin >> n >> m && n)
	{
		int numnodes = n + m + 2;
		MCMF flow(numnodes);
		int source = numnodes - 1;
		int sink = numnodes - 2;

		rep(i, n)
		{
			dread(int, c);
			flow.addEdge(m + i, sink, c, 0);
		}
		rep(i, m)
		{
		...
		rep(i, m)
		{
			dread5(int, y, c1, c2, c3, c4);
			y--;
			vi choice = { c1,c2,c3,c4 };
			rep(j, 4)
			{
				flow.addEdge(i, m + choice[j], 1, -sat[y][j]);
			}
		}
		flow.setpi(source);
		cout << -flow.maxflow(source, sink).second << "\n";
	}
