...
int budget;
double dp[1 << 16][17];
double bestprobability(int bribed, int success, int spent)
{
    if (spent > budget)
    {
        return 0;
    ...
    double& v = dp[bribed][success];
    if (v != -1) return v;

    double ret = 0;
    rep(i, n)
    {
        if (bribed & (1 << i)) continue;
        double ifsuc = bestprobability(bribed | (1 << i), success + 1, spent + cost[i]) * prob[i];
        double iffail = bestprobability(bribed | (1 << i), success, spent + cost[i]) * (1 - prob[i]);
        ret = max(ret, ifsuc + iffail);
    }
    return v = ret;
}
...
int32_t main()
{
    ...
    while (q--)
    {
        ...
        rep(i, n)
        {
            dread2(int, c, p);
            cost.push_back(c);
            prob.push_back(p / 100.);
        }

        cout << setprecision(15) << bestprobability(0, 0, 0) << "\n";
    }
