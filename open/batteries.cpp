...
int mincost(int hi, bool isdead, int cost, vvi& dp)
{
    if (isdead)
    {
        return hi+cost;
    ...
    if (hi <= 0) return cost;
    if (hi == 1) return cost+1;

    int& v = dp[hi][isdead];
    if (v != -1) return cost + v;
    ...
    repp(i, 1, hi)
    {
        int a = mincost(i-1,false,cost+1,dp); // Yes
        int b = mincost(hi-i,true,cost+1,dp); // No
        ans = min(ans, max(a, b));
    }
