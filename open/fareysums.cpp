...
int32_t main()
{
    ...
    cout << "";
    vi ret;
    ret.reserve(30460116);
    // We know first two terms are 0/1 and 1/n
    int a = 0, b = 1, c = 1, d = n;

    ret.emplace_back(max(a, b));
    ret.emplace_back(max(c, d));

    while (c <= n) {
        int k = (n + b) / d;
        /*vi nextV = { c, d, k * c - a,k * d - b};
        a = nextV[0];
        b = nextV[1];
        c = nextV[2];
        d = nextV[3];*/
    ...
    rep(i, sorter.size())
    {
        rep(j, sorter[i]) sorted.push_back(i);
    }
    ...
    {
        dread2(int, q, n);
        int ans = upper_bound(all(sorted), n) - begin(sorted);
        cout << q << " " << 3*ans-4 << "/2\n";
    }
