...
int32_t main()
{
	...
	auto bestingroup = [&](vi& a, int l)
	{
		...
		rep(i, a.size())
		{
			...
			if (lo == i)
			{
				if (lo != 0) best = max(best, a[i] + a[lo - 1]);
			}
			else best = max(best, a[i] + a[lo]);
		}
		return best;
	};
	...
	rep(i, 50)
	{
		...
		rep(i, n)
		{
			if (randint(0, 1)) a.push_back(nums[i]);
			else b.push_back(nums[i]);
		}

		ans = max(ans, bestingroup(a, l1) + bestingroup(b, l2));
	}
