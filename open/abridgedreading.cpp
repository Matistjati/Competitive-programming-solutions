...
int ans = inf;
vi singles;
int best(int u, int d, vi& cost, vvi& edges)
{
    d += cost[u];
    ...
    repe(e, edges[u])
    {
        children.push_back(best(e, d, cost, edges));
    }
    sort(all(children));
    if (children.size() > 1)
    {
        ans = min(ans, children[0] + children[1] + d);
    }
    int ret = children.size() ? children[0] : 0;
    return cost[u] + ret;
}
...
int32_t main()
{
    ...
    sort(all(singles));
    cout << min(ans, singles[0] + singles[1]);
