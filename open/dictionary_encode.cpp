...
void savedict(vector<string>& dict)
{

	repe(s, dict)
	{
		if (s.size() > 1 && s[s.size() - 1] == 's' && s[s.size() - 2] == '\'')
		{
			s.pop_back();
			s.pop_back();
			s.push_back('%');
	...
	rep(i, dict.size())
	{
		...
		if (v < 10) cout << "0" << v << end;
		else cout << v << end;


		string curr = "";
		if (i) curr = dict[i - 1].substr(0, lcp);
		while (curr != s)
		{
			cout << (char)s[curr.size()] << end;
			curr += s[curr.size()];
		}
	}
}
