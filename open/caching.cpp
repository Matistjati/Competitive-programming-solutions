...
int32_t main()
{
    ...
    rep(i, n)
    {
        occs[people[i]].push_back(i);
    }

    auto findnext = [&](int c, int i)
    {
        auto it = upper_bound(all(occs[c]), i);
        if (it == occs[c].end()) return inf;
        return *it;
    };

    rep(i, n)
    {
        if (setcontains(machines, people[i]))
        {
            chop.erase(p2(rv[people[i]], people[i]));
            rv[people[i]] = findnext(people[i], i);
            chop.insert(p2(rv[people[i]], people[i]));
            continue;
        }

        if (machines.size() < k)
        {
            machines.insert(people[i]);
            rv[people[i]] = findnext(people[i], i);
            chop.insert(p2(rv[people[i]], people[i]));
            ans++;
            continue;
        ...
        p2 v = *prev(end(chop));
        chop.erase(prev(end(chop)));

        machines.erase(v.second);
        machines.insert(people[i]);
