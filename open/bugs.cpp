...
struct dualhash
{
    ...
    ull bm = 1;
    dualhash() {}
    ull combine() { return a + b << 32; }
    bool equals(dualhash& other) { return combine() == other.combine(); }
    void add(ull v, bool mul)
    {
        a = (a + v * am) % mod1;
        b = (b + v * bm) % mod2;
        if (mul)
    ...
    void sub(ull v)
    {
        a = ((int(a) - int(v)) % mod1 + mod1) % mod1;
        b = ((int(b) - int(v)) % mod2 + mod2) % mod2;
    }
...
vector<char> ans(4e7);
int ansp = 0;
void cleansmart(string& line, string& rem)
{
    vector<tuple<char, dualhash, int>> final;
    final.reserve(line.size());

    dualhash remhash;
    rep(i, rem.size())
    {
        remhash.add(rem[i], i != rem.size() - 1);
    }
    ...
    rep(i, line.size())
    {
        ...
        if (final.size())
        {
            ...
            tie(_, h, cnt) = final.back();


            if (cnt < rem.size())
            {
                h.add(line[i], cnt != rem.size() - 1);
                cnt++;
            }
            else
            {
                h.sub(get<0>(final[final.size() - rem.size()]));
                h.invone();
                //p = (p*594771246ULL)%mod;
                h.add(line[i], false);
            }
        }
        else
        {
            h.add(line[i], rem.size() > 1);
            cnt = 1;
        }

        if (h.combine() == remhash.combine())
        {
            rep(i, rem.size() - 1) final.pop_back();
        }
        else
        {
            final.emplace_back(line[i], h, cnt);
        }
    }

    rep(i, final.size()) ans[ansp++] = get<0>(final[i]);
    ans[ansp++] = '\n';
}
