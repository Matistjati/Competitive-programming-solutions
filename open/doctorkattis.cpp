...
typedef tuple<int, int, string> cat;
...
int32_t main()
{
    ...
    vector<string> cats;
    map<string, p2> catToState;
    multiset<cat> catqueue;
    ...
    rep(i, n)
    {
        ...
        if (t==0)
        {
            ...
            dread(int, level);
            cats.push_back(name);
            catToState[name] = { -level, cats.size() - 1 };
            catqueue.emplace(catToState[name].first,catToState[name].second,name);
        }
        else if (t==1)
        {
            dread(string, name);
            dread(int, level);
            catqueue.erase(tuple(catToState[name].first, catToState[name].second, name));
            catToState[name].first -= level;
            catqueue.emplace(catToState[name].first, catToState[name].second, name);
        }
        else if (t==2)
        {
        ...
        {
            if (catqueue.size()) cout << get<2>(*begin(catqueue)) << "\n";
