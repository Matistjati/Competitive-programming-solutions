...
bool works(string s)
{
    try
    {
        ...
        if (f == string::npos)
        {
            repe(c, s) if (c < '0' || c>'9') return false;
            return true;
        }

        int b = stoi(s.substr(0, f));

        if (b < 2 || b>16) return false;

        map<char, int> valmap;
        rep(i, 10) valmap[i + '0'] = i;
        rep(i, 6) valmap[i + 'a'] = i + 10;
        s = s.substr(f);
        int it = 0;
        while (true)
        {
            if (s.empty()) return it > 0;
            if (b < 2 || b>16) return false;

            if (count(all(s), '#') % 2 == 1) return false;
            if (s[1] == '#') return false;
            int u = s.find('#', 1);
            string v = s.substr(1, u - 1);
            repe(c, v)
            {
                if (valmap[c] >= b) return false;
            }
