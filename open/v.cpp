...
vi pow10;
int dp[2][2][14][int(1e6)];
int solve(int i, int residue, bool unlimited, bool allzero, int x, vi& digits, vb& allowed)
{
	if (i == digits.size())
	{
		return residue == 0;
	...
	int& v = dp[unlimited][allzero][i][residue];
	if (v != -1) return v;

	int ret = 0;
	int hi = unlimited ? 10 : digits[i] + 1;
	rep(d, hi)
	{
		if (!allowed[d] && !(allzero && d == 0)) continue;
		ret += solve(i + 1, (residue + pow10[sz(digits) - i - 1] * d) % x, unlimited || (d < digits[i]), allzero && (d == 0), x, digits, allowed);
	}

	return v = ret;
}

int doit(int hi, int x, vb& allowed)
{
	vi digits;
	while (hi)
	{
		digits.emplace_back(hi % 10);
		hi /= 10;
	}
	reverse(all(digits));
	memset(dp, -1, sizeof(dp));
	return solve(0, 0, 0, 1, x, digits, allowed);
}
...
int32_t main()
{
	...
	if (x < 1e5)
	{
		cout << doit(b, x, allowed) - doit(a - 1, x, allowed) << "\n";
	}
	else
	{
		int v = x;
