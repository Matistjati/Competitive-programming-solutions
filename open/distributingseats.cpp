...
int32_t main()
{
    ...
    vp2 intervals;
    priority_queue<p2> starting;
    rep(i, n)
    {
        dread3(int, p, _, s);
        p--;
        intervals.emplace_back(p - s, p + s);
        starting.emplace(-(p - s), i);
    }
    int ans = 0;
    ...
    rep(i, r)
    {
        while (starting.size() && (-starting.top().first) <= i)
        {
            p2 p = starting.top();
            starting.pop();
            activeIntervals.emplace(-intervals[p.second].second, p.second);
        }

        while (activeIntervals.size() && (-activeIntervals.top().first) < i) activeIntervals.pop();
