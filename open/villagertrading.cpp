...
void dfs(int u, vi& vis, vvi& edges)
{
    if (vis[u]) return;
    vis[u] = 1;
...
int dfs(int u, vi& vis, vector<vector<pair<int, ld>>>& edges, vector<tuple<int, int, ld>>& edgelist)
{
    if (vis[u]) return 0;
    vis[u] = 1;
    int ret = 1;
    repe(e, edges[u])
    {
        edgelist.emplace_back(u, e.first, e.second);
        ret += dfs(e.first, vis, edges, edgelist);
    }
    return ret;
}
...
int32_t main()
{
    ...
    dread(int, n);
    map<string, int> itemind;

    vector<vector<pair<int, ld>>> edges;
    vector<tuple<int, int, ld>> edgelist;
    vvi redges;
    rep(i, n)
    {
        dread(int, m);
        rep(i, m)
        {
            ...
            if (!setcontains(itemind, na)) itemind[na] = itemind.size();
            if (!setcontains(itemind, nb)) itemind[nb] = itemind.size();
            int ia = itemind[na];
            int ib = itemind[nb];

            while (edges.size() <= max(ia, ib)) edges.push_back(vector<pair<int, ld>>()), redges.push_back(vi());
            ld w = -logl((vb + 0.0) / va); // a*b*... > 1 <=> log(a)+log(b) > 0. We make it negative cause finding negative cycle easier than finding positive cycle
    ...
    n = itemind.size();
    vi seesemerald(n);
    if (!setcontains(itemind, "Emerald"))
    {
        cout << "no";
        quit;
    }
    ...
    {
        if (vis[i]) continue;
        vector<tuple<int, int, ld>> newedgelist;
        int k = dfs(i, vis, edges, newedgelist);

        vector<ld> dist(n, infv);
        dist[i] = 0;
        ...
        rep(j, n) // Phases of relaxation
        {
            repe(e, newedgelist)
            {
                ...
                double w;
                tie(a, b, w) = e;
                if (dist[a] == infv) continue;
                if (dist[a] + w < dist[b])
                {
                    dist[b] = dist[a] + w;
                    if (j == n - 1 && seesemerald[b]) ans = 1; // If relaxing in last phase, we are in negative cycle
                }
            }
        }
