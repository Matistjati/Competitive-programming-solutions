...
vp2 dirs = { {0,1},{0,-1},{1,0} };

typedef tuple<int, int, int, int, int, int> p6;
int vis[4][52][52][52][52];
...
int32_t main()
{
    ...
    rep(i, n) rep(j, m) read(rows[i][j]);
    readvector(string, dict, k);
    ...
    rep(i, m)
    {
        rep(j, k)
        {
            if (rows[0][i] == dict[j][0])
            {
                q.emplace(0, i, 1, j, 0, 3);
            }
        }
    }
    ...
    {
        int r, c, l, i, d, p;
        tie(r, c, l, i, d, p) = q.front(); q.pop();

        if (vis[p][r][c][l][i]) continue;
        vis[p][r][c][l][i] = 1;

        if (r == n - 1 && l == dict[i].size())
        {
            cout << d + 1;
            quit;
        }
        ...
        rep(dirind, dirs.size())
        {
            if (p == 0 && dirind == 1) continue;
            if (p == 1 && dirind == 0) continue;
            p2 np = p2(r, c) + dirs[dirind];
            ...
            if (l == dict[i].size())
            {
                rep(j, k)
                {
                    if (rows[np.first][np.second] == dict[j][0])
                    {
                        q.emplace(np.first, np.second, 1, j, d + 1, dirind);
                    }
                }
            }
            else
            {
                if (rows[np.first][np.second] != dict[i][l]) continue;
                q.emplace(np.first, np.second, l + 1, i, d + 1, dirind);
            }
