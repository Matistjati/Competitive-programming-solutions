...
int ans = 0;
int best(int u, int p, vvp2& edges)
{
    vi children;
    repe(e, edges[u])
    {
        if (e.first == p) continue;
        children.push_back(e.second + best(e.first, u, edges));

    }

    sort(rall(children));
    if (children.size()) ans = max(ans, children[0]);
    if (children.size() > 1) ans = max(ans, children[0] + children[1]);
