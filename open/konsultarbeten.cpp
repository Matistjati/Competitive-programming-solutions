...
vi workTimes = {2,3,4};
int best(int index, vi& times, vi& dp)
{
    ...
    repe(time, workTimes)
    {
        int newTime = times[index] + time*int(1e5);
        auto it = lower_bound(all(times), newTime);
        int newIndex;
        if (it == end(times)) newIndex = times.size();
        else newIndex = it - begin(times);
        ret = max(ret, time + best(newIndex, times, dp));
    }
...
int32_t main()
{
    ...
    dread(int, n);
    readvector(int, times, n);
    sort(all(times));

    vi dp(n, -1);
    cout << best(0, times, dp);
