...
vector<array<int, 3>> manhattanMST(vector<p2> ps) {
	vi id(sz(ps));
	iota(all(id), 0);
	vector<array<int, 3>> edges;
	rep(k, 4) {
		sort(all(id), [&](int i, int j) {
			return (ps[i] - ps[j]).first < (ps[j] - ps[i]).second; });
		map<int, int> sweep;
		for (int i : id) {
			for (auto it = sweep.lower_bound(-ps[i].second);
				it != sweep.end(); sweep.erase(it++)) {
				int j = it->second;
				p2 d = ps[i] - ps[j];
				if (d.second > d.first) break;
				edges.push_back({ d.second + d.first, i, j });
			}
			sweep[-ps[i].second] = i;
		}
		for (p2& p : ps) if (k & 1) p.first = -p.first; else swap(p.first, p.second);
	}
	return edges;
}
...
int32_t main()
{
	...
	vp2 points(n);
	rep(i, n) cin >> points[i].first, cin >> points[i].second;

	vector<array<int, 3>> edges = manhattanMST(points);
	sort(all(edges));
