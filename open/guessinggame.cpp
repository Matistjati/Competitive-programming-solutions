...
int32_t main()
{
    ...
    int guess;
    string strN;
    while (getline(cin, strN) && strN != "0")
    {
        guess = stoi(strN);
        set<int> numbers;
        repp(i, 1, 11) numbers.insert(i);

        string response;

        while (getline(cin, response) && response != "right on")
        {
            if (response == "too high")
            {
                repp(i, guess, 11) numbers.erase(i);
            }
            else
            {
                rep(i, guess + 1) numbers.erase(i);
            }
            getline(cin, strN);
            guess = stoi(strN);
        }

        if (!setcontains(numbers, guess))
        {
