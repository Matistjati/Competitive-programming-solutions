...
typedef __gnu_pbds::gp_hash_table<int, null_type, chash> fast_set;
#endif
...
#if _LOCAL
#define fastmap unordered_map<int, int>
#define fastset unordered_set<int>
#else
#define fastmap fast_map
#define fastset fast_set
#endif

ll solve(int n, vi nums)
{
    unordered_map<int, vp2> sumset;
    rep(i, n)
    {
        repp(j, i + 1, n)
        {
            int k = min(i, j);
            int l = max(i, j);
            sumset[nums[i] + nums[j]].emplace_back(k,l);
        }
    }
    ...
    rep(i, n)
    {
        rep(j, n)
        {
            if (i == j) continue;
            int v = nums[i] - nums[j];
            if (setcontains(sumset, v))
            {
                int lo = min(i, j);
                int hi = max(i, j);
                repe(m, sumset[v])
                {
                    if (m.first == lo || m.first == hi || m.second == lo || m.second == hi) continue;
...
int32_t main()
{
    ...
    ll ans = solve(n, nums);
    if (ans == -LLONG_MAX) write("no solution");
    else cout << ans;


    //rep(i, 1000)
    //{
    //    int n = randint(3500, 4000);
    //    vi nums(n);
    //    rep(i, n) nums[i] = randint(int(-1e5), int(1e5));
    //    //ll a = solvestupid(nums);
    //    ll b = solve(n, nums);
    //    deb;
    //}
