...
int sethash(multiset<int>& s, multiset<int>& b)
{
    int pow = 153;
    int ret = 0;
    int base = 1;
...
// a is mine, b is opponent
double ans(multiset<int> a, multiset<int> b, int d)
{
    if (d==0||(b.size()==0)||(a.empty()&&b.empty()))
    {
        return b.empty();
    }

    int h = sethash(a, b);
    if (setcontains(dp, h)) return dp[h];

    double selectionProb = 1.0 / (a.size()+b.size());

    double ret = 0;
    rep(i, a.size())
    {
        multiset<int> c(a);
        auto it = next(begin(c), i);
        int v = *it;
        c.erase(it);
        if (v > 1) c.insert(v - 1);
        ret += (selectionProb)*ans(c, b, d - 1);
    }
...
int32_t main()
{
    ...
    cout << setprecision(15) << ans(a, b, d);
