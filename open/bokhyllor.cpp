...
int sol(int ones, int twos, int threes, int currcapacity, int capacity, vvvvi& dp)
{
    if (ones == 0 && twos == 0 && threes == 0) return 0;
    int& v = dp[ones][twos][threes][currcapacity];
    if (v != -1) return v;

    int ret = inf;
    vi bois = { ones,twos,threes };
    rep(i, 3)
    {
        if (bois[i] == 0) continue;
        bois[i]--;
        int c = 0;
        int nc = currcapacity;
        if (currcapacity-(i+1)>=0)
        {
            nc -= (i + 1);
        }
        else
        ...
        ret = min(ret, c + sol(bois[0],bois[1],bois[2], nc, capacity,dp));
        bois[i]++;
    }
...
int32_t main()
{
    ...
    vvvvi dp(s + 1, vvvi(m + 1, vvi(l + 1, vi(w + 1, -1))));
    cout << sol(s, m, l, w, w, dp)+1;
