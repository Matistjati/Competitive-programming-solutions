...
int32_t main()
{
    ...
    while (getline(cin, line))
    {
        if (i < n)
        {
            categorynames.push_back(split(line, ' ')[0]);
            vector<string> markers = split(line, ' ');
            repp(j, 2, markers.size())
            {
                whomst[markers[j]].push_back(i);
            }
            i++;
        }
        else
        {
            ...
            repe(word, words)
            {
                repe(who, whomst[word]) cnt[who]++;
            }
        }
    }
    vector<string> ans;
    int most = 0;
    rep(i, n) most = max(most, cnt[i]);
    rep(i, n) if (cnt[i] == most) ans.push_back(categorynames[i]);
    sort(all(ans));
    repe(w, ans) cout << w << "\n";
