...
int best(int count, bool player, vvi& dp, vvi& choice)
{
    ...
    int& v = dp[count][player];
    if (v != -1) return v;

    bool ret = !player;


    if (best(count + 1, !player, dp, choice) == player)
    {
        choice[count][player] = 1;
        ret = player;
    }
    if (best(count + 2, !player, dp, choice) == player)
    {
        choice[count][player] = 2;
        ret = player;
    }
    if (ret == !player)
    {
        choice[count][player] = randint(1, 2);
    }
...
int32_t main()
{
    ...
    vvi dp(target + 1, vi(2, -1));
    vvi choice(target + 1, vi(2, -1));
    cerr << best(0, 1, dp, choice);
    int curr = 0;
    while (true)
    {
        cout << curr + choice[curr][1] << endl;
        if (curr + choice[curr][1] == target) return 0;
        cin >> curr;
        if (curr == target)
        {
            return 0;
