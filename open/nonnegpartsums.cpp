...
int32_t main()
{
    ...
    while (true)
    {
        ...
        rep(i, nums.size())
        {
            ...
            if (prefixLargest.empty() || preSum[i] < prefixLargest.back().first) prefixLargest.emplace_back(preSum[i], i);
        }


        int cum = 0;
        int ans = 0;
        int smallestSuffix = inf;

        per(i, n)
        {
            while (prefixLargest.back().second > i) prefixLargest.pop_back();
            if (smallestSuffix + cum >= 0 && prefixLargest.back().first + cum >= 0) ans++;

            smallestSuffix = min(smallestSuffix, -cum);
            cum += nums[i];
        }
