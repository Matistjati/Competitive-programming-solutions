...
int32_t main()
{
	...
	indexed_set<int> movies;
	rep(i, n) movies.insert(i);
	...
	rep(i, q)
	{
		...
		tie(t, v) = queries[i];
		if (t == 2) continue;
		auto it = movies.find_by_order(v);
		finalpos[k++] = *it;
		movies.erase(it);
	}

	reverse(all(queries));
	reverse(all(finalpos));
	...
	rep(i, q)
	{
		...
		if (t == 1)
		{
			...
			m2.insert({ finalpos[k++],m2.size() + 1 });

		}
		if (t == 2)
		{
			auto it = m2.find_by_order(v);
