...
struct Node
{
    ...
    Node* insert(string& s, int i = 0)
    {
        if (i == s.size()) return this;

        if (!setcontains(children, s[i]))
        {
            children[s[i]] = new Node();
        }
        return children[s[i]]->insert(s, i + 1);
    }


    pair<map<int, int>, incmap> dfs(vi& l, vi& ans, int d)
    {
        pair<map<int, int>, incmap> ret;
        ...
        repe(node, children)
        {
            pair<map<int, int>, incmap> r = node.second->dfs(l, ans, d + 1);

            if (r.first.size() > ret.first.size()) swap(r, ret);

            repe(c, r.second)
            {
                ret.second.increment(c.first, r.second.get(c.first));
            }

            repe(c, r.first)
            {
                assert(c.second);
                ret.first[c.first] += c.second;
                if (ret.first[c.first] == l[c.first])
                {

                    ret.second.increment(c.first, 0);
                }
                if (ret.first[c.first] > l[c.first])
                {
                    //assert(setcontains(ret.second, c.first));

                    ans[c.first] += ret.second.get(c.first);
                    ret.second.erase(c.first);
                }
            }
        }
        if (d != 0) ret.second.inc++;

#if _LOCAL && 0
        pair<map<int, int>, incmap> copy(ret);
        return copy;
#else
        return ret;
#endif
...
int32_t main()
{
    ...
    {
        dread(string, name);
        nodes.push_back(root->insert(name));
    }
    ...
    rep(i, q)
    {
        ...
        rep(j, k)
        {
            dread(int, pokemon);
            pokemon--;
            nodes[pokemon]->queries.push_back(i);
        }
    }
    vi ans(q);

    pair<map<int, int>, incmap> ret = root->dfs(l, ans, 0);

    repe(c, ret.second) ans[c.first] += ret.second.get(c.first);
