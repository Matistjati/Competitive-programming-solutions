...
int32_t main()
{
    ...
    rep(i, n)
    {
        dread(string, name);
        dread(string, s);
        nametoi[name] = status.size();
        int state = -1;
        if (s[0] == 'm') state = MARRIED;
        if (s[0] == 'u') state = UNMARRIED;
    ...
    rep(i, l)
    {
        ...
        redges[bi].emplace_back(ai);

        if (status[ai] == UNKNOWN)
        {
            if (status[bi] == UNKNOWN || status[bi] == UNMARRIED) ans = 1;
        }
        if (status[ai] == MARRIED)
        {
            if (status[bi] == UNKNOWN) ans = 1;
            if (status[bi] == UNMARRIED)
            {
                cout << "1";
                return 0;
            }
    ...
    rep(i, n)
    {
        if (status[i] == MARRIED && edges[i].size())
        {
            queue<int> q;
            q.emplace(edges[i][0]);
            while (q.size())
            {
                int u = q.front();
                q.pop();
                if (status[u] == UNMARRIED)
                {
                    cout << "1";
                    return 0;
                }
                if (status[u] == MARRIED) continue;
                status[u] = MARRIED;
                repe(e, edges[u]) q.emplace(e);
            }
        }
    }
