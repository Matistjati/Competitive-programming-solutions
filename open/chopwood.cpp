...
int32_t main()
{
    ...
    vector<set<int>> amounts(n);
    vi loc(n + 1);
    rep(i, n + 1)
    {
        amounts[occurences[i]].insert(i);
    }
    ...
    rep(i, n)
    {
        ...
        int v = nums[i];
        auto hasleaf = amounts[occurences[v]].find(v);
        amounts[occurences[v]].erase(hasleaf);
