...
int32_t main()
{
    ...
    dread5(int, n, e, x, y, c);

    vvp2 times(101);
    rep(i, c)
    {
        dread3(int, x, y, t);
        times[t].emplace_back(x, y);
    }


    vector<pair<p2, p3>> relevant;
    relevant.emplace_back(p2(x, y), p3(e, 0, 0));
    ...
    rep(i, times.size())
    {
        ...
        map<p3, int> newrelevant;
        repe(cell, goodcells)
        {
            repe(rel, relevant)
            {
                int oldenergy, oldscore, oldtime;
                tie(oldenergy, oldscore, oldtime) = rel.second;
                p2 oilpos = cell.first;
                p2 robotpos = rel.first;
                p2 diff = oilpos - robotpos;
                int dist = abs(diff.first) + abs(diff.second);
                if (dist + oldtime > i || oldenergy < dist)
                {
                    continue;
                }
                p2 profit = cell.second;
                p3 state = { oilpos.first,oilpos.second,oldenergy + profit.second - dist };
                newrelevant[state] = max(newrelevant[state], oldscore + profit.first);
            }
        }

        repe(rel, newrelevant) relevant.emplace_back(p2(get<0>(rel.first), get<1>(rel.first)), p3(get<2>(rel.first), rel.second, i));
    }

    int ans = 0;
    repe(rel, relevant) ans = max(ans, get<1>(rel.second));
    cout << ans;
