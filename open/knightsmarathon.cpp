...
int distance(int x, int y) {
    ...
    int delta = x - y;
    if (y > delta) {
        return delta - 2 * floor((delta - y) / 3.0);
    }
    else {
...
map<p2, int> getmoves(p2 start, int r, int c)
{
    ...
    while (q.size() && vis.size() < 100)
    {
        ...
        if (setcontains(vis, p2(x, y))) continue;
        vis[p2(x, y)] = d;


        repe(dir, dirs)
        {
            q.emplace(x + dir.first, y + dir.second, d + 1);
        }
    }
...
int32_t main()
{
    ...
    p2 goal = { gx,gy };
    p2 p = p2(sx, sy);
    // Later on, we move a bit away from the initial spots so that the borders of the board don't bother us
    // However, if they are too close, that will cause an error. Therefore we brute force in that case
    if (dist(p, goal) < 100)
    {
        map<p2, int> vis;
    ...
    map<p2, int> s = getmoves(p2(sx, sy), r, c);
    map<p2, int> g = getmoves(goal, r, c);
    ...
    repe(a, s)
    {
        repe(b, g)
        {
            p2 c = b.first - a.first;
            int v = a.second + b.second + distance(c.first, c.second);
            debif(v == 14);
            ans = min(ans, v);
        }
    }
