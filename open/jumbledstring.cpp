...
int32_t main()
{
    ...
    {
        int v = i * (i - 1) / 2;
        if (v == a) ra.push_back(i);
        if (v == d) rd.push_back(i);
    }
    ...
    repe(nzero, ra)
    {
        repe(none, rd)
        {
            ...
            if (u - c == b)
            {
                ...
                {
                    int v = diff / nzero;
                    if (none - v - (diff % nzero > 0) < 0) continue;
                    s += string(v, '1');
                    s += string(nzero - diff % nzero, '0');
                    if (diff % nzero) s += string(1, '1');
                    s += string(diff % nzero, '0');
                    s += string(none - v - (diff % nzero > 0), '1');
                    ans = s;
                }
                else
                {
        ...
        repe(none, rd)
        {
            ...
            if (u - b == c)
            {
                ...
                if (none)
                {
                    string s = "";
                    int v = diff / none;
                    if (nzero - v - (diff % none > 0) < 0) continue;
                    s += string(v, '0');
                    s += string(none - diff % none, '1');
                    if (diff % none) s += string(1, '0');
                    s += string(diff % none, '1');
                    s += string(nzero - v - (diff % none > 0), '0');
                    ans = s;
                }
                else
                {
