...
double count(int i, int prev, int k, int n, vector<vector<double>>& dp)
{
	if (i == n) return 1;
	...
	rep(j, k + 1)
	{
		if (prev != 19 && abs(j - prev) > 1) continue;
		ret += count(i + 1, j, k, n, dp);
	}
	return v = ret;
}
...
int32_t main()
{
	...
	while (cin >> k >> n)
	{
		vector<vector<double>> dp(n, vector<double>(20, -1));
		cout << setprecision(15) << 100 * count(0, 19, k, n, dp) / pow(k + 1, n) << "\n";
	}
