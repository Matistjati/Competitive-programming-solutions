...
int32_t main()
{
    ...
    int hi = 440;
    vvi fact(hi);
    vector<multiset<ull>> primefactor(hi);
    fact[0] = { 1 };
    fact[1] = { 1 };
    repp(i, 2, hi)
    {
        ...
        f.push_back(i);
        fact[i] = f;
        repe(num, f) repe(fact, GetFactors(num)) primefactor[i].insert(fact);
    }

    int n, k;
    while (cin >> n >> k)
    {
        multiset<ull> pf = primefactor[n];

        repe(m, primefactor[k]) eraseOne(pf, m);
        repe(m, primefactor[n-k]) eraseOne(pf, m);

        vector<ull> factorCount(all(pf));
        cout << getDivisorcount(factorCount) << "\n";
    }
