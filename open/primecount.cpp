...
struct chash2 { // large odd number for C
    const uint64_t C = ll(4e18 * acos(0)) | 71;
    ll operator()(pair<ll,ll> x) const { return __builtin_bswap64(x.first * C)* __builtin_bswap64(x.second * C);; }
};
typedef __gnu_pbds::gp_hash_table<ll, ll, chash> fast_map;
typedef __gnu_pbds::gp_hash_table<pair<ll,ll>, ll, chash2> fast_map2;
#endif
...
int32_t main()
{
	...
	//cout << "sdfjhn";
	//cout << pi(limit);
	/*uniform_int_distribution<ll> dist(0, 1e11);
	ll ans = 0;
	rep(i, 10000000)
	{
		ans += pi(dist(rng));
