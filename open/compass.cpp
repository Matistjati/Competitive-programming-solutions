...
int32_t main()
{
    ...
    dread2(int, start, goal);

    int clockwise = (start < goal) ? (goal - start) : (360-start+goal);
    int counterclock = (start < goal) ? (360-goal+ start) : (start - goal);

    if (clockwise==counterclock)
    {
        cout << 180;
    }
    else if (clockwise<counterclock)
    {
        cout << clockwise;
    }
    else
