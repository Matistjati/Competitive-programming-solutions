...
vi digits;
ull dp[2][hi + 1][hi + 1];
ull best(int i, int larger, int k)
{
    if (k == 0)
    {
        if (larger) return 0;
    ...
    ull& dpv = dp[larger][i][k];
    if (dpv != -1) return dpv;

    ull ret = inf;
    if (larger)
    {
        ret = min(ret, (1LL << (hi - i)) + best(i + 1, larger, k - 1));
        ret = min(ret, best(i + 1, larger, k));
    }
    else
    {
        if (digits[i] == 0)
        {
            ret = min(ret, best(i + 1, 0, k));
            ret = min(ret, (1LL << (hi - i)) + best(i + 1, 1, k - 1));
        }
        else
        {
            ret = min(ret, (1LL << (hi - i)) + best(i + 1, 0, k - 1));
        }
    }
...
int32_t main()
{
    ...
    while (digits.size() < hi) digits.push_back(0);
    reverse(all(digits));
