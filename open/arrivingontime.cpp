...
bool works(int t, vector<vp4>& edges, int limit)
{
    ...
    while (pq.size())
    {
        ...
        if (u == edges.size() - 1) return w <= limit;

        repe(edge, edges[u])
        {
            int v, t, p, c;
            tie(v, t, p, c) = edge;
            int newt = w + timetillcross(w, t, p) + c;
            pq.emplace(-newt, v);
        }
    }
...
int32_t main()
{
    ...
    rep(i, m)
    {
        dread5(int, u, v, t, p, d);
        edges[u].emplace_back(v, t, p, d);
    }
