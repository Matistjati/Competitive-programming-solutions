...
struct Node
{
    ...
    void extend()
    {
        if (!lchild) lchild = new Node(l, mid);
        if (!rchild) rchild = new Node(mid + 1, r);
    }
    ...
    void update(int ql, int qr, int v)
    {
        ...
        {
            extend();
            lchild->update(ql, qr, v);
            rchild->update(ql, qr, v);
        }
        if (lchild) score = lchild->score + rchild->score;
        else score = 0;

        if (sum > 0) score = r - l + 1;
    }
};
...
int32_t main()
{
    ...
    sort(all(events), [](const p4& a, const p4& b)
        {
    ...
    {
        int a, b, c, d;
        tie(a, b, c, d) = events[i];

        if (i) totarea += root->score * (a - lastx);
        lastx = a;
