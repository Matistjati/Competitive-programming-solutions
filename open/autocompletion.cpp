...
struct node
{
	...
	void add(string& s, int i)
	{
		...
		if (!setcontains(children, s[i])) children[s[i]] = new node();
		children[s[i]]->add(s, i + 1);
	}

	int calc()
	{
		numchildren = isterminal;
		repe(c, children) numchildren += c.second->calc();
		return numchildren;
	}

	void walkto(string& s, int& i, string& ans, int& cntind, vi& cntnum, int left, node* src)
	{
		if (this != src && isterminal) left--;
		if (left == -1)
		{
			ansq(s, i, ans, cntind, cntnum);
			return;
		}
		int k = 0;
		repe(c, children)
		{
			...
			if (k + c.second->numchildren > left)
			{
				left -= k;
				ans += c.first;
				c.second->walkto(s, i, ans, cntind, cntnum, left, nullptr);
				return;
			}
			k += c.second->numchildren;
		}
		assert(0);
	}

	void ansq(string& s, int& i, string& ans, int& cntind, vi& cntnum)
	{
		i++;
		if (i == s.size()) return;
		if (s[i] != '#')
		{
			if (!setcontains(children, s[i])) return;
			ans += s[i];

			children[s[i]]->ansq(s, i, ans, cntind, cntnum);
		}
		else
		{
			if (numchildren - isterminal)
			{
				int num = cntnum[cntind];
				cntind++;
				num %= (numchildren - isterminal);
				walkto(s, i, ans, cntind, cntnum, num, this);
			}
			else
			{
				this->ansq(s, i, ans, cntind, cntnum);
			}
		}
	}
};
...
int32_t main()
{
	...
	rep(i, q)
	{
		...
		int ind = -1;
		int cnt = 0;
		root->ansq(c, ind, ans, cnt, counts);
		//cerr << "::";
		while (ind < c.size())
		{
			if (c[ind] != '#') ans += c[ind];
