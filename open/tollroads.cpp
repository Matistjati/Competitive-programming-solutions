...
void root(int u, int p, int ev, vvp2& edges, vvi& par, vvi& edgemax)
{
    tin[u] = timer++;
    par[u][0] = p;
    edgemax[u][0] = ev;
    repe(e, edges[u]) if (e.first != p) root(e.first, u, e.second, edges, par, edgemax);
    tout[u] = timer++;
}
...
int32_t main()
{
    ...
    repe(e, edges)
    {
        ...
        {
            uf.merge(a, b);
            treedges[a].emplace_back(b, w);
            treedges[b].emplace_back(a, w);
        }
    }
    ...
    tin.resize(n);
    tout.resize(n);
    root(0, 0, 0, treedges, up, edgemax);
    repp(d, 1, 20)
    {
        rep(i, n)
        {
            up[i][d] = up[up[i][d - 1]][d - 1];
            edgemax[i][d] = max(edgemax[i][d - 1], edgemax[up[i][d - 1]][d - 1]);
        }
    }
    ...
    auto pathmax = [&](int a, int b) // path max from a -> lca(a,b)
    {
        if (isancestor(a, b)) return 0LL;
        int u = a;
        int ret = 0;
        per(d, 20)
        {
            ...
            {
                ret = max(ret, edgemax[u][d]);
                u = up[u][d];
            }
        }

        return max(ret, edgemax[u][0]);
    };
    ...
    rep(i, q)
    {
        ...
        int m = max(pathmax(a, b), pathmax(b, a));
        ans[i].first = m;
        queries.emplace_back(m, i, a);
    }
    sort(all(queries));
    ...
    rep(i, q)
    {
        while (p < edges.size() && get<0>(edges[p]) <= get<0>(queries[i]))
        {
            sizeuf.merge(get<1>(edges[p]), get<2>(edges[p]));
            p++;
        }

        ans[get<1>(queries[i])].second = sizeuf.size[sizeuf.find(get<2>(queries[i]))];
    }
