...
bool solve(const vector<string>& strings)
{

    map<ull, set<int>> hashes;
    ...
    auto hashstring = [&](string s, ull base, ull mod, ull inv)
    {
        ull h = 0;
        ull m = 1;
    ...
    auto addtotable = [&](string& s, int it)
    {
        vector<ull> a = hashstring(s, base1, mod1, inv1);
        vector<ull> b = hashstring(s, base2, mod2, inv2);

        rep(i, s.size())
        {
            hashes[(a[i] << 32) + b[i]].insert(it);
        }
    };
    rep(it, strings.size())
    {
    ...
    bool any = 0;
    repe(h, hashes) any |= h.second.size() > 1;
    return any;
}
...
int32_t main()
{
    ...
    dread(int, n);
    readvector(string, strings, n);
    cout << (solve(strings) ? "Yes" : "No");
