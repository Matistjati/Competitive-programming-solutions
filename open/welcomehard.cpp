...
int solve(int i, int ind, vvi& nextocc, vvi& dp)
{
	if (ind == target.size()) return 1;
	auto it = lower_bound(all(nextocc[target[ind]]), i);
	if (it == nextocc[target[ind]].end()) return 0;
	int& v = dp[ind][i];
	if (v != -1) return v;

	int ret = 0;

	ret = (ret + solve(*it, ind + 1, nextocc, dp)) % 10000;
	ret = (ret + solve(*it + 1, ind, nextocc, dp)) % 10000;
...
int32_t main()
{
	...
	rep(CASEFUCK, t)
	{
		...
		rep(i, line.size())
		{
			nextocc[line[i]].push_back(i);
		}


		vvi dp(target.size(), vi(line.size(), -1));
		int v = solve(0, 0, nextocc, dp);
		string ans = to_string(v);
		while (ans.size() < 4) ans.insert(begin(ans), '0');
		cout << "Case #" << CASEFUCK + 1 << ": " << ans << "\n";
	}
