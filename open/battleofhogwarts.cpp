...
vp2 dirs = { {0,1},{1,1},{-1,1} };

map<char, int> cost;
int best(int r, int c, vector<string>& grid, vvi& dp)
{
	if (grid[r][c] == '@') return inf;
	...
	repe(dir, dirs)
	{
		...
		if (np.first < 0 || np.second >= sz(grid[0]))
		{
			ret = 0;
		}
		else
		{
			if (np.first >= grid.size() || np.second < 0) continue;
			ret = min(ret, cost[grid[np.first][np.second]] + best(np.first, np.second, grid, dp));
		}
...
int32_t main()
{
	...
	while (cin >> n >> m && n)
	{
			...
			rep(i, pq.size())
			{
				...
				while (q.size())
				{
					...
					repe(dir, dirs)
					{
						...
						if (np.first < 0 || np.second >= sz(rows[0]))
						{
							ans = min(ans, i);
						}
						else
						{
							if (np.first >= rows.size() || np.second < 0) continue;
							if (rows[np.first][np.second] == '@') continue;
							pq[i + cost[rows[np.first][np.second]]].emplace_back(np.first, np.second);
						}
					}
				}
		...
		rep(i, n)
		{
			if (rows[i][0] != '@') pq[cost[rows[i][0]]].emplace_back(i, 0);
		}
		rep(i, m)
		{
			if (rows[n - 1][i] != '@') pq[cost[rows[n - 1][i]]].emplace_back(n - 1, i);
		}
