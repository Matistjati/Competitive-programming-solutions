...
int bestNaive(int a, int b, int c, int d, int depth)
{
    if (depth > 9 || d == 0) return a * a + b * b + c * c + 7 * min(min(a, b), c);

    int ret = 0;
    ret = max(ret, bestNaive(a + 1, b, c, d - 1, depth + 1));
    ret = max(ret, bestNaive(a, b + 1, c, d - 1, depth + 1));
    ret = max(ret, bestNaive(a, b, c + 1, d - 1, depth + 1));
...
int32_t main()
{
    ...
    rep(i, n)
    {
        dread4(int, a, b, c, d);
        vi nums = { a,b,c };
        (* max_element(all(nums)))+=d;
        int v = nums[0] * nums[0] + nums[1] * nums[1] + nums[2] * nums[2] + 7 * (*min_element(all(nums)));

        cout << max(bestNaive(a, b, c, d, 0), v) << "\n";
    }
