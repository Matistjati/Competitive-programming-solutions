...
int32_t main()
{
    ...
    queue<p4> q;
    vvp2 par(24, vp2(60, { -1, -1 }));
    q.emplace(starthour, startminute, -2, -2);

    while (q.size())
    {
        int h, m, p1, p2;
        tie(h, m, p1, p2) = q.front();
        q.pop();

        if (par[h][m].first != -1) continue;
        par[h][m] = { p1,p2 };

        if (h == endhour && m == endminute) break;

        auto tryadd = [&](int i, int p, int v)
        {
            int hn = h;
            int mn = m;
            int& ed = (i == 0) ? hn : mn;
            ...
            {
                if (v == 1 && ed % 10 == 9) ed -= 9;
                else if (v == -1 && ed % 10 == 0) ed += 9;
                else ed += v;
            }
            else if (p == 1)
            {
                if (v == 1 && (ed / 10 * 10) == 9) ed -= 90;
                else if (v == -1 && (ed / 10 * 10) == 0) ed += 90;
                else ed += v * 10;
            }

            if (hn > 23 || mn > 59) return;
            q.emplace(hn, mn, h, m);
        };
        tryadd(0, 0, 1);
        tryadd(0, 0, -1);
    ...
    repe(s, states)
    {
        if (s.first == 0) cout << "00";
        else if (s.first / 10 == 0) cout << "0" << s.first;
        else cout << s.first;
        cout << ":";
        if (s.second == 0) cout << "00";
        else if (s.second / 10 == 0) cout << "0" << s.second;
        else cout << s.second;
        cout << "\n";
    }
