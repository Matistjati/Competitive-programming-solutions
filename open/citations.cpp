...
p3 dfs(int u, vvi& edges, vi& weight)
{
    p3 ret = { weight[u] + 1,1,weight[u] + 1 };

    vp3 subtrees;
    repe(e, edges[u])
    {
        subtrees.emplace_back(dfs(e, edges, weight));
    }
    sort(all(subtrees), [](p3& a, p3& b)
        {
            return (0. + get<0>(a)) / get<1>(a) < (0. + get<0>(b)) / get<1>(b);
        });
    ...
    p3 k = { get<0>(ret),get<1>(ret),get<2>(ret) };
    return k;
}
...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        dread(int, k);
        edges[i].resize(k);
        rep(j, k) cin >> edges[i][j];
        rep(j, k) edges[i][j]--;
    }

    p3 ans = dfs(0, edges, weight);
    cout << get<2>(ans);
