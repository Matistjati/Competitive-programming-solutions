...
void genprimes(int index, int num, int lo, int hi, vi& primes, set<int>& ret)
{
    if (num>hi) return;
    if (num >= lo) ret.insert(num);
    if (index == primes.size()) return;

    genprimes(index+1, num, lo, hi, primes, ret);
    genprimes(index, num * primes[index], lo, hi, primes, ret);
}
...
int32_t main()
{
    ...
    while ((cin>>n)&&n)
    {
        readvector(int, primes, n);
        dread2(int, lo, hi);
        set<int> nums;

        genprimes(0, 1, lo, hi, primes, nums);

        if (nums.empty()) cout << "none\n";
        else
        {
            repe(num, nums)
            {
                cout << num;
                if (nums.find(num)!=prev(end(nums)))
                {
                    cout << ",";
                }
            }
