...
void normalize(deque<int>& state)
{
    bool changed = 1;
    while (state.size() > 1 && changed)
    {
        changed = 0;
        if (state[0] == state[1])
        {
            ...
            state.pop_front();
            state.pop_front();
            state.push_front(2 * v);
            changed = 1;
        }
        else if (state.back() == state[state.size() - 2])
        {
            int v = state.back();
            state.pop_back();
            state.pop_back();
...
bool works(int i, deque<int> state, vi& blocks, vector<map<int, int>>& dp)
{
    repp(j, 1, sz(state) - 1)
    {
        if (state[j] < state[j - 1] && state[j] < state[j + 1])
        {
            return 0;
        }
    }
    ...
    bool ret = 0;
    {
        deque<int> statec(state);
        statec.push_front(blocks[i]);
        normalize(statec);
        ret |= works(i + 1, statec, blocks, dp);
        if (ret)
        {
            dp[i][h] = LEFT;
        }
