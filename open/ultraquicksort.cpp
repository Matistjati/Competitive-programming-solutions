...
struct Tree
{
    ...
    void update(int x, int l, int r, int v, int i)
    {
        if (l == r) tree[x] = v;
        else
        {
            int mid = (l + r) / 2;
            if (i <= mid) update(x * 2, l, mid, v, i);
            else update(x * 2 + 1, mid + 1, r, v, i);
            tree[x] = tree[x * 2] + tree[x * 2 + 1];
        }
    }

    void update(int v, int i) { update(1, 0, n - 1, v, i); }
...
int32_t main()
{
    ...
    dread(int, n);
    readvector(int, unsorted, n);

    vi sorted(unsorted);
    sort(all(sorted));
    map<int, int> finalPos;
    rep(i, n) finalPos[sorted[i]] = i;

    vi nums(n);
    rep(i, n) nums[i] = finalPos[unsorted[i]];

    int ans = 0;
    Tree tree(n);
    rep(i, n)
    {
        ans += tree.query(nums[i] + 1, n - 1);
        tree.update(tree.query(nums[i], nums[i]) + 1, nums[i]);
    }
