...
const int fastmod = (1 << 16) - 1;
//void countsort(vi& nums, int power)
//{
//    static vi out(nums.size());
//    vi count(radix);
//    rep(i, radix) count[i] = 0;
//
//    repe(num, nums) count[(num / power) % radix]++;
//    repp(i, 1, radix) count[i] += count[i - 1];
//
//    for (int i = nums.size() - 1; i >= 0; i--)
//    {
//        int num = nums[i];
//        out[count[(num / power) % radix]-1] = num;
...
int32_t main()
{
    ...
    while (t--)
    {
        ...
        repp(i, 1, n)
        {
            nums[i] = (nums[i - 1] * (ll)b + a) % c;
        }

        rep(i, radix) count[i] = 0;

        repe(num, nums) count[num & fastmod]++;
        repp(i, 1, radix) count[i] += count[i - 1];

        per(j, nums.size())
        {
            int num = nums[j];
            out[count[num & fastmod] - 1] = num;
            count[num & fastmod]--;
        }

        rep(i, nums.size()) nums[i] = out[i];

        rep(i, radix) count[i] = 0;

        repe(num, nums) count[num / radix]++;
        repp(i, 1, radix) count[i] += count[i - 1];

        per(j, nums.size())
        {
            int num = nums[j];
            out[count[num / radix] - 1] = num;
            count[num / radix]--;
        }
