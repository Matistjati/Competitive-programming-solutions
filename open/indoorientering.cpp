...
int32_t main()
{
    ...
    vvi dist(n, vi(n));
    rep(i, n) rep(j, n) read(dist[i][j]);

    if (n < 8)
    {
        vi nums;
        repp(i, 1, n) nums.push_back(i);

        do
        {
            int curr = nums[0];
            int d = dist[0][curr] + dist[nums.back()][0];
    ...
    rep(mask, (1 << n))
    {
        ...
        vector<set<int>> lo(n + 1);
        int mid = n / 2;
        vi nums;
        vi antinums;
        repp(i, 1, n)
        {
            if (mask & (1 << i)) nums.push_back(i);
            else antinums.push_back(i);
        }
        ...
        do
        {
            ...
            repe(num, antinums)
            {
                int d2 = d + dist[num][nums[0]];
                if (setcontains(lo[num], l - d2))
                {
                    cout << "possible";
                    quit;
                }
