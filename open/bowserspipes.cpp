...
p2 dfs(int u, int c, vi& color, vvi& redges)
{
    p2 ret = p2(inf, inf);
    if (redges[u].empty()) ret = p2(0, u);
    color[u] = c;

    repe(e, redges[u])
    {
        p2 r = dfs(e, c, color, redges);
        r.first++;
        ret = min(ret, r);
    }
    return ret;
...
int32_t main()
{
    ...
    rep(i, n)
    {
        if (edge[i] == -1) closest[i] = dfs(i, i, color, redges).second;
    }
