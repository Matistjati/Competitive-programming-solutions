...
int query(int a, int b)
{
	k++;
	assert(k < 1000);
	assert(a >= 0 && a <= int(1e6));
	assert(b >= 0 && b <= int(1e6));
	cout << a << " " << b << endl;
	dread(int, k);
	return k;
}
...
int32_t main()
{
	...
	while (found < n)
	{
		p2 p = { randint(0, int(1e6)) ,randint(0, int(1e6)) };
		int d = query(p.first, p.second);
		if (d == 0)
		{
			found++;
			continue;
		...
		rep(a, 1e6 + 1)
		{
			int d1 = (p.first - a);
			d1 *= d1;
			if (!setcontains(squares, d - d1)) continue;

			int b = -(squares[d - d1] - p.second);
			if (b >= 0 && b <= int(1e6)) numsols++;

			b = -(-squares[d - d1] - p.second);
			if (b >= 0 && b <= int(1e6)) numsols++;
		}

		if (numsols > 140) continue;
		cerr << numsols << "\n\n" << endl;
