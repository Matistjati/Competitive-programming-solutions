...
inline int hashp2(const p2& v) {
    return v.first * (4 * 100000 + 19) + v.second;
}

inline int hashp2(int a, int b) {
    return hashp2(mp(a, b));
}
...
int32_t main()
{
    ...
    fast_set<int> seenedges;
    rep(i, n - 1)
    {
        dread2(int, a, b);
        a--; b--;
        if (a > b) swap(a, b);
        if (setcontains(seenedges, hashp2(a, b))) continue;
        seenedges.insert(hashp2(a, b));
    ...
    int ans = 0;
    // Suppose that i is the middle vertex of length-2 path. For each of its neighbours, it has a path to all its other neighbours
    rep(i, n)
    {
        ans += edges[i].size() * (edges[i].size() - 1);
    }

    int sqrtn = sqrt(n);
    vi isheavy(n);
    rep(i, n) if (edges[i].size() >= sqrtn) isheavy[i] = 1;

    // However, what if there is an edge between two neighbours? Then it's not a valid path. This is equivalent to counting triangles
    vi trongles(4);
    vi heavy;
    rep(i, n)
    {
        // There are at most sqrt(m) nodes with deg(i)<sqrt(m). for each, do sqrt(m)^2=m work. there are at most sqrt(m), so this becomes msqrt(m)
        if (edges[i].size() < sqrtn)
        {
            rep(j, edges[i].size())
            {
                rep(k, edges[i].size())
                {
                    p2 nodes = { edges[i][j],edges[i][k] };
                    if (nodes.first > nodes.second) swap(nodes.first, nodes.second);

                    if (j != k && setcontains(seenedges, hashp2(nodes)))
                    {
                        int num_heavy = isheavy[i] + isheavy[edges[i][j]] + isheavy[edges[i][k]];
                        trongles[num_heavy]++;
                    }
                }
            }
    ...
    // There are at most sqrt(m) nums with with degree > sqrt(m). thus we can check all triplets and check if they work
    rep(i, heavy.size())
    {
        repp(j, i + 1, heavy.size())
        {
            repp(k, j + 1, heavy.size())
            {
                int a = heavy[i];
                int b = heavy[j];
                int c = heavy[k];
                p2 ab = { min(a,b),max(a,b) };
                p2 ac = { min(a,c),max(a,c) };
                p2 bc = { min(b,c),max(b,c) };
                if (setcontains(seenedges, hashp2(ab)) && setcontains(seenedges, hashp2(ac)) && setcontains(seenedges, hashp2(bc)))
                {
                    trongles[3]++;
                }
            }
    ...
    cout << ans - trongles[0] - trongles[1] / 2 * 3 - trongles[2] * 3 - trongles[3] * 6;
