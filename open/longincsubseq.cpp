...
struct Tree
{
    int n;
    vp2 tree;
    Tree(int n) : n(n), tree(n*4, p2(-INT_MAX, -1)) {}
    ...
    void update(int x, int l, int r, int i, p2 v)
    {
        if (l == r) tree[x] = v;
        else
        {
    ...
    void update(int i, p2 v) { update(1, 0, n - 1, i, v); }

    p2 query(int x, int l, int r, int ql, int qr)
    {
        if (r<ql || l>qr) return p2(-inf, -1);
...
int32_t main()
{
    ...
    while (cin>>n)
    {
        ...
        rep(i, sorted.size()) numToPos[sorted[i]] = i+1;
        rep(i, n + 2) tree.update(i, p2(0,-1));
        tree.update(0, p2(0,-1));
        ...
        rep(i, n)
        {
            int hi = numToPos[nums[i]] - 1;
            p2 best = tree.query(0, hi)+p2(1,0);
            prev[i] = best;
            tree.update(numToPos[nums[i]], p2(best.first, i));
        ...
        while (bestInd != -1)
        {
            ans.push_back(bestInd);
            bestInd = prev[bestInd].second;
        }
        reverse(all(ans));
        cout << ans.size() << "\n";
        repe(v, ans) cout << v << " ";
