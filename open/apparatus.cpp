...
// Each query splits our input into disjoint sets
// ans = factorial(size of component) for each component
// From here, just detect two errors conditions: lights lit up != num switches pressed, or that even after combining each set we don't light up them all
...
int32_t main()
{
    ...
    dread2(int, n, m);
    vector<bset> adjacent(n);
    rep(i, n) rep(j, n) adjacent[i][j] = 1;
    rep(i, m)
    {
        dread2(string, switches, lamps);
        int a = 0;
        rep(j, n) a += switches[j] == '1';

        bset lamp;
        rep(j, n) lamp[j] = (lamps[j] == '1');
    ...
    int ans = 1;
    unordered_map<bset, int> components;
    repe(adj, adjacent) components[adj]++;

    ModFactorial fact(1010, mod);

    bset possible;
    repe(comp, components)
    {
        ans *= fact[comp.second];
        ans %= mod;
        possible |= comp.first;
    }
