...
struct pointpair
{
    ...
    pd a;
    pd b;
    pointpair(pd a, pd b, double dist) : dist(dist), a(a), b(b) {}
};

inline double dist(pd a, pd b)
{
...
inline pointpair stripbruteforce(vector<pd>& points, double mindist)
{
    double closest = inf;
    int besti = -1;
    int bestj = -1;
...
pointpair closestpair(vector<pd> sortx, vector<pd> sorty)
{
    ...
    double closest = min(leftclosest.dist, rightclosest.dist);
...
int32_t main()
{
    ...
    while (cin >> n && n)
    {
        ...
        cout << ans.a.first << " " << ans.a.second << " " << ans.b.first << " " << ans.b.second << "\n";
    }
