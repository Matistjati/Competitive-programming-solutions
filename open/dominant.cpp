...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        string s = dict[i];
        int mask = 0;
        repe(c, s) assert(c >= 'a' && c <= 'z');
        repe(c, s) mask |= 1LL << (c - 'a');
    ...
    rep(i, n)
    {
        rep(j, n)
        {
            ...
            rep(k, 26)
            {
                if (freq[j][k] < freq[i][k]) // j does not dominate i
                {
                    goto end;
                }
            }

            goto done; // j dominates i, since each index is >= 
        end:;
        }
        ans.push_back(i);
    done:;
