...
struct Node
{
	...
	bool has(string& s, int i)
	{
		if (i == s.size()) return 1;
		if (setcontains(children, s[i]))
		{
			return children[s[i]]->has(s, i + 1);
		}
		return 0;
	}
};

void dfs(p2 u, vvi& vis, vector<string>& grid, Node* c, int d)
{
	...
	if (!setcontains(c->children, grid[u.first][u.second])) c->children[grid[u.first][u.second]] = new Node();
	c = c->children[grid[u.first][u.second]];

	repe(dir, dirs)
	{
		p2 np = u + dir;
		if (np.first < 0 || np.second < 0 || np.first >= grid.size() || np.second >= grid[0].size() || vis[np.first][np.second]) continue;
		dfs(np, vis, grid, c, d + 1);
	}
