...
int32_t main()
{
    ...
    dread(int, n);
    readvector(int, bottles, n);

    vi leftcnt(n);
    indexed_multiset<int> seen;
    rep(i, n)
    {
        leftcnt[i] = seen.size() - seen.order_of_key(bottles[i] * 2);
        seen.insert(bottles[i]);
    }

    seen = indexed_multiset<int>();
    int ans = 0;
    per(i, n)
    {
        ans += leftcnt[i] * (seen.order_of_key(bottles[i] / 2 + 1));
        seen.insert(bottles[i]);
    }
