...
int32_t main()
{
    ...
    dread(string, instructions);
    p2 start;
    rep(i, n) rep(j, m) if (rows[i][j] == 'S') start = p2(i, j);
    ...
    while (q.size())
    {
        ...
        if (t >= instructions.size()) continue;

        rep(i, 4)
        {
            p2& dir = dirs[i];
            p2 np = p2(r, c) + dir;
            if (np.first < 0 || np.second < 0 || np.first >= n || np.second >= m || rows[np.first][np.second] == '#') continue;
            if (names[i] == instructions[t]) continue;
            q.emplace(np.first, np.second, t + 1);
        }
    }
    ...
    while (q.size())
    {
        ...
        if (d == instructions.size() && vis[r][c])
        {
            ans[r][c] = 1;
        }
