...
int dfs(int u, vvi& edges, vi& nodevalue, vi& dp)
{
    if (dp[u] != -1) return dp[u];
    int ret = nodevalue[u];
    repe(e, edges[u])
    {
        return dp[u] = dfs(e, edges, nodevalue, dp);
    }
    return dp[u] = ret;
}
...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        if (t == 1)
        {
            dread(int, v);
            assert(v<int(1e9));
            if (!seen[v]) allvalues.push_back(v);
        ...
        else
        {
            ...
            assert(a <= int(1e9 + 10));
            assert(b <= int(1e9 + 10));
            if (!seen[a]) allvalues.push_back(a);
            if (!seen[b]) allvalues.push_back(b);
            queries.emplace_back(t, a, b);

        }
    }
    sort(all(allvalues));
    fast_map<int, int> myind;
    rep(i, allvalues.size()) myind[allvalues[i]] = i;

    vvi edges;
    vi nodeind;
    vvi waiting(allvalues.size());
    vi nodevalue;
    rep(i, n)
    {
        ...
        {
            int v = myind[get<1>(queries[i])];
            waiting[v].emplace_back(edges.size());
            nodeind.emplace_back(edges.size());
            nodevalue.emplace_back(v);
            edges.emplace_back(vi());
        }
        else
        {
            int a = myind[get<1>(queries[i])];
            int b = myind[get<2>(queries[i])];
            if (a == b) continue;

            edges.emplace_back(vi());
            edges.emplace_back(vi());
            nodevalue.emplace_back(a);
            nodevalue.emplace_back(b);
            edges[edges.size() - 2].emplace_back(edges.size() - 1);
            waiting[b].emplace_back(edges.size() - 1);
            repe(o, waiting[a])
            {
                edges[o].emplace_back(edges.size() - 2);
            }
            waiting[a] = vi();
        }
    }
    ...
    rep(i, nodeind.size())
    {
        cout << allvalues[dfs(nodeind[i], edges, nodevalue, dp)] << "\n";
    }
    quit;
}