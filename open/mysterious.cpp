...
struct Tree
{
	...
	Tree(int n) : n(n), sum(n * 4), score(n * 4) {}
	...
	void update(int x, int l, int r, int ql, int qr, int val)
	{
		...
		{
			int mid = (l + r) / 2;
			update(x * 2, l, mid, ql, qr, val);
			update(x * 2 + 1, mid + 1, r, ql, qr, val);
		}
		if (x * 2 + 1 < score.size()) score[x] = score[x * 2] + score[x * 2 + 1];
		else score[x] = 0;

		if (sum[x] != 0) score[x] = (r - l + 1);
	}
};
...
int32_t main()
{
	...
	vb pivot(n);
	vp2 pivotbounds(n, { 0,n - 1 });
	vp2 leftbounds(n, { inf,-inf });
	rep(i, q)
	{
		dread3(int, a, b, c);
		a--; b--;
		c--;
		pivot[c] = 1;
		pivotbounds[c].first = max(pivotbounds[c].first, a);
		pivotbounds[c].second = min(pivotbounds[c].second, b);
		leftbounds[c].first = min(leftbounds[c].first, a);
		leftbounds[c].second = max(leftbounds[c].second, b);
	}

	rep(i, leftbounds.size())
	{
		p2 l = leftbounds[i];
		if (pivot[i])
		{
			tree.update(1, 0, n - 1, l.first, l.second, 1);
		}
	}
	...
	rep(i, n)
	{
		if (pivot[i])
		{
			tree.update(1, 0, n - 1, leftbounds[i].first, leftbounds[i].second, -1);
			tree.update(1, 0, n - 1, 0, pivotbounds[i].first - 1, 1);
			tree.update(1, 0, n - 1, pivotbounds[i].second + 1, n - 1, 1);
			cnt[i] = n - tree.score[1];
			tree.update(1, 0, n - 1, 0, pivotbounds[i].first - 1, -1);
			tree.update(1, 0, n - 1, pivotbounds[i].second + 1, n - 1, -1);
		}
		else
