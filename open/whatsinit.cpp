...
#define KNOWN 1
	...
	rep(ind, n)
	{
		...
		repp(p, prevknown, 101)
		{
			vp2 numc(n);
			int s = 0;
			rep(i, n)
			{
				if (nums[i] == -1) numc[i] = p2(-1, !KNOWN);
				else numc[i] = p2(nums[i], KNOWN);
			}
			...
			{
				while (s < 100 && numc[i].first < prev && numc[i].second == !KNOWN)
				{
					numc[i].first++;
					s++;
				}
			...
			rep(i, n - 1)
			{
				if (numc[i].first > numc[i + 1].first) works = 0;
			}
			if (!works) continue;

			bounds[ind].first = min(bounds[ind].first, p);
			bounds[ind].second = max(bounds[ind].second, p);
		}
	}
	...
	rep(i, n)
	{
		if (nums[i] != -1) cout << words[i] << " " << nums[i] << " " << nums[i] << "\n";
		else cout << words[i] << " " << bounds[i].first << " " << bounds[i].second << "\n";
	}
