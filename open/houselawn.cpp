...
int32_t main()
{
    ...
    int l = stoi(split(startLine, ' ')[0]);
    int m = stoi(split(startLine, ' ')[1]);


    vector<tuple<string, int, int, int, int>> mowers(m);
    rep(i, m)
    {
        string line;
        getline(cin, line);
        ...
        mowers[i] = { parts[0], stoi(parts[1]),stoi(parts[2]),stoi(parts[3]),stoi(parts[4]) };
    }

    stable_sort(all(mowers), [](const auto& lhs, const auto& rhs)
        {
    ...
    rep(i, mowers.size())
    {
        string name;
        int p, c, t, r;
        tie(name, p, c, t, r) = mowers[i];

        int cut = c*t*10080. / (t+r);
    ...
    if (found)
    {
        ...
        rep(i, mowers.size())
        {
            if (accepted[i])
            {
                if (cheapest == -1) cheapest = get<1>(mowers[i]);
                if (get<1>(mowers[i]) > cheapest) break;
                cout << get<0>(mowers[i]) << "\n";
            }
        }
    }
