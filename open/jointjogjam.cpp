...
int32_t main()
{
    ...
    p2 ks; read(ks.first); read(ks.second);
    p2 os; read(os.first); read(os.second);
    p2 ke; read(ke.first); read(ke.second);
    p2 oe; read(oe.first); read(oe.second);
    ...
    cout << fixed << setprecision(15) << max(dist(ks, os), dist(ke, oe));
