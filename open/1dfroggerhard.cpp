...
int currcomponent = 0;
int assigncomponent(int u, vi& jump, vi& color, vp2& cyclehead, vi& components)
{
    if (components[u] != -1) return components[u];
    if (jump[u] + u < 0 || jump[u] + u >= jump.size())
    {
        cyclehead.emplace_back(-1, u);
        return (components[u] = currcomponent++);
    }
    else if (color[u] == 1)
    {
        cyclehead.emplace_back(u, u);
        return (components[u] = currcomponent++);
    }

    color[u] = 1;
    components[u] = assigncomponent(jump[u] + u, jump, color, cyclehead, components);
    color[u] = 2;
    return components[u];
}

p2 counttree(int u, int p, multiset<int>& used, vvi& edges, set<int>& forbidden, vi& jump)
{


    p2 ret = { 0,1 };

    bool valid = !setcontains(used, jump[u]) && !setcontains(forbidden, jump[u]);
    used.insert(jump[u]);
    repe(e, edges[u])
    {
        if (e == p) continue;
        p2 r = counttree(e, u, used, edges, forbidden, jump);

        ret += r;
        if (valid) ret.first += r.second;
    }
    if (valid) ret.first++;
    eraseOne(used, jump[u]);
...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        assigncomponent(i, jump, color, cyclehead, component);
    }
    ...
    rep(i, n)
    {
        nodesincomp[component[i]].push_back(i);
    }
    int ans = 0;

    vector<set<int>> compnums;
    repe(comp, nodesincomp)
    {
        set<int> nums;
        repe(i, comp) nums.insert(jump[i]);
        set<int> forbidden;

        repe(c, comp)
        {
            if (incycle[c] != -1 && setcontains(nums, jump[c]))
            {
                ans += comp.size();
                nums.erase(jump[c]);
                forbidden.insert(jump[c]);
            }
        }

        compnums.emplace_back(forbidden);
    }
    ...
    {
        int u = i + jump[i];
        if (isvalid(u) && incycle[i] == -1 && incycle[u] != -1)
        {
            roots.push_back(i);
        }
    }
    repe(c, cyclehead) if (c.first == -1) roots.push_back(c.second);


    repe(root, roots)
    {
        int p = root + jump[root];
        multiset<int> used;

        ans += counttree(root, p, used, edges, compnums[component[root]], jump).first;
    }
