...
int32_t main()
{
	...
	while (getline(cin, nline))
	{
		...
		rep(i, n) getline(cin, queries[i]);
		string text;
		getline(cin, text);

		SuffixArray suf(text);

		repe(pattern, queries)
		{
			vi occurences;

			int l = -1, r = suf.a.size();  // Initialize left and right indexes
			while (r - l > 1)
			{
				// See if 'pat' is prefix of middle suffix in suffix array
				int mid = (l + r) / 2;
				int res = strncmp(pattern.c_str(), &text[suf.a[mid]], pattern.size());
			...
			if (strncmp(pattern.c_str(), &text[suf.a[left]], pattern.size()) == 0)
			{
				repp(i, left, r)
				{
					occurences.push_back(suf.a[i]);
				}
			}


			sort(all(occurences));
			repe(oc, occurences) cout << oc << " ";
			cout << "\n";
		}
	}
