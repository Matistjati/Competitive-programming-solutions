...
int32_t main()
{
    ...
    dread5(int, a, b, t, ca, cb);
    dread(int, c);
    vp2 points(c);
    rep(i, c) read(points[i].first), read(points[i].second);

    Tree2D tree(points);


    int ans = tree.query(0, a, 0, b);
    rep(i, c)
    {
        int cost = 0;
        if (points[i].first > a) cost += (points[i].first - a) * ca;


        if (cost <= t)
        {
            int incb = (t - cost) / cb;
            ans = max(ans, tree.query(0, max(a, points[i].first), 0, b + incb));
        }
    }
    ...
    {
        int cost = 0;
        if (points[i].second > b) cost += (points[i].second - b) * cb;


        if (cost <= t)
        {
            int inca = (t - cost) / ca;
            ans = max(ans, tree.query(0, a + inca, 0, max(b, points[i].second)));
        }
    }
    cout << ans;
