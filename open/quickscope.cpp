...
int32_t main()
{
    ...
    vector<vector<string>> topop;
    topop.push_back({});
    map<string, vector<pair<int,string>>> variables;
    int scope = 0;
    rep(i, n)
    {
        dread(string, cmdtype);
        if (cmdtype=="{")
        {
            topop.push_back({});
            scope++;
        ...
        else if (cmdtype=="}")
        {
            ...
            repe(s, topop.back())
            {
                while (variables[s].size() && variables[s].back().first > scope) variables[s].pop_back();
            }
            topop.pop_back();
        }
        else if (cmdtype=="TYPEOF")
        {
            dread(string, q);
            while (variables[q].size() && variables[q].back().first > scope) variables[q].pop_back();
            if (variables[q].size())
            {
                cout << variables[q].back().second << "\n";
            }
            else
            {
                cout << "UNDECLARED\n";
        ...
        {
            dread2(string, a,b);
            if (variables[a].size()&& variables[a].back().first==scope)
            {
            ...
            {
                variables[a].emplace_back(scope, b);
                topop.back().push_back(a);
            }
        }
    }
