...
p2 solve(vvp2& dp, vvb& dpd, vi& nums, vi& ops, int a, int b)
{
    ...
    if (!dpd[a][b])
    {
        ...
        repp(i, a, b)
        {
            p2 left = solve(dp, dpd, nums, ops, a, i);
            p2 right = solve(dp, dpd, nums, ops, i+1, b);
            rep(j, 2)
            {
                rep(k, 2)
                {
                    int v = applyoperation(indexpair(left, j), indexpair(right, k), ops[i]);
                    chmax(dp[a][b].first, v);
                    chmin(dp[a][b].second, v);
                }
            }
        }
    }
...
int32_t main()
{
    ...
    vvb dpd(ops.size()+1, vb(ops.size()+1));
    vvp2 dp(ops.size()+1, vp2(ops.size()+1, p2(-inf,inf)));

    cout << solve(dp,dpd,nums,ops,0,ops.size()).first;
