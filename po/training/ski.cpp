...
int findcycle(int u)
{
    ...
    if (color[u] == TREE || color[u] == CYCLE) return -1;
    color[u] = VISITING;

    int ret = findcycle(edge[u]);

    color[u] = TREE;
    return ret;
}

int colorcycle(int u, int which, int depth)
{
    if (color[u] == CYCLE) return 0;
    color[u] = CYCLE;
    whichcycle[u] = which;
    cycledepth[u] = depth;
    return colorcycle(edge[u], which, depth + 1) + 1;
}
...
p2 handletree(int u)
{
    ...
    {
        p2 o = handletree(e);
        ret.first = max(ret.first, o.first - 1);
        ret.second += o.second;
    }
...
int32_t main()
{
    ...
    color.resize(n);
    whichcycle.resize(0);
    whichcycle.resize(n, -1);
    cycledepth.resize(0);
    cycledepth.resize(n, -1);
    ...
    rep(i, n)
    {
        int cyclehead = findcycle(i);
        if (cyclehead != -1)
        {
            cyclen.push_back(colorcycle(cyclehead, cyclecnt++, 0));
    ...
    rep(i, n)
    {
        if (color[i] == TREE && color[edge[i]] == CYCLE)
        {
            roots.push_back(i);
        }
    }

    vector<map<int, int>> refills(cyclecnt);
    int ans = 0;
    repe(root, roots)
    {
        p2 r = handletree(root);
        ans += r.second;
        int c = edge[root];
        refills[whichcycle[c]][c] = max(refills[whichcycle[c]][c], r.first);
    }
    ...
    rep(i, cyclecnt)
    {
        ...
        rep(_, len * 2)
        {
            if (setcontains(refills[i], u)) paint = max(paint, refills[i][u]);
            if (paint > 0) isdone[u] = 1;
            paint--;
            u = edge[u];
        }
    }
    ...
    vvi cyclenodes(cyclecnt);
    rep(i, n)
    {
        if (isdone[i] && whichcycle[i] != -1)
        {
            cyclenodes[whichcycle[i]].push_back(cycledepth[i]);
        }
    }
    ...
    rep(i, cyclen.size())
    {
        ...
        rep(i, len * 2)
        {
            ...
            {
                nextzero[i % len] = i % len;
                while (ones.size()) nextzero[ones.top()] = i % len, ones.pop();
            }
        }
        ...
        rep(s, min(len, k + 1))
        {
            ...
            while (1)
            {
                int nextpos = nextzero[pos];
                if (nextpos < pos) steps += len - pos + nextpos;
                else steps += nextpos - pos;
                if (steps >= len) break;
                pos = (nextpos + k) % len;
                steps += k;
                paints++;
