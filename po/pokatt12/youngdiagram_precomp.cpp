...
int goal = 80;
p2 doit(int i, int tot, int prev, vvvp2& dp)
{
    if (tot == goal) return { 1,0 };
    p2& v = dp[i][tot][prev];
    if (v.first != -1) return v;

    p2 ret = { 0, 0 };
    repp(j, prev, goal - tot + 1)
    {
        p2 r = doit(i + 1, tot + j, j, dp);
        ret.first += r.first;
        ret.second = max(ret.second, r.first);
    }
...
vector<pair<int, vi>> best;


void findall(int prev, int s, vi& placement)
{
    if (s <= goal) reachable.push_back(placement);

    repp(i, prev, goal + 1)
    {
        if (s + i > goal) break;
        placement.push_back(i);
        findall(i, s + i, placement);
        placement.pop_back();
    }
}

int precompreachable()
{
    vi placement = {};
    findall(1, 0, placement);
    vi bestV;
    int most = 0;
    sort(all(reachable));
...
int solvestupid(int s)
{
    precompreachable();

    int bestcover = 0;
    int b = -1;
    rep(i, reachable.size()) reverse(all(reachable[i]));
    rep(i, reachable.size())
    {
        if (accumulate(all(reachable[i]), 0) == s)
        {
            int cover = 0;
            rep(j, reachable.size()) if (i != j)
            {
                if (reachable[j].size() > reachable[i].size()) continue;
                bool works = 1;

                rep(k, reachable[j].size()) if (reachable[j][k] > reachable[i][k]) works = 0;
...
int num_covered(int prev, int i, vi& placement)
{
    ...
    repp(diff, 1, min(prev + 1, placement[i] + 1))
    {
...
void findbest(int prev, int s, vi& placement)
{
    ...
    repp(i, 1, prev + 1)
    {
        ...
        version++;
        int covered = num_covered(goal, 0, placement);
        if (covered > best[s].first)
        {
            best[s].first = covered;
            best[s].second = placement;
        }

        findbest(i, s, placement);
        placement.pop_back();
        s -= i;
    }
}
...
struct Node
{
    ...
    {
        nodecnt++;
        repp(diff, max(1LL, num), goal + 1)
        {
            if (sum + diff > goal) break;
            children[diff] = new Node(diff, sum + diff);
        }
    }
};
...
{
    best.resize(goal + 1);
    coverdp.resize(goal + 1, vp2(goal + 1, { 0,-1 }));
    vi pl;
    findbest(goal, 0, pl);
    ...
    return 0;
    int n = goal + 1;
    // position, sum, last value
    vvvi dp(n + 1, vvi(n + 1, vi(n + 1, 0)));
    vvvi dp2(n + 1, vvi(n + 1, vi(n + 1, 0)));
    dp[0][1][0] = 1;
    ...
    repp(i, 0, n)
    {
        rep(prev, n + 1)
        {
            rep(sum, n + 1)
            {
                repp(diff, prev, n + 1)
                {
                    if (sum + diff >= n + 1) break;
                    dp[i + 1][diff][sum + diff] += dp[i][prev][sum];
                }
    ...
    int m = 0;
    rep(i, n + 1) rep(j, n + 1) m = max(dp2[i][j][goal], m);

    cout << m << " " << solvestupid(goal);

    return 0;

    int s = 0;
    rep(i, n + 1) rep(j, n + 1) s += dp[i][j][goal];
    ...
    rep(i, reachable.size())
    {
        if (accumulate(all(reachable[i]), 0) == goal)
        {
            brute++;
        }
    }

    vvvp2 dptable(goal + 1, vvp2(goal + 1, vp2(goal + 1, { -1,-1 })));
    int dptop = doit(0, 0, 1, dptable).first;

    cout << s << " " << dptop << " " << brute << " " << m;
