...
vi valid;
int best(int l, int r, bool p1)
{
    p3 state = { l,r,p1 };
    if (setcontains(dp, state)) return dp[state];
    if (l > r) return p1;
    if (r < l) return p1;
    if (r < valid[l]) return p1;

    int ret = !p1;
    if (best(l + 1, r, !p1) == p1) ret = p1;
    if (best(l, r - 1, !p1) == p1) ret = p1;
    return dp[state] = ret;
}
...
int32_t main()
{
    ...
    rep(i, n)
    {
        ...
        valid[i] = j - 1;
        count[creams[i]]--;
        if (count[creams[i]] == 0) nonzero--;
    }


    valid.push_back(n - 1);

    rep(i, n) if (valid[i] == -1) valid[i] = inf;
    ...
    rep(i, n)
    {
        int u = valid[i];
        if (u == inf) continue;
        while (u < n && u <= valid[i + 1])
        {
            diags[i + u] = best(i, u, 1);
            u++;
        }
    }
