...
int32_t main()
{
    ...
    {
        dread3(int, s, e, l);
        edges[s].emplace_back(e, l);
    }

    rep(i, m + 1)
    {
        if (i != 0) edges[i].emplace_back(i - 1, g);
        if (i != m) edges[i].emplace_back(i + 1, g);
    }
    ...
    while (pq.size())
    {
        ...
        repe(edge, edges[pos])
        {
            ...
            if (d<dist[edge.first])
            {
                ...
                {
                    inq[edge.first] = true;
                    if (pq.size() && dist[pq.front()] < dist[edge.first]) pq.push_back(edge.first);
                    else pq.push_front(edge.first);
