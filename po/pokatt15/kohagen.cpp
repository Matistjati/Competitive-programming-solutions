...
void sortfast(vector<double>& row)
{
    if (row.size() <= 1) return;
    bool incstart = row[1] > row[0];

    static vector<double> temprow(401);
    int tempInd = 0;
    ...
    if (incstart)
    {
        while (hi >= lo)
        {
            if (row[lo] < row[hi])
            {
                temprow[tempInd++] = row[lo];
                lo++;
            }
            else
            {
                temprow[tempInd++] = row[hi];
                hi--;
            }
        }
    }
    else
    {
        ...
        reverse(temprow.begin(), temprow.begin() + row.size());
    }
    rep(i, row.size()) row[i] = temprow[i];
}
...
int32_t main()
{
    ...
    vector<double> sinv(361);
    rep(i, 361) sinv[i] = sin(i);

    dread3(int, n, m, r);
    readvector(double, angles, n);
    sort(all(angles));
    rep(i, n) angles[i] = angles[i] / 360. * 6.28318530718;

    double ans = 1e42;

    double target = m * 2. / r / r;
    vector<vector<double>> sindiff(n, vector<double>(n));
    rep(i, n) rep(j, n) sindiff[i][j] = sin(angles[i] - angles[j]);

    vector<vector<double>> sindiffswap(n, vector<double>(n));
    rep(i, n) rep(j, n) sindiffswap[i][j] = sin(angles[j] - angles[i]);

    rep(i, n)
    {
        vector<vector<double>> reachable(n);
        repp(j, i + 1, n)
        {
            repp(k, j + 1, n)
            {
                double area = sindiffswap[i][j] + sindiffswap[j][k];

                reachable[k].emplace_back(area);

            }
        }
        rep(j, n) sortfast(reachable[j]);

        vector<vector<double>> queries(n);
        repp(k, i + 1, n)
        {
            repp(l, k + 1, n)
            {
                double v = sindiffswap[k][l] + sindiff[i][l];
                queries[k].push_back(v);
            }
        }
        rep(j, n) sortfast(queries[j]);



        rep(k, n)
        {
            int l = reachable[k].size() - 1;
            if (reachable[k].empty()) continue;

            rep(j, queries[k].size())
            {
                double q = queries[k][j] - target;
                while (l > 0 && q + reachable[k][l - 1] >= 0)
                {
                    l--;
                }
                q += reachable[k][l] + target;
                if (q >= target)
                {
                    ans = min(ans, q);
                }
    ...
    if (ans == 1e42) cout << -1;
    else cout << setprecision(20) << ans * r * r / 2.;
    quit;
}
