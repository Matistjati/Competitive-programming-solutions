...
int32_t main()
{
    ...
    index++;
    vi nextinterval(index, -inf);

    int p = 0;

    rep(i, index)
    {
        if (illegalIntervals.empty()) break;
        while (illegalIntervals[p] < i)
        {
            p++;
            if (p == illegalIntervals.size()) goto done;
        }
    ...
    while (q--)
    {
        ...
        left = compress[left - 1];
        right = compress[right - 1];

        int idx = nextinterval[left];
        // There are no illegal intervals to the right of left
        if (idx == -inf)
        {
            cout << "ja\n";
            continue;
        ...
        int firstIllegal = illegalIntervals[idx];
        // The illegal interval lies outside of right
        if (firstIllegal + m > right)
        {
            cout << "ja\n";
        }
        else
