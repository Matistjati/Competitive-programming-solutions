...
#define pd3 tuple<double,double,int>
...
int kattis(int n, int h, int x[], int y[], int z[])
{
	...
	rep(i, n)
	{
		while (hull.size() > 1 && cross(hull[hull.size() - 2], hull[hull.size() - 1], mp(x[i], y[i])) >= 0) hull.pop_back();
		hull.push_back(mp(x[i], y[i]));
		if (z[i])
		{
			pair<double, double> bounds = { 0,x[n - 1] };
			if (hull.size()>1 && hull[hull.size()-2].second > hull.back().second)
			{
