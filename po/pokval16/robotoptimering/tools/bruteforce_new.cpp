...
inline int calcscore_slow(vector<instruction>& program, int extra = -1, bool random = true)
{
    ...
    //           0   1   2    3    4    5    6  7
    vi highs = { 150,75, 10, 70,  100,   5, 150, 50 };
    int high = highs[t];
...
void buildprogram(vector<instruction>& program)
{
    ...
    rep(i, program.size())
    {
        ...
        if (inst.what == gotO || inst.what == gotoblocked || inst.what == call)
        {
            ...
            debif(inst.target < 0);
            //debif(inst.target == program.size() - 1);
        }
    ...
    while (cost < costlen)
    {
        if (usegoto(rng))
        {
            ...
            case loop:
                ...
                looplevel++;
                maxnestingseen = max(maxnestingseen, looplevel);
            ...
            if (looplevel==0) addi(label, "l" + to_string(i));
            i++;
        }
        else
        {
...
//
//    p2 ending = simulate();
//    int newscore = calcscore_slow(program, int(1e7), false);
...
#define J 0
    ...
    //               0    1   2    3    4    5   6   7
    vi scorereqs = { 175, 44, 98, 685, 800, 69, 100, 125 };
    ...
    while (true)
    {
        ...
        if (newscore < scorereq) newscore = min(newscore, calcscore_slow(program, 100, false));
