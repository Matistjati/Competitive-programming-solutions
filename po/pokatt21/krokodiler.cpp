...
int M;
inline int h(int f, int s, int d)
{
    return f * 4 * M + 4 * s + d;
}

inline int h2(int r, int c)
{
    return r * M + c;
}
...
int findnext(int u, int dir, vi& jump, string& grid)
{
    if (u == inf) return u;
    if (grid[u] != '.')
    {
...
p2 dfs(int u, string& grid, vi& jump, vi& canbemoved)
{
    int ans = 0;
    while (true)
    {
        ...
        int next = findnext(jump[u * 4 + dirmap[grid[u]]], dirmap[grid[u]], jump, grid);
