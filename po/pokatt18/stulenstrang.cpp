...
int best(int i, vi state, int h, bool playerA)
{
    if (i == neck.size())
    {
        //debif(state == balanced && cuts == 1);
        return state == balanced ? 0 : inf;
    }

    assert(h<int(3e6));

    int& v = dp[i + playerA * neck.size()][h];
    if (v != -1) return v;
    ...
    else
    {
        ...
        {
            ret = min(ret, best(i + 1, state, h, 0));
        }
    }
...
int32_t main()
{
    ...
    repe(c, s)
    {
        if (!setcontains(mapping, c)) mapping[c] = mapping.size();
        neck.push_back(mapping[c]);
    }
    ...
    if (s.size() > 40)
    {
        ...
        vi invcnt(k);
        rep(i, k) invcnt[i] = balanced[i];
        rep(i, neck.size())
        {
            possible.erase(possible.find(suf[i]));
            cnt[neck[i]]++;
            invcnt[neck[i]]--;

            if (cnt == balanced || setcontains(possible, invcnt))
            {
                works = 1;
            }
        }
    ...
    else
    {
        ...
        hashfactor.resize(mapping.size(), 1);
        balanced.resize(mapping.size());
        rep(i, cnt.size())
        {
            balanced[i] = cnt[i] / 2;
            repp(j, i + 1, mapping.size()) hashfactor[j] *= (balanced[i] + 1);
        }

        vi state(mapping.size());
        cout << best(0, state, 0, 0);
    }
