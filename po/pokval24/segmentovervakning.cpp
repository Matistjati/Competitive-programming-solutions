...
int32_t main()
{
    ...
    rep(i, q)
    {
        ...
        {
            dread(int, lo, hi);
            if (lo <= l && hi >= r) coversall.push_back(id);
            if (lo <= l) lq.emplace(hi, id);
            if (hi >= r) rq.emplace(-lo, id);
        }
        else
        {
            removed[id] = 1;
        }
        while (coversall.size() && removed[coversall.back()]) coversall.pop_back();
        while (lq.size() && removed[lq.top().second]) lq.pop();
        while (rq.size() && removed[rq.top().second]) rq.pop();

        if (coversall.size()) cout << "1\n";
        else if (lq.size() && rq.size() && lq.top().first >= (-rq.top().first)) cout << "2\n";
        else cout << "-1\n";
    }
