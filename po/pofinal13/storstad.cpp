...
int32_t main()
{
    ...
    per(i, n-1) x[i].second += x[i + 1].second + xcoords[i];

    repp(i, 1, n) y[i].first += y[i - 1].first + ycoords[i];
    per(i, n-1) y[i].second += y[i + 1].second + ycoords[i];


    int ans = inf;

    // We can do better if we sort the queries (?)
    rep(i, m)
    {
        int d = 0;
