...
int a3 = 0;
int a4 = 0;
void brute(int str, int sz, vi strengths, vi choice)
{
    bool all_zero = true;
    rep(i, 4) if (strengths[i]) all_zero = false;
    if (all_zero)
    {
        if (str != 0 || sz != 0) return;

        cout << a1 + choice[0] << " " << a2 + choice[1] << " " << a3 + choice[2] << " " << a4 + choice[3];
    ...
    rep(i, 4)
    {
        if (strengths[i])
        {
            vi newstrengths(strengths);
            vi newchoice(choice);
            newchoice[i]++;
            newstrengths[i]--;
            brute(str - (i + 1), sz - 1, newstrengths, newchoice);
            newstrengths = vi(strengths);
            newchoice = vi(choice);
            newstrengths[i]--;
            brute(str + (i + 1), sz + 1, newstrengths, newchoice);
...
int32_t main()
{
    ...
    // At this point, both teams have equal strength and size
    brute(0, 0, { s1,s2,s3,s4 }, { 0,0,0,0 });
