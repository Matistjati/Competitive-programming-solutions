...
p2 deepest(int u, int p, int d, vvi& edges, vb& isfruit)
{
...
int up[500000 + 10][20];
void finddepth(int u, int p, int d, vvi& edges, vb& isfruit, vp2& depth)
{
    up[u][0] = p;
...
void dfsdown(int u, int p, vb& vis, vvi& edges, vi& dist)
{
    if (vis[u]) return;
    vis[u] = 1;
    repe(e, edges[u])
...
string solvesmart(vp2& edgelist, vi& fruits)
{
    ...
    rep(i, n - 1)
    {
        int a = edgelist[i].first - 1;
        int b = edgelist[i].second - 1;
        edges[a].push_back(b);
        edges[b].push_back(a);
    }
    ...
    int root = deepest(fruits[0], fruits[0], 0, edges, isfruit).second;
