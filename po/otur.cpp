...
int countD(vvvvi& memo, vi& digits, bool unlimited, int index, int currDigit, bool has13)
{
    ...
    int& v = memo[index][currDigit + 1][has13][unlimited];
    ...
    rep(i, limit)
    {
        ...
        ans += countD(memo, digits, unlimited || (i < digits[index]), index - 1, i, has13 || (currDigit == 1 && i == 3));
    }
...
{
    vvvvi memo(30, vvvi(11, vvi(2, vi(2, -1))));
    ...
    return countD(memo, digits, false, digits.size() - 1, -1, false);
}



int binsearch(int upper, int n)
{
    int low = 0;
    int high = upper;
    while (true)
