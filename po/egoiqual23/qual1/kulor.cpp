...
int solve(string top, string bottom, char topcolor)
{
    int n = top.size();
    ...
    rep(i, n)
    {
        if (top[i] != topcolor && bottom[i] == topcolor)
        {
            swap(top[i], bottom[i]);
            ans++;
        }
...
int32_t main()
{
    ...
    cout << min(solve(top, bottom, 'S'), solve(top, bottom, 'V'));
