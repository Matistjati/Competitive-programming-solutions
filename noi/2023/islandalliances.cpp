...
struct UF
{
    ...
    vi par;
    vi size;
    vector<fast_set<int>> mine;

    UF(int n) : par(n), size(n, 1), mine(n), enemies(n)
    {
        rep(i, n) par[i] = i;
        rep(i, n) mine[i].insert(i);
    ...
    int merge(int a, int b)
    {
        ...
        repe(enemy, enemies[b])
        {
            if (setcontains(mine[a], enemy))
            {
                return 0;
            }
        }
        ...
        par[b] = a;
        size[a] += size[b];
        repe(enemy, enemies[b]) enemies[a].push_back(enemy);
        repe(t, mine[b]) mine[a].insert(t);
        return 1;
    }
};
...
int32_t main()
{
    ...
    rep(i, m)
    {
        dread2(int, a, b);
        a--; b--;
        uf.enemies[a].push_back(b);
        uf.enemies[b].push_back(a);
    }
