...
vi prefwhite;
vi prefblack;
int dp[102][int(2e5 + 10)];
int dpd[102][int(2e5 + 10)];
...
int BLACK = 2;
int BLACKMANY = 4;
int possible(int i, int t)
{
    if (dpd[t][i]) return dp[t][i];
    if (i >= n)
    {
        return t == k;
    }
    ...
    if (t < k)
    {
        ...
        {
            if (t + 1 == k && getsum(prefwhite, i, p - 1) == 0)
            {
                cols |= BLACKMANY;
            }
        }
        else if ((s[p] == '.' || s[p] == '_') && getsum(prefwhite, i, p - 1) == 0)
        {
            if (possible(i + c[t] + 1, t + 1))
            {
                cols |= BLACKMANY;
            }
        }
...
std::string solve_puzzle(std::string S, std::vector<int> C) {
    ...
    rep(i, n)
    {
        ...
        if (s[i] == 'X') prefblack[i]++;
    }
    repp(i, 1, n) prefwhite[i] += prefwhite[i - 1];
    repp(i, 1, n) prefblack[i] += prefblack[i - 1];
    ...
    rep(i, n)
    {
        if (anywhite[i] && !anyblack[i])
        {
            ans[i] = '_';
        }
        if (anyblack[i] && !anywhite[i])
        {
            ans[i] = 'X';
        }
    }
