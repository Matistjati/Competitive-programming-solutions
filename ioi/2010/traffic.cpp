...
vi subtreewt;
vi subtreemax;
int count(int u, int p, vvi& edges, int cost[])
{
    int ret = cost[u];
    repe(e, edges[u])
    {
        if (e == p) continue;
        int k = count(e, u, edges, cost);
        subtreewt[u] += k;
        subtreemax[u] = max(subtreemax[u], k);
        ret += k;
    }
    return ret;
}
...
int LocateCentre(int n, int p[], int s[], int d[])
{
    ...
    rep(i, n)
    {
        int k = max(subtreemax[i], t - subtreewt[i] - p[i]);
        if (k < best)
        {
            best = k;
            ret = i;
...
int32_t main()
{
    ...
    cout << LocateCentre(n, &nums[0], &ea[0], &eb[0]);
