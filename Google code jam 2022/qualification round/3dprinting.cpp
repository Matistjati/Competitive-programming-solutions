...
int32_t main()
{
    ...
    {
        vvi printers(3, vi(4));
        rep(i, 3) rep(j, 4) read(printers[i][j]);

        int target = (int)(1e6);

        vi highest(4, inf);
        rep(i, 3) rep(j, 4) chmin(highest[j], printers[i][j]);

        if (accumulate(all(highest), 0)>=target)
        {
            int s = accumulate(all(highest), 0)-target;
            rep(i, 4)
            {
                if (highest[i] < s)
                {
