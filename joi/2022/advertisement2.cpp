...
int32_t main()
{
    ...
    vp2 points(n);
    rep(i, n) read(points[i].first), read(points[i].second);
    set<p2> pointset(all(points));
    points = vp2(all(pointset));
    n = points.size();

    sort(all(points), [](p2& l, p2& r) {
    ...
    rep(i, n)
    {
        auto it = seen.upper_bound(points[i].first - points[i].second);
        if (it != seen.begin())
        {
            done.insert(points[i]);
        }
        seen.insert(points[i].first - points[i].second);
    }
    seen = set<int>();
    ...
    rep(i, n)
    {
        auto it = seen.lower_bound(points[i].first + points[i].second);
        if (it != seen.end())
        {
            done.insert(points[i]);
        }
