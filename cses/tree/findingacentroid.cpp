...
int dfs(int u, int p, vi& numchildren, vi& maxchild, vvi& edges)
{
	int ret = 1;
	repe(e, edges[u])
	{
		if (e == p) continue;
		int k = dfs(e, u, numchildren, maxchild, edges);
		ret += k;
		maxchild[u] = max(maxchild[u], k);
	}
	return numchildren[u] = ret;
}
...
int32_t main()
{
	...
	vi numchildren(n);
	vi maxchild(n);
	dfs(0, 0, numchildren, maxchild, edges);
	int sz = inf;
	int best = -1;
	rep(i, n)
	{
		int m = maxchild[i];
		m = max(m, n - numchildren[i]);
		if (m < sz)
		{
			sz = m;
			best = i;
