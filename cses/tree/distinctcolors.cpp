...
set<int> dfs(int u, int p, vi& col, vvi& edges, vi& ans)
{
	set<int> cols = { col[u] };
	repe(e, edges[u])
	{
		if (e == p) continue;
		set<int> k = dfs(e, u, col, edges, ans);
		if (cols.size() < k.size()) swap(cols, k);
		repe(u, k) cols.insert(u);
	}
