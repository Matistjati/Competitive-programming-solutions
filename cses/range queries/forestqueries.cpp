...
struct Tree2D
{
	vector<Tree> tree;
	int n;
	Tree2D(int n) : n(n), tree(n * 4, Tree(n)) {}

	void update(int x, int l, int r, int a, int b, int v)
	{
		if (l > a || r < a) return;
		if (l == r) tree[x].update(b, v);
		else
		{
			int mid = (l + r) / 2;
			if (a <= mid) update(x * 2, l, mid, a, b, v);
			else update(x * 2 + 1, mid + 1, r, a, b, v);
			tree[x].update(b, v);
		}
	}

	int query(int x, int l, int r, int ql, int qr, int al, int ar)
	{
		if (l > qr || r < ql) return 0;
		if (l >= ql && r <= qr) return tree[x].query(al, ar);
		int mid = (l + r) / 2;
		return query(x * 2, l, mid, ql, qr, al, ar) + query(x * 2 + 1, mid + 1, r, ql, qr, al, ar);
	}
};
...
int32_t main()
{
	...
	while (q--)
	{
		dread4(int, a, c, b, d);
		a--; b--; c--; d--;
		cout << tree.query(1, 0, n - 1, a, b, c, d) << "\n";
	}
