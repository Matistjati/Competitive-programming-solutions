...
void remove(int i)
{
    if (numset[nums[i]] == 1) ans--;
    numset[nums[i]]--;
}
void add(int i)
{
    if (numset[nums[i]] == 0) ans++;
    numset[nums[i]]++;
}
int get_answer()
{
...
vector<int> moqueries(vector<Query> queries) {
...
int32_t main()
{
    ...
    numset.resize(n);
    rep(i, n) read(nums[i]);
    set<int> sor(all(nums));
    map<int, int> idx;
    repe(num, sor) idx[num] = idx.size();
    rep(i, n) nums[i] = idx[nums[i]];
