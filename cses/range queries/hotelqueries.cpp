...
struct Tree
{
	...
	{
		if (l == r) return tree[x] >= v ? ind[x] : -1;
		int mid = (l + r) / 2;
		if (tree[x * 2] >= v) return query(x * 2, l, mid, v);
		else return query(x * 2 + 1, mid + 1, r, v);
	}
};
