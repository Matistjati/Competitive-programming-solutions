...
int timer = 0;
void dfs(int u, int p, vvi& edges, vp2& tour)
{
	tour[u].first = timer++;

	repe(e, edges[u]) if (e != p) dfs(e, u, edges, tour);
...
int32_t main()
{
	...
	rep(i, n)
	{
		tree.update(1, 0, n * 2 - 1, tour[i].first, value[i]);
	}
	...
	while (q--)
	{
		...
		if (t == 1)
		{
			dread2(int, i, v);
			i--;
			tree.update(1, 0, n * 2 - 1, tour[i].first, v);
		}
		else
		{
			dread(int, s);
			s--;
			cout << tree.query(1, 0, n * 2 - 1, tour[s].first, tour[s].second) << "\n";
		}
	}
