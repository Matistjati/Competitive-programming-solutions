...
int ans(int i, vp2& projects, vi& startpoint, vi& dp)
{
	if (i == startpoint.size()) return 0;
	int& v = dp[i];
	if (v != -1) return v;

	int ret = 0;
	ret = max(ret, ans(i + 1, projects, startpoint, dp));

	auto it = upper_bound(all(startpoint), projects[i].first);
	ret = max(ret, projects[i].second + ans(it - begin(startpoint), projects, startpoint, dp));
...
int32_t main()
{
	...
	rep(i, n)
	{
		int a, b, c;
		tie(a, b, c) = in[i];
		projects.emplace_back(b, c);
		startpoints.push_back(a);
	}
	vi dp(n, -1);
	cout << ans(0, projects, startpoints, dp);
