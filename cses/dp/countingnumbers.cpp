...
int count(int i, int prev, bool unlimited, bool allzero, vi& digits, vvvvi& dp)
{
	if (i == digits.size()) return 1;
	int& v = dp[unlimited][allzero][prev][i];
	if (v != -1) return v;
	...
	rep(d, hi)
	{
		if (d == prev && !(allzero && prev == 0)) continue;
		ret += count(i + 1, d, unlimited || (d < hi - 1), allzero && (d == 0), digits, dp);
	}
...
int ans(int hi)
{
	...
	while (hi)
	{
		digits.push_back(hi % 10);
		hi /= 10;
	}
	reverse(all(digits));
	vvvvi dp(2, vvvi(2, vvi(11, vi(19, -1))));

	return count(0, 10, 0, 1, digits, dp);
}
