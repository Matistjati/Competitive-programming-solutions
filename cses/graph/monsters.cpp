...
int32_t main()
{
	...
	vvi vis(n, vi(m));
	vvp2 visplayer(n, vp2(m, p2(-1, -1)));
	while (q.size())
	{
		int r, c, pr, pc, player;
		tie(r, c, pr, pc, player) = q.front();
		q.pop();
		...
		if (player)
		{
			...
			if (r == 0 || r == n - 1 || c == 0 || c == m - 1)
			{
				...
				while (pos.first != -2)
				{
					p2 p = visplayer[pos.first][pos.second];
					p2 delta = pos - p;
					path += string(1, dir[delta]);
					pos = p;
				}
				cout << "YES\n";
				path.pop_back();
			...
			if (visplayer[r][c].first != -1) continue;
			visplayer[r][c] = p2(pr, pc);
		}
		else
		{
		...
		repe(dir, dirs)
		{
			...
			q.emplace(np.first, np.second, r, c, player);
		}
	}
