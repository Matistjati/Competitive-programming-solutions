...
int32_t main()
{
    ...
    vvp2 par(n, vp2(m, p2(-1, -1)));
    queue<p4> q;
    q.emplace(startpos.first, startpos.second, -2, -2);
    while (q.size())
    {
        int r, c, pr, pc;
        tie(r, c, pr, pc) = q.front(); q.pop();

        if (par[r][c].first != -1) continue;
        par[r][c] = p2(pr, pc);
        ...
        {
            p2 np = p2(r, c) + dir;
            if (np.first < 0 || np.second < 0 || np.first >= rows.size() || np.second >= rows[0].size() || rows[np.first][np.second] == '#') continue;

            q.emplace(np.first, np.second, r, c);
        }
    }
