...
void topsort(int u, vi& ordering, vi& vis, vvi& edges)
{
    if (vis[u]) return;
    vis[u] = 1;

    repe(e, edges[u]) topsort(e, ordering, vis, edges);

    ordering.push_back(u);
}

void dfs(int u, int c, vi& color, vi& vis, vvi& edges)
{
    if (vis[u]) return;
    vis[u] = 1;
    color[u] = c;

    repe(e, edges[u]) dfs(e, c, color, vis, edges);
}
