...
struct Tree
{
	...
	vi sum;
	vi lazy;
	Tree(int n) : tree(n * 4), lazy(n * 4), sum(n * 4) {}
	...
	int sumquery(int x, int l, int r, int ql, int qr)
	{
		if (l > qr || r < ql) return 0;
		if (l >= ql && r <= qr) return sum[x];
		push(x);
		int mid = (l + r) / 2;
		return sumquery(x * 2, l, mid, ql, qr) + sumquery(x * 2 + 1, mid + 1, r, ql, qr);
	}
};
...
int32_t main()
{
	...
	while (q--)
	{
		...
		if (t == 1)
		{
			dread2(int, i, v);
			i--;
			tree.add(1, 0, n - 1, i, n - 1, -value[i]);
			value[i] = v;
			tree.add(1, 0, n - 1, i, n - 1, value[i]);
			tree.set(1, 0, n - 1, i, v);
		}
		else
		{
			dread2(int, l, r);
			l--; r--;
			cout << max(0LL, tree.query(1, 0, n - 1, l, r) - tree.sumquery(1, 0, n - 1, 0, l - 1)) << "\n";
		}
	}
