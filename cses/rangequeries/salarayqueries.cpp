...
int32_t main()
{
	...
	vi pos;
	repe(p, positions) pos.push_back(p);

	rep(i, n)
	{
		int s = lower_bound(all(pos), salary[i]) - begin(pos);
		tree.update(s, 1);
	}
	...
	repe(qu, queries)
	{
		...
		if (t == 0)
		{
			auto it = lower_bound(all(pos), a);
			if (it != pos.end()) a = it - begin(pos);
			else a = prev(it) - begin(pos);
			it = upper_bound(all(pos), b);
			if (it != pos.end()) b = it - begin(pos) - 1;
			else b = n + q;

			cout << tree.query(b) - tree.query(a - 1) << "\n";
		}
		else
		{
			int p = lower_bound(all(pos), salary[a]) - begin(pos);
			tree.update(p, -1);
			salary[a] = b;
			p = lower_bound(all(pos), salary[a]) - begin(pos);
			tree.update(p, 1);
		}
	}
