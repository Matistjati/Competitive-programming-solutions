...
int32_t main()
{
    ...
    rep(i, k)
    {
        ...
        if (a > b) swap(a, b);
        dead.emplace(a, b);
        deadlink.emplace_back(a, b);
    }

    UF uf(n);
    rep(i, m)
    {
        if (!setcontains(dead, initial[i]))
        {
            uf.merge(initial[i].first, initial[i].second);
        }
    }
    vi ans(k);
    reverse(all(deadlink));
    rep(i, k)
    {
        ans[i] = uf.numcomps;
        uf.merge(deadlink[i].first, deadlink[i].second);
    }
