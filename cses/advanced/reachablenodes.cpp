...
void topsort(int u, vi& vis, vvi& edges, vi& order)
{
	if (vis[u]) return;
	vis[u] = 1;

	repe(e, edges[u]) topsort(e, vis, edges, order);
	order.push_back(u);
}

void findcomp(int u, vi& vis, vi& nodes, vvi& redges)
{
	if (vis[u]) return;
	vis[u] = 1;
	nodes.push_back(u);
	repe(e, redges[u]) findcomp(e, vis, nodes, redges);
...
int32_t main()
{
	...
	rep(i, n)
	{
		if (!comps.size() || comps.back().size()) comps.push_back({});
		findcomp(order[i], vis, comps.back(), redges);
		repe(u, comps.back()) par[u] = i;
	}

	reverse(all(order));
	vector<bitset<int(5e4 + 10)>> sets(comps.size());
	vis = vi(comps.size());
	rep(i, n)
	{
		int u = order[i];
		...
		{
			vis[par[u]] = 1;
			repe(v, comps[par[u]]) sets[par[u]][v] = 1;
		}

		repe(r, redges[u])
		{
			sets[par[r]] |= sets[par[u]];
		}
	}
	rep(i, n)
	{
		cout << sets[par[i]].count() << " ";
	}
