...
void dc(int l, int r, vp3 queries, vi& ans, vp2& edgelist, UF& uf)
{
	if (l == edgelist.size()) return;
	if (queries.size() == 0) return;
	if (l == r)
	{
		repe(q, queries) ans[get<2>(q)] = l;
		return;
	}
	...
	repp(i, max(0LL, l), mid + 1) uf.merge(edgelist[i].first, edgelist[i].second);
	...
	repe(q, queries)
	{
		...
		else right.emplace_back(q);
	}

	dc(mid + 1, r, right, ans, edgelist, uf);
	repp(i, l, mid + 1) uf.undo();
	dc(l, mid, left, ans, edgelist, uf);
}
...
int32_t main()
{
	...
	rep(i, q)
	{
		...
		else queries.emplace_back(a, b, i);
	}
	UF uf(n);
	dc(0, m, queries, ans, edgelist, uf);
